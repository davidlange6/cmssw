...
MTDTopology::MTDTopology(const int &topologyMode, const BTLValues &btl, const ETLValues &etl)
    : mtdTopologyMode_(topologyMode),
      btlVals_(btl),
      etlVals_(etl),
      bits_per_field{[BTLModule] = {btlVals_.moduleStartBit_, btlVals_.moduleMask_, MTDDetId::BTL},
...
std::string MTDTopology::print(DetId id) const {
  ...
  if (subdet == MTDDetId::BTL) {
    ...
    strstr << "BTL"
           << " Side   " << theSide << side << " Layer  " << theLayer << " Tray   " << theTray << " Module "
           << theModule;
    strstr << " (" << id.rawId() << ")";
    return strstr.str();
  }
  ...
  if (subdet == MTDDetId::ETL) {
    ...
    strstr << "ETL"
           << " Side   " << theSide << side << " Layer  " << theLayer << " Ring   " << theRing << " Module "
           << theModule;
    strstr << " (" << id.rawId() << ")";
    return strstr.str();
  }
