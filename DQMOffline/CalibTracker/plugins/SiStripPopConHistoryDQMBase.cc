...
bool SiStripPopConHistoryDQMBase::checkForCompatibility(const std::string& otherMetaData) const {
  ...
  edm::LogInfo("DQMHistoryServiceBase") << "[DQMHistoryServiceBase::checkForCompatibility] extracted string "
                                        << previousRun;
...
void SiStripPopConHistoryDQMBase::dqmEndJob(DQMStore::IBooker&, DQMStore::IGetter& getter) {
  ...
  for (const auto& histoParams : histoList_) {
    const std::string keyName{histoParams.getUntrackedParameter<std::string>("keyName")};
    scanTreeAndFillSummary(
        MEs, &m_obj, keyName, histoParams.getUntrackedParameter<std::vector<std::string> >("quantitiesToExtract"));
  }
}

HDQMSummary* SiStripPopConHistoryDQMBase::getObj() const { return new HDQMSummary(m_obj); }
