...
def filters_by_type(process, *types):
    ...
    return (filter for filter in list(six.itervalues(process._Process__filters)) if filter._TypedParameterizable__type in types)

def analyzers_by_type(process, *types):
    "Find all EDAnalyzers in the Process that are instances of the given C++ type."
    return (analyzer for analyzer in list(six.itervalues(process._Process__analyzers)) if analyzer._TypedParameterizable__type in types)

def esproducers_by_type(process, *types):
    "Find all ESProducers in the Process that are instances of the given C++ type."
    return (module for module in list(six.itervalues(process._Process__esproducers)) if module._TypedParameterizable__type in types)
