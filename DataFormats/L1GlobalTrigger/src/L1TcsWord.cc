...
// unequal operator
bool L1TcsWord::operator!=(const L1TcsWord& result) const { return !(result == *this); }

// methods

// set the BoardId value from a 64-bits word
void L1TcsWord::setBoardId(const cms_uint64_t& word64) { m_boardId = (word64 & BoardIdMask) >> BoardIdShift; }
...
void L1TcsWord::setBxNr(const cms_uint64_t& word64) { m_bxNr = (word64 & BxNrMask) >> BxNrShift; }
...
void L1TcsWord::setDaqNr(const cms_uint64_t& word64) { m_daqNr = (word64 & DaqNrMask) >> DaqNrShift; }
...
void L1TcsWord::setTriggerTypeWord64(cms_uint64_t& word64, int iWord) {
  if (iWord == TriggerTypeWord) {
    word64 = word64 | (static_cast<cms_uint64_t>(m_triggerType) << TriggerTypeShift);
  }
}

// set the Status value from a 64-bits word
void L1TcsWord::setStatus(const cms_uint64_t& word64) { m_status = (word64 & StatusMask) >> StatusShift; }
...
void L1TcsWord::setLuminositySegmentNrWord64(cms_uint64_t& word64, int iWord) {
  if (iWord == LuminositySegmentNrWord) {
    word64 = word64 | (static_cast<cms_uint64_t>(m_luminositySegmentNr) << LuminositySegmentNrShift);
  }
}

// set the PartRunNr value from a 64-bits word
void L1TcsWord::setPartRunNr(const cms_uint64_t& word64) { m_partRunNr = (word64 & PartRunNrMask) >> PartRunNrShift; }
...
void L1TcsWord::setPartRunNrWord64(cms_uint64_t& word64, int iWord) {
  if (iWord == PartRunNrWord) {
    word64 = word64 | (static_cast<cms_uint64_t>(m_partRunNr) << PartRunNrShift);
  }
}
...
void L1TcsWord::setAssignedPartitionsWord64(cms_uint64_t& word64, int iWord) {
  if (iWord == AssignedPartitionsWord) {
    word64 = word64 | (static_cast<cms_uint64_t>(m_assignedPartitions) << AssignedPartitionsShift);
  }
}
...
void L1TcsWord::setPartTrigNrWord64(cms_uint64_t& word64, int iWord) {
  if (iWord == PartTrigNrWord) {
    word64 = word64 | (static_cast<cms_uint64_t>(m_partTrigNr) << PartTrigNrShift);
  }
}

// set the EventNr value from a 64-bits word
void L1TcsWord::setEventNr(const cms_uint64_t& word64) { m_eventNr = (word64 & EventNrMask) >> EventNrShift; }
...
void L1TcsWord::setOrbitNrFrom(const cms_uint64_t& word64) { m_orbitNr = (word64 & OrbitNrMask) >> OrbitNrShift; }
...
void L1TcsWord::print(std::ostream& myCout) const {
  ...
  myCout << "  DaqNr:               " << std::hex << " hex: "
         << "           " << std::setw(1) << m_daqNr << std::dec << " dec: " << m_daqNr << std::endl;

  myCout << "  TriggerType:         " << std::hex << " hex: "
         << "           " << std::setw(1) << m_triggerType << std::dec << " dec: " << m_triggerType << std::endl;

  myCout << "  Status:              " << std::hex << " hex: "
         << "           " << std::setw(1) << m_status << std::dec << " dec: " << m_status << std::endl;

  myCout << "  LuminositySegmentNr: " << std::hex << " hex: "
         << "        " << std::setw(4) << m_luminositySegmentNr << std::dec << " dec: " << m_luminositySegmentNr
         << std::endl;

  iWord++;
  myCout << "\n Word " << iWord << std::endl;

  myCout << "  PartRunNr:           " << std::hex << " hex: "
         << "    " << std::setw(8) << std::setfill('0') << m_partRunNr << std::setfill(' ') << std::dec
         << " dec: " << m_partRunNr << std::endl;

  myCout << "  AssignedPartitions:  " << std::hex << " hex: "
         << "    " << std::setw(8) << std::setfill('0') << m_assignedPartitions << std::setfill(' ') << std::dec
         << " dec: " << m_assignedPartitions << std::endl;

  iWord++;
  myCout << "\n Word " << iWord << std::endl;

  myCout << "  PartTrigNr:          " << std::hex << " hex: "
         << "    " << std::setw(8) << std::setfill('0') << m_partTrigNr << std::setfill(' ') << std::dec
         << " dec: " << m_partTrigNr << std::endl;

  myCout << "  EventNr:             " << std::hex << " hex: "
         << "    " << std::setw(8) << std::setfill('0') << m_eventNr << std::setfill(' ') << std::dec
  ...
  myCout << "  OrbitNr:             " << std::hex << " hex: "
         << "" << std::setw(12) << std::setfill('0') << m_orbitNr << std::setfill(' ') << std::dec
         << " dec: " << m_orbitNr << std::endl;

  iWord++;
  myCout << "\n Word " << iWord << std::endl;

  myCout << "  Empty word          " << std::endl;
}
...
void L1TcsWord::unpack(const unsigned char* tcsPtr) {
  ...
  const cms_uint64_t* payload = reinterpret_cast<cms_uint64_t const*>(tcsPtr);
  ...
  if (edm::isDebugEnabled()) {
    for (int iWord = 0; iWord < BlockSize; ++iWord) {
      LogTrace("L1GtTcsWord") << std::setw(4) << iWord << "  " << std::hex << std::setfill('0') << std::setw(16)
