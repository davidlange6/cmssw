...
HGCEEDetId::HGCEEDetId(ForwardSubdetector subdet, int zp, int lay, int sec, int subsec, int cell)
  ...
  if (zp > 0)
    id_ |= ((zp & kHGCEEZsideMask) << kHGCEEZsideOffset);
}
  ...
  if (!gen.null()) {
    ...
    if (gen.det() != Forward || (subdet != HGCEE)) {
      throw cms::Exception("Invalid DetId")
          << "Cannot initialize HGCEEDetId from " << std::hex << gen.rawId() << std::dec;
    }
  }
  id_ = gen.rawId();
}
...
std::ostream& operator<<(std::ostream& s, const HGCEEDetId& id) {
  switch (id.subdet()) {
    case (HGCEE):
      return s << "isEE=" << id.isEE() << " zpos=" << id.zside() << " layer=" << id.layer()
