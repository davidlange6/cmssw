...
/*****************************************************************/
void HGCalTriggerBestChoiceTester::beginRun(const edm::Run& /*run*/, const edm::EventSetup& es)
/*****************************************************************/
{
...
/*****************************************************************/
void HGCalTriggerBestChoiceTester::analyze(const edm::Event& e, const edm::EventSetup& es)
/*****************************************************************/
{
...
/*****************************************************************/
void HGCalTriggerBestChoiceTester::checkSelectedCells(const edm::Event& e, const edm::EventSetup& es)
/*****************************************************************/
{
  ...
  std::map<std::tuple<uint32_t, uint32_t, uint32_t, uint32_t>, std::vector<std::pair<uint32_t, uint32_t>>>
      module_triggercells_all;
  for (auto cl_itr = be_clusters_all.begin(0); cl_itr != be_clusters_all.end(0); cl_itr++) {
    ...
    auto itr_insert =
        module_triggercells_all.emplace(std::make_tuple(zside, cluster.subdetId(), cluster.layer(), cluster.module()),
                                        std::vector<std::pair<uint32_t, uint32_t>>());
    itr_insert.first->second.emplace_back(
        cluster.hwEta(), cluster.hwPt());  // FIXME: the index within the module has been stored in hwEta
  }
  std::map<std::tuple<uint32_t, uint32_t, uint32_t, uint32_t>, std::vector<std::pair<uint32_t, uint32_t>>>
      module_triggercells_select;
  for (auto cl_itr = be_clusters_select.begin(0); cl_itr != be_clusters_select.end(0); cl_itr++) {
    const l1t::HGCalCluster& cluster = *cl_itr;
    uint32_t zside = cluster.eta() < 0. ? 0 : 1;
    auto itr_insert = module_triggercells_select.emplace(
        std::make_tuple(zside, cluster.subdetId(), cluster.layer(), cluster.module()),
        std::vector<std::pair<uint32_t, uint32_t>>());
    itr_insert.first->second.emplace_back(
        cluster.hwEta(), cluster.hwPt());  // FIXME: the index within the module has been stored in hwEta
  }
  ...
  for (const auto& module_cells : module_triggercells_all) {
    ...
    for (const auto& id_energy : module_cells.second)
      energy_all += id_energy.second;
    for (const auto& id_energy : module_cells_select_itr->second)
      energy_select += id_energy.second;
...
/*****************************************************************/
void HGCalTriggerBestChoiceTester::rerunBestChoiceFragments(const edm::Event& e, const edm::EventSetup& es)
/*****************************************************************/
{
...
/*****************************************************************/
void HGCalTriggerBestChoiceTester::fillModule(const std::vector<HGCalDataFrame>& dataframes,
                                              const std::vector<std::pair<HGCalDetId, uint32_t>>& linearized_dataframes,
                                              const HGCalBestChoiceDataPayload& fe_payload_TCsums_woBestChoice,
                                              const HGCalBestChoiceDataPayload& fe_payload_TCsums_BestChoice,
...
/*****************************************************************/
{
  ...
  for (const auto& tc : fe_payload_TCsums_BestChoice.payload) {
    if (tc > 0) {
      ...
      if (is_Simhit_comp_) {
        if (TC_simhit_energies.at(icell_BestChoice) > 0)
          triggerCellData_BestChoice_vsSimHits_->Fill(TC_simhit_energies.at(icell_BestChoice), tc);
      }
    }
    icell_BestChoice++;
  }
  ...
  for (const auto& tc : fe_payload.payload) {
    ...
    if (tc > 0) {
      ...
      if (is_Simhit_comp_) {
        if (TC_simhit_energies.at(icell) > 0)
          triggerCellData_vsSimHits_->Fill(TC_simhit_energies.at(icell), tc);
      }
    }
    icell++;
  }
