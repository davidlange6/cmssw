...
/*****************************************************************/
void HGCalTriggerGeomTester::beginRun(const edm::Run& /*run*/, const edm::EventSetup& es)
/*****************************************************************/
{
...
void HGCalTriggerGeomTester::checkMappingConsistency() {
  ...
  for (const auto& id : triggerGeometry_->eeGeometry()->getValidGeomDetIds()) {
    ...
    for (int i = 0; i < nCells; i++) {
      HGCalDetId id(
          ForwardSubdetector(waferid.subdetId()), waferid.zside(), waferid.layer(), wafer_type, waferid.wafer(), i);
  ...
  for (const auto& id : triggerGeometry_->fhGeometry()->getValidGeomDetIds()) {
    ...
    for (int i = 0; i < nCells; i++) {
      HGCalDetId id(
          ForwardSubdetector(waferid.subdetId()), waferid.zside(), waferid.layer(), wafer_type, waferid.wafer(), i);
      if (!triggerGeometry_->fhTopology().valid(id))
        continue;
      // fill trigger cells
      uint32_t trigger_cell = triggerGeometry_->getTriggerCellFromCell(id);
      auto itr_insert = triggercells_to_cells.emplace(trigger_cell, std::unordered_set<uint32_t>());
  ...
  // BH
  for (const auto& id :
       triggerGeometry_->caloGeometry()->getSubdetectorGeometry(DetId::Hcal, HcalEndcap)->getValidDetIds()) {
    if (id.rawId() == 0 || id.subdetId() != 2)
      continue;
    // fill trigger cells
    uint32_t trigger_cell = triggerGeometry_->getTriggerCellFromCell(id);
    auto itr_insert = triggercells_to_cells.emplace(trigger_cell, std::unordered_set<uint32_t>());
    ...
    for (const auto& c : triggercell_cells.second) {
      if (id.subdetId() == ForwardSubdetector::HGCHEB) {
        ...
        GlobalPoint cell_position =
            (cId.subdetId() == ForwardSubdetector::HGCEE ? triggerGeometry_->eeGeometry()->getPosition(cId)
                                                         : triggerGeometry_->fhGeometry()->getPosition(cId));
        triggerCellCell_id_.get()[ic] = c;
        triggerCellCell_zside_.get()[ic] = cId.zside();
        triggerCellCell_subdet_.get()[ic] = cId.subdetId();
...
/*****************************************************************/
void HGCalTriggerGeomTester::analyze(const edm::Event& e, const edm::EventSetup& es)
/*****************************************************************/
{}
