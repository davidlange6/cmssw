...
RPCStripsRing::RPCStripsRing(const RPCRoll* roll, std::shared_ptr<L1RPCConeBuilder::TConMap> cmap)
    : m_didVirtuals(false), m_didFiltering(false), m_connectionsMap(cmap) {
...
int RPCStripsRing::getRingId() { return getRingId(m_etaPartition, m_hwPlane); }
...
void RPCStripsRing::fillWithVirtualStrips() {
  ...
  for (; it != this->end(); ++it) {
    ...
    */
    ...
    if (isReferenceRing() && m_hwPlane == 6)
      ++stripsToAdd;
...
void RPCStripsRing::compressConnections() {
          ...
          for (L1RPCConeBuilder::TCompressedConVec::iterator itCompConn = (*m_compressedConnectionMap)[detId].begin();
                ...
                itCompConn->m_logplane == itConn->m_logplane)  // connection allready compressed
            {
              ...
              if (logStrip != itConn->m_logstrip) {
                ...
                edm::LogWarning("RPCTriggerConfig")
                    << " Compression failed for det " << detId << " strip " << (int)itStrip->first << " . Got "
                    << (int)logStrip << " expected " << (int)itConn->m_logstrip << std::endl;
              } else {
  ...
  edm::LogInfo("RPCTriggerConfig") << " Compressed: " << compressedCons << " "
                                   << sizeof(L1RPCConeBuilder::TCompressedCon)
                                   << " Uncompressed before: " << uncompressedConsBefore << " "
                                   << sizeof(L1RPCConeBuilder::TStripCon)
