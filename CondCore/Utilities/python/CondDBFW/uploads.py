...
class uploader(object):
	...
	def __init__(self, metadata_source=None, debug=False, verbose=False, testing=False, server="https://cms-conddb-dev.cern.ch/cmsDbCondUpload/", **kwargs):
		...
		if self.metadata_source.get("destinationTags") == None:
			...
			if isinstance(self.metadata_source.get("destinationTags"), dict) and self.metadata_source.get(list(six.iterkeys("destinationTags")))[0] == None:
				self.exit_upload("No destination Tag was given.")
	...
	def check_response_for_error_key(self, response_dict, exit_if_error=True):
		...
		if isinstance(response_dict, dict) and "error" in list(six.iterkeys(response_dict)):
			splitter_string = "\n%s\n" % ("-"*50)
			...
			if exit_if_error:
				if self._testing:
					...
					exit()
		elif not("error" in list(six.iterkeys(response_dict))) and "log_data" in list(six.iterkeys(response_dict)):
	...
	def get_upload_session_id(self):
		...
		body_data = base64.b64encode(json.dumps(
				{
					"destinationTag" : list(six.iterkeys(self.data_to_send["destinationTags"]))[0],
	...
	def send_metadata(self, upload_session_id):
		...
		if self.data_to_send["userText"] in ["", None]:
			self.data_to_send["userText"] = "Tag '%s' uploaded from CondDBFW client." % list(six.iterkeys(self.data_to_send["destinationTags"]))[0]
