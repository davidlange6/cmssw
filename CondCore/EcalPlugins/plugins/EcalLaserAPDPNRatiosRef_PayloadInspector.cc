  ...
  *******************************************************/
  ...
  public:
    EcalLaserAPDPNRatiosRefEBMap()
        : cond::payloadInspector::Histogram2D<EcalLaserAPDPNRatiosRef>("ECAL Barrel Laser APDPN Ratios Ref- map ",
                                                                       "iphi",
                                                                       MAX_IPHI,
                                                                       MIN_IPHI,
                                                                       MAX_IPHI + 1,
      ...
      for (auto const& iov : iovs) {
        ...
        if (payload.get()) {
          ...
          for (int cellid = EBDetId::MIN_HASH; cellid < EBDetId::kSizeForDenseIndexing; ++cellid) {
            ...
            // check the existence of ECAL LaserAPDPNRatiosRef, for a given ECAL barrel channel
  ...
  *******************************************************/
  ...
  public:
    EcalLaserAPDPNRatiosRefEEMap()
        : cond::payloadInspector::Histogram2D<EcalLaserAPDPNRatiosRef>("ECAL Endcap Laser APDPN Ratios Ref- map ",
                                                                       "ix",
                                                                       EEhistXMax,
                                                                       IX_MIN,
                                                                       EEhistXMax + 1,
  ...
  *************************************************/
  ...
  public:
    EcalLaserAPDPNRatiosRefPlot()
        : cond::payloadInspector::PlotImage<EcalLaserAPDPNRatiosRef>("Ecal Laser APDPN Ratios Ref - map ") {
      setSingleIov(true);
    }
    ...
    bool fill(const std::vector<std::tuple<cond::Time_t, cond::Hash> >& iovs) override {
      ...
      TH2F* endc_p =
          new TH2F("EE+", "Laser APDPN Ratios Ref EE+", IX_MAX, IX_MIN, IX_MAX + 1, IY_MAX, IY_MIN, IY_MAX + 1);
      TH2F* endc_m =
          new TH2F("EE-", "Laser APDPN Ratios Ref EE-", IX_MAX, IX_MIN, IX_MAX + 1, IY_MAX, IY_MIN, IY_MAX + 1);
      ...
      if (payload.get()) {
        ...
        fillEBMap_SingleIOV<EcalLaserAPDPNRatiosRef>(payload, barrel);

        if (payload->endcapItems().empty())
          return false;

        fillEEMap_SingleIOV<EcalLaserAPDPNRatiosRef>(payload, endc_m, endc_p);
      ...
      t1.SetTextAlign(26);
      t1.SetTextSize(0.04);
      t1.DrawLatex(0.5, 0.96, Form("Ecal Laser APDPN Ratios Ref, IOV %i", run));
  ...
  *****************************************************************/
  ...
  public:
    ...
    bool fill(const std::vector<std::tuple<cond::Time_t, cond::Hash> >& iovs) override {
      ...
      t1.SetTextAlign(26);
      t1.SetTextSize(0.04);
      t1.DrawLatex(0.5, 0.96, Form("Ecal Laser APDPN Ratios Ref Diff, IOV %i - %i", run[1], run[0]));
 ...
 *******************************************************/
  ...
  public:
    EcalLaserAPDPNRatiosRefSummaryPlot()
        : cond::payloadInspector::PlotImage<EcalLaserAPDPNRatiosRef>("Ecal Laser APDPN RatiosRef Summary- map ") {
      setSingleIov(true);
    }
    ...
    bool fill(const std::vector<std::tuple<cond::Time_t, cond::Hash> >& iovs) override {
      ...
      if (payload.get()) {
        ...
        fillTableWithSummary(
            align, "Ecal Laser APDPN Ratios Ref", mean_x_EB, rms_EB, num_x_EB, mean_x_EE, rms_EE, num_x_EE);
      ...
      t1.SetTextSize(0.04);
      t1.SetTextColor(2);
      t1.DrawLatex(0.5, 0.96, Form("Ecal Laser APDPN Ratios Ref Summary, IOV %i", run));
...
PAYLOAD_INSPECTOR_MODULE(EcalLaserAPDPNRatiosRef) {
