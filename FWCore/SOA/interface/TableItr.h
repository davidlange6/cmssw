  ...
  namespace soa {
    ...
    public:
      ...
      explicit TableItr(std::array<void*, sizeof...(Args)> const& iValues, unsigned int iOffset)
          : m_values{iValues} {
        impl::TableItrAdvance<sizeof...(Args) - 1, Args...>::advance(m_values,
                                                                     static_cast<long>(iOffset));
      }
    ...
    public:
      ...
      ConstTableItr(std::array<void const*, sizeof...(Args)> const& iValues, unsigned int iOffset)
          : m_values{iValues} {
        impl::ConstTableItrAdvance<sizeof...(Args) - 1, Args...>::advance(
            m_values, static_cast<long>(iOffset));
      }
