    ...
    private:
      ...
      void copyValueFromImpl(O const& iObj, std::true_type) {
        ...
        *ptr = value_for_column(iObj, static_cast<ColumnType*>(nullptr));
        copyValueFromImpl<I + 1>(
            iObj, std::conditional_t<I + 1 == sizeof...(Args), std::false_type, std::true_type>{});
      }
      template <int I, typename O>
      void copyValueFromImpl(O const& iObj, std::false_type) {}

      template <int I, typename E, typename F>
      static void copyValuesUsingFiller(F& iFiller, E const& iItem,
        ...
        *pElement = iFiller.value(iItem, static_cast<ColumnType*>(nullptr));
        copyValuesUsingFiller<I + 1>(
            iFiller, iItem, oValues,
            std::conditional_t<I + 1 == sizeof...(Args), std::false_type, std::true_type>{});
      }
      template <int I, typename E, typename F>
      static void copyValuesUsingFiller(F&, E const&, std::array<void*, sizeof...(Args)>& oValues,
                                        std::false_type) {}
    };
  }
}
