  ...
  namespace soa {
    ...
    public:
      ...
      Table(T const& iContainer, CArgs... iArgs) : m_size(iContainer.size()) {
        using CtrChoice =
            std::conditional_t<sizeof...(CArgs) == 0, CtrFillerFromAOS, CtrFillerFromContainers>;
      ...
      ColumnValues<typename U::type> column() const {
        return ColumnValues<typename U::type>{static_cast<typename U::type*>(columnAddress<U>()),
                                              m_size};
      }
      template <typename U>
      MutableColumnValues<typename U::type> column() {
        return MutableColumnValues<typename U::type>{
            static_cast<typename U::type*>(columnAddress<U>()), m_size};
      }
    ...
    private:
      ...
      static void dtr(std::array<void*, sizeof...(Args)>& iArray, std::true_type) {
        ...
        dtr<I + 1>(iArray,
                   std::conditional_t<I + 1 < sizeof...(Args), std::true_type, std::false_type>{});
      }
      ...
      struct CtrFillerFromContainers {
        ...
        static size_t fill(std::array<void*, sizeof...(Args)>& oValues, T const& iContainer,
                           U... iArgs) {
          static_assert(sizeof...(Args) == sizeof...(U) + 1,
      ...
      private:
        ...
        static void ctrFiller(std::array<void*, sizeof...(Args)>& oValues, size_t iSize,
                              T const& iContainer, U... iU) {
      ...
      struct CtrFillerFromAOS {
        ...
        static size_t fillUsingFiller(F& iFiller, std::array<void*, sizeof...(Args)>& oValues,
                                      T const& iContainer) {
          presize<0>(oValues, iContainer.size(), std::true_type{});
          unsigned index = 0;
          for (auto&& item : iContainer) {
      ...
      private:
        ...
        static void fillElement(E const& iItem, size_t iIndex,
          ...
          *pElement = value_for_column(iItem, static_cast<ColumnType*>(nullptr));
          fillElement<I + 1>(
              iItem, iIndex, oValues,
              std::conditional_t<I + 1 == sizeof...(Args), std::false_type, std::true_type>{});
        }
        template <int I, typename E>
        static void fillElement(E const& iItem, size_t iIndex,
                                std::array<void*, sizeof...(Args)>& oValues, std::false_type) {}
        ...
        template <int I, typename E, typename F>
        static void fillElementUsingFiller(F&, E const&, size_t,
                                           std::array<void*, sizeof...(Args)>& oValues,
                                           std::false_type) {}
      };
      ...
      static void copyFromToWithResize(size_t iNElements,
        ...
        oTo[I] = ptr;
        std::copy(static_cast<Type const*>(iFrom[I]),
                  static_cast<Type const*>(iFrom[I]) + iNElements, ptr);
        delete[] oldPtr;
        copyFromToWithResize<I + 1>(
            iNElements, iFrom, oTo,
            std::conditional_t<I + 1 == sizeof...(Args), std::false_type, std::true_type>{});
      }
      template <int I>
      ...
      template <int I>
      static void resizeFromTo(size_t iOldSize, size_t iNewSize,
                               std::array<void*, sizeof...(Args)>& ioArray, std::true_type) {
        using Layout = std::tuple<Args...>;
        using Type = typename std::tuple_element<I, Layout>::type::type;
        Type* oldPtr = static_cast<Type*>(ioArray[I]);
        auto ptr = new Type[iNewSize];
        auto nToCopy = std::min(iOldSize, iNewSize);
        std::copy(static_cast<Type const*>(ioArray[I]),
                  static_cast<Type const*>(ioArray[I]) + nToCopy, ptr);
        resizeFromTo<I + 1>(
            iOldSize, iNewSize, ioArray,
            std::conditional_t<I + 1 == sizeof...(Args), std::false_type, std::true_type>{});
      ...
      template <int I>
      static void resizeFromTo(size_t, size_t, std::array<void*, sizeof...(Args)>&,
                               std::false_type) {}

      template <int I>
      static void resetStartingAt(size_t iStartIndex, size_t iEndIndex,
                                  std::array<void*, sizeof...(Args)>& ioArray, std::true_type) {
        using Layout = std::tuple<Args...>;
        using Type = typename std::tuple_element<I, Layout>::type::type;
        ...
        resetStartingAt<I + 1>(
            iStartIndex, iEndIndex, ioArray,
            std::conditional_t<I + 1 == sizeof...(Args), std::false_type, std::true_type>{});
      }

      template <int I>
      static void resetStartingAt(size_t, size_t, std::array<void*, sizeof...(Args)>&,
                                  std::false_type) {}
    };
    ...
    namespace impl {
      ...
      struct RemoveColumnCheck<LHS, E, std::tuple<T, U...>> {
        using type = typename std::conditional<
            ...
            typename RemoveColumnCheck<typename AddColumns<LHS, std::tuple<T>>::type, E,
                                       std::tuple<U...>>::type>::type;
      };
