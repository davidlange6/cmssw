...
/**\class ColumnFillers ColumnFillers.h "ColumnFillers.h"
 ...
 first argument is a const reference to the class in question, or its base class. The second
 argument
...
namespace edm {
  namespace soa {
    ...
    class ColumnFillers {
      ...
      auto callFiller(ELEMENT&& iEl, RET*, std::true_type)
      ...
      typename COLUMN::type tryValue(ELEMENT&& iEl, COLUMN*, std::true_type, std::true_type) {
        ...
        if (std::is_same<COL, COLUMN>::value) {
          return callFiller<I>(iEl, static_cast<typename COLUMN::type*>(nullptr),
                               std::conditional_t<std::is_same<COL, COLUMN>::value, std::true_type,
                                                  std::false_type>{});
        }
        return tryValue<I + 1>(
            iEl, static_cast<COLUMN*>(nullptr),
            std::conditional_t<I + 1 == sizeof...(Args), std::false_type, std::true_type>{},
            std::conditional_t<std::is_same<COL, COLUMN>::value, std::false_type,
                               std::true_type>{});
      }
