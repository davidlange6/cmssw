...
/**\class SoATuple SoATuple.h "FWCore/Utilities/interface/SoATuple.h"
 ...
 the structure into a container (e.g. std::vector<Foo>). This is referred to as an 'Array of
Structures'.
 However, this grouping can be inefficient if not all of the data about one item are used at the
same
 time. So to access one data for one item will cause the CPU to retrieve nearby memory which is then
 not used. If the code is looking at the same data for multiple items this will lead to many data
cache
 misses in the CPU.

 A different grouping is to place the first data element for all items into one array, the second
data
 element for all items into a second array, then so on. This is referred to as a 'Structure of
Arrays'.

 This class will take an arbitrary number of template arguments and it will group data of that
argument
...
To help keep track of the purpose of the template arguments, we suggest using an enum to denote each
one:
\code
    enum {kPx,kPy,kPz};
...
A non-default alignment for a stored type can be specified by using the edm::Aligned<T,I> where I is
an
 unsigned int value denoting the requested byte alignment. There is also a specialized version,
edm::AlignedVec
...
Explicitly aligned types and defaultly aligned types can be freely mixed in any order within the
template arguments.
  ...
  public:
    ...
    get(unsigned int iIndex) const {
      typedef typename soahelper::AlignmentHelper<
          typename std::tuple_element<I, std::tuple<Args...>>::type>::Type ReturnType;
    ...
    typename soahelper::AlignmentHelper<
        typename std::tuple_element<I, std::tuple<Args...>>::type>::Type const*
    begin() const {
      typedef soahelper::AlignmentHelper<typename std::tuple_element<I, std::tuple<Args...>>::type>
          Helper;
    ...
    /** Adds one entry to the end of the list. The arguments are used to instantiate each data
     * element in the order defined in the template arguments.*/
  ...
  private:
    void changeSize(unsigned int iToSize) {
      ...
      const size_t memoryNeededInBytes =
          soahelper::SoATupleHelper<sizeof...(Args), Args...>::spaceNeededFor(iToSize);
      // align memory of the array to be on the strictest alignment boundary for any type in the
      // Tuple
      // This is done by creating an array of a type that has that same alignment restriction and
      // minimum size.
      // This has the draw back of possibly padding the array by one extra element if the
      // memoryNeededInBytes is not
      // a strict multiple of max_alignment.
      // NOTE: using new char[...] would likely cause more padding based on C++11 5.3.4 paragraph
      // 10 where it
      // says the alignment will be for the strictest requirement for an object whose size < size
      // of array. So
      // if the array were for 64 bytes and the strictest requirement of any object was 8 bytes
      // then the entire
      // char array would be aligned on an 8 byte boundary. However, if the SoATuple<char,char>
      ...
      soahelper::SoATupleHelper<sizeof...(Args), Args...>::moveToNew(newMemory, m_size, iToSize,
                                                                     m_values);
      m_reserved = iToSize;
