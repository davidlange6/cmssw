...
namespace edm {
  ...
  public:
    ...
    void push_back(const T& value) {
      if (size_ >= N) throw std::length_error("push_back on already-full VecArray (N=" + std::to_string(N) + ")");
    ...
    void emplace_back(Args&&... args) {
      if (size_ >= N) throw std::length_error("emplace_back on already-full VecArray (N=" + std::to_string(N) + ")");
