...
/**\class typelookup typelookup.h FWCore/Utilities/interface/typelookup.h
 ...
 TYPELOOKUP_METHODS(Tname): explicitly instantiates the template functions className classTypeInfo
 using
 ...
 Example: You have a new data class called 'DummyData'.  Then to register that class with the system
 you
 place the lines
 ...
 The actual name of the file that uses the 'TYPELOOKUP_DATA_REG' macro is not important.  The only
 important point
 the file that uses the 'TYPELOOKUP_DATA_REG' macro must be in the same library as the data class it
 is registering.
