...
namespace edm {
  namespace soahelper {
    ...
    /**Given a leading memory size, iSizeSoFar, and an alignment boundary requirement of
     iBoundary, returns how much additional memory padding is needed.
    ...
    /**
     ...
     will recursively step through the arguments, Args, until it hits the one you want, Jth, and
     then will return it as return type Ret.
     */

    template <unsigned int I, unsigned int J, typename Ret, typename... Args>
    struct arg_puller;

    /**Given a variable number of arguments, returns the 'J'th one. The first caller must use
     * I=0.*/
    ...
    /**Implements most of the internal functions used by SoATuple. The argument I is used to
     recursively step
     through each arugment Args when doing the work. SoATupleHelper<I,Args> actually operates on
     the I-1 argument.
     There is a specialization of SoATulpeHelper with I=0 which is used to stop the template
     recursion.
     */
    template <unsigned int I, typename... Args>
    struct SoATupleHelper {
      typedef AlignmentHelper<typename std::tuple_element<I - 1, std::tuple<Args...>>::type>
          AlignmentInfo;
    ...
    template <unsigned int I, typename... Args>
    size_t SoATupleHelper<I, Args...>::moveToNew(char* iNewMemory, size_t iSize, size_t iReserve,
                                                 void** oToSet) {
      ...
      Type* newStart =
          reinterpret_cast<Type*>(iNewMemory + usedSoFar + padding_needed(usedSoFar, boundary));
    ...
    template <unsigned int I, typename... Args>
    size_t SoATupleHelper<I, Args...>::copyToNew(char* iNewMemory, size_t iSize, size_t iReserve,
                                                 void* const* iFrom, void** oToSet) {
