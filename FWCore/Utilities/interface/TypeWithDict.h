...
namespace edm {
  ...
  public:
    ...
    // FunctionWithDict functionMemberByName(std::string const& name, TypeWithDict const&
    // signature, int mods, TypeMemberQuery memberQuery) const;
    // Note: Used only by CondFormats/PhysicsToolsObjects/src/MVAComputer.cc
    FunctionWithDict functionMemberByName(std::string const& name, std::string const& proto,
                                          bool isConst) const;
