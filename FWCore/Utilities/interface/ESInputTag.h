...
/**\class ESInputTag ESInputTag.h FWCore/Utilities/interface/ESInputTag.h
 ...
 Usage:
    The ESInputTag can be used in conjunction with an EventSetup Record to retrieve a particular
 data
 ...
 In addition, every piece of data in the EventSetup comes from either an ESSource or an ESProducer.
 Every
 ESSource and ESProducer is assigned a label (referred to here as the moduleLabel) in the
 configuration of
 the job.  This label may be explicitly set or it may just be the C++ class type of the
 ESSource/ESProducer.
 For example, say there is an ESProducer of C++ class type FooESProd. If the python configuration
 has
 ...
 The ESInputTag allows one to specify both the dataLabel and moduleLabel.  The dataLabel is used to
 find the data
 being requested.  The moduleLabel is only used to determine if the data that was found comes from
 the specified
 module.  If the data does not come from the module then an error has occurred.  If the moduleLabel
 is set to the
