...
namespace edm {
  ...
  InputTag::InputTag(std::string const& label, std::string const& instance,
  ...
  ProductResolverIndex InputTag::indexFor(TypeID const& typeID, BranchType branchType,
    ...
    if (index < ProductResolverIndexInitializing && typeID_ == typeID &&
        branchType_ == branchType && productRegistry_ == productRegistry) {
      return index;
    }
    return ProductResolverIndexInvalid;
  }

  void InputTag::tryToCacheIndex(ProductResolverIndex index, TypeID const& typeID,
                                 BranchType branchType, void const* productRegistry) const {
    unsigned int invalidValue = static_cast<unsigned int>(ProductResolverIndexInvalid);
    if (index_.compare_exchange_strong(
            invalidValue, static_cast<unsigned int>(ProductResolverIndexInitializing))) {
      typeID_ = typeID;
