...
namespace {
  ...
  //! A template to select either 32-bit or 64-bit constant as compile time, depending on machine
  //! word size.
  template <unsigned u, unsigned long long ull>
  struct select_size_t_constant {
    // Explicit cast is needed to avoid compiler warnings about possible truncation.
    // The value of the right size,   which is selected by ?:, is anyway not truncated or
    // promoted.
  ...
  struct StringHash {
    inline size_t operator()(const char* s) const {
      size_t h = 0;
      for (const char* c = s; *c; ++c) h = static_cast<size_t>(*c) ^ (h * hash_multiplier);
      return h;
    }
  };
  ...
  using TypeNameToValueMap =
      tbb::concurrent_unordered_map<const char*, const std::type_info*, StringHash, StringEqual>;
