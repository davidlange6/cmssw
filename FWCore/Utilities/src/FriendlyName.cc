...
// NOTE:  This should probably be rewritten so that we break the class name into a tree where the
// template arguments are the node.  On the way down the tree
// we look for '<' or ',' and on the way up (caused by finding a '>') we can apply the
// transformation to the output string based on the class name for the
// templated class.  Up front we'd register a class name to a transformation function (which would
// probably take a std::vector<std::string> which holds
...
namespace edm {
  namespace friendlyname {
    ...
    std::string removeExtraSpaces(std::string const& iIn) {
      return std::regex_replace(std::regex_replace(iIn, reBeginSpace, emptyString), reEndSpace,
                                emptyString);
    }
    ...
    static std::regex const reSorted(
    ...
    static std::regex const reAIKR(
        ", *edm::helper::AssociationIdenticalKeyReference");  // this is a default so can replaced
                                                              // with empty
    // force first argument to also be the argument to edm::ClonePolicy so that if OwnVector is
    // within
    // a template it will not eat all the remaining '>'s
    static std::regex const reOwnVector("edm::OwnVector<(.*), *edm::ClonePolicy<\\1 *> >");

    // NOTE: the '?' means make the smallest match. This may lead to problems where the template
    // arguments themselves have commas
    // but we are using it in the cases where edm::AssociationMap appears multiple times in
    // template arguments
    static std::regex const reOneToOne(
        "edm::AssociationMap< *edm::OneToOne<(.*?),(.*?), *u[a-z]*> >");
    static std::regex const reOneToMany(
        "edm::AssociationMap< *edm::OneToMany<(.*?),(.*?), *u[a-z]*> >");
    static std::regex const reOneToValue(
    ...
    static std::regex const reToRangeMap(
        "edm::RangeMap< *(.*), *(.*), *edm::ClonePolicy<([^>]*)> >");
    // NOTE: If container is a template with one argument which is its 'type' then can simplify
    // name
    static std::regex const reToRefs1(
        "edm::RefVector< *(.*)< *(.*) *>, *\\2 *, *edm::refhelper::FindUsingAdvance< *\\1< *\\2 "
        "*> *, *\\2 *> *>");
    ...
    std::string standardRenames(std::string const& iIn) {
      ...
      name =
          regex_replace(name, reOneToManyWithQuality, "Association<$1,ToMany,$2,WithQuantity,$3>");
      ...
      if (s_fillToFriendlyName.end() == itFound) {
        itFound = s_fillToFriendlyName
                      .insert(Map::value_type(
                          iFullName, handleNamespaces(subFriendlyName(standardRenames(iFullName)))))
                      .first;
      }
