...
namespace edm {
  ...
  FunctionWithDict TypeWithDict::functionMemberByName(std::string const& functionName, std::string const& proto,
                                                      bool isConst) const {
    if (!isClass()) {
      return FunctionWithDict();
    }
    ...
    TMethod* meth = class_->GetMethodWithPrototype(functionName.c_str(), proto.c_str(), /*objectIsConst=*/isConst,
