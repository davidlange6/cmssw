...
namespace edm {
  ...
  TypeWithDict TypeWithDict::byName(std::string const& name, long property) {
    ...
    // For a reason not understood, TClass::GetClass sometimes cannot find
  ...
  bool TypeWithDict::isFundamental() const {
  ...
  bool TypeWithDict::isVirtual() const {
  ...
  void const* TypeWithDict::pointerToBaseType(void const* ptr,
  ...
  void const* TypeWithDict::pointerToContainedType(void const* ptr,
                                                   TypeWithDict const& derivedType) const {
    if (!isClass()) {
      return ptr;
    }
  ...
  TypeWithDict TypeWithDict::nestedType(char const* nestedName) const {
  ...
  FunctionWithDict TypeWithDict::functionMemberByName(std::string const& member) const {
    ...
    TMethod* meth =
        reinterpret_cast<TMethod*>(class_->GetListOfMethods()->FindObject(member.c_str()));
    if (meth == nullptr) {
      return FunctionWithDict();
    }
  ...
  FunctionWithDict TypeWithDict::functionMemberByName(std::string const& functionName,
    ...
    TMethod* meth = class_->GetMethodWithPrototype(functionName.c_str(), proto.c_str(),
                                                   /*objectIsConst=*/isConst,
                                                   /*mode=*/ROOT::kConversionMatch);
    if (meth == nullptr) {
      return FunctionWithDict();
    }
