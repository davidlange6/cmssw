...
namespace edm {
  namespace messagedrop {
    ...
    public:
      ...
      void set(std::string const& name, std::string const& label, unsigned int moduleID,
               const char* phase) {
        name_ = &name;
        label_ = &label;
        moduleID_ = moduleID;
  ...
  void MessageDrop::setModuleWithPhase(std::string const& name, std::string const& label,
