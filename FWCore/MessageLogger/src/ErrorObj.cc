...
namespace edm {
  ...
  ELstring ErrorObj::fullText() const {
    ELstring result;
    for (ELlist_string::const_iterator it = myItems.begin(); it != myItems.end(); ++it) result += *it;
    return result;
  ...
  void ErrorObj::setID(const ELstring& id) {
    ...
    if (id.length() > maxIDlength) myIdOverflow = ELstring(id, maxIDlength, id.length() - maxIDlength);
  }
