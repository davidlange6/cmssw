...
namespace edm {
  ...
  JobReport::InputFile& JobReport::JobReportImpl::getInputFileForToken(InputType inputType,
                                                                       JobReport::Token t) {
    ...
    if (inputFile->fileHasBeenClosed) {
      throw edm::Exception(edm::errors::LogicError) << "Access reported for input file with token "
                                                    << t << " after this file has been closed.\n"
                                                    << "File record follows:\n"
                                                    << *inputFile << '\n';
    }
    return *inputFile;
  ...
  JobReport::OutputFile& JobReport::JobReportImpl::getOutputFileForToken(JobReport::Token t) {
    ...
    if (outputFiles_[t].fileHasBeenClosed) {
      throw edm::Exception(edm::errors::LogicError) << "Access reported for output file with token "
                                                    << t << " after this file has been closed.\n"
                                                    << "File record follows:\n"
                                                    << outputFiles_[t] << '\n';
    }
  ...
  void JobReport::JobReportImpl::associateInputLumiSection(unsigned int runNumber,
      ...
      if (!inputFile.fileHasBeenClosed) {
        ...
        if (iter == theMap.end() || runNumber < iter->first) {  // not found
          theMap.emplace_hint(
              ...
              JobReport::RunReport{
                  runNumber,
                  {{lumiSect, std::numeric_limits<unsigned long>::max()}}});  // insert it
        } else {
  ...
  JobReport::Token JobReport::outputFileOpened(
      ...
      std::string const& moduleLabel, std::string const& guid, std::string const& dataType,
      std::string const& branchHash, std::vector<std::string> const& branchNames) {
    ...
    for (tbb::concurrent_vector<Token>::size_type i = 0, iEnd = impl_->inputFilesSecSource_.size();
         i < iEnd; ++i) {
  ...
  void JobReport::eventWrittenToFile(JobReport::Token fileToken, RunNumber_t /*run*/,
                                     EventNumber_t) {
    JobReport::OutputFile& f = impl_->getOutputFileForToken(fileToken);
    ++f.numEventsWritten;
  }
  ...
  void JobReport::reportFastCopyingStatus(JobReport::Token fileToken,
                                          std::string const& inputFileName, bool fastCopying) {
    JobReport::OutputFile& f = impl_->getOutputFileForToken(fileToken);
  ...
  void JobReport::reportLumiSection(JobReport::Token token, unsigned int run,
                                    unsigned int lumiSectId, unsigned long nEvents) {
  ...
  void JobReport::reportError(std::string const& shortDesc, std::string const& longDesc,
                              int const& exitCode) {
    if (impl_->ost_) {
      {
        std::lock_guard<std::mutex> lock(write_mutex);
  ...
  void JobReport::reportFallbackAttempt(std::string const& pfn, std::string const& lfn,
  ...
  void JobReport::reportMemoryInfo(std::vector<std::string> const& memoryData) {
    if (impl_->ost_) {
      ...
      for (const_iterator pos = memoryData.begin(), posEnd = memoryData.end(); pos != posEnd;
           ++pos) {
        msg << *pos << "\n";
      }
      msg << "</MemoryService>\n";
  ...
  void JobReport::reportMessageInfo(std::map<std::string, double> const& messageData) {
    if (impl_->ost_) {
      ...
      for (const_iterator pos = messageData.begin(), posEnd = messageData.end(); pos != posEnd;
           ++pos) {
        msg << "  <" << pos->first << "  Value=\"" << pos->second << "\" />"
            << "\n";
      }
  ...
  void JobReport::reportPerformanceSummary(std::string const& metricClass,
    ...
    if (impl_->ost_) {
      ...
      typedef std::map<std::string, std::string>::const_iterator const_iterator;
      for (const_iterator iter = metrics.begin(), iterEnd = metrics.end(); iter != iterEnd;
           ++iter) {
