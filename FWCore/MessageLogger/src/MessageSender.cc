...
MessageSender::MessageSender(ELseverityLevel const& sev, ELstring const& id, bool verbatim,
                             bool suppressed)
  ...
  try {
    ...
    if (errorSummaryIsBeingKept.load(std::memory_order_acquire) &&
      ...
      if (i != errorSummaryMap.end()) {
        i->second.value().fetch_add(1,
                                    std::memory_order_acq_rel);  // same as ++errorSummaryMap[key]
      } else {
