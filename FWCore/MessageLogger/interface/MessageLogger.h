...
namespace edm {
  ...
  public:
    explicit LogWarning(std::string const& id)
        : ap(ELwarning, id, false, (MessageDrop::warningAlwaysSuppressed ||
  ...
  public:
    explicit LogInfo(std::string const& id)
        : ap(ELinfo, id, false, (MessageDrop::infoAlwaysSuppressed ||
  ...
  public:
    explicit LogVerbatim(std::string const& id)
        : ap(ELinfo, id, true, (MessageDrop::infoAlwaysSuppressed ||
                                !MessageDrop::instance()->infoEnabled))  // Change log 21
    {}
  ...
  public:
    explicit LogPrint(std::string const& id)
        : ap(ELwarning, id, true, (MessageDrop::warningAlwaysSuppressed ||
                                   !MessageDrop::instance()->warningEnabled))  // Change log 21
    {}
...
// The preprocessor symbol controlling suppression of LogDebug is EDM_ML_DEBUG.  Thus by default
// (BEHAVIOR CHANGE) LogDebug is
