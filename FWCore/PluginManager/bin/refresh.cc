...
int main(int argc, char** argv) try {
  ...
  desc.add_options()(kHelpCommandOpt, "produce help message")(
      ...
      //(kAllCommandOpt, "when no paths given, try to update caches for all known directories
      //[default is to only scan the first directory]")
      ;
  ...
  /*if(argc == 1) {
    std::cerr << "Requires at least one argument.  Please pass either one directory or a list of
  files (all in the same directory)." << std::endl;
    return 1;
  } */
  ...
  try {
    ...
    if (boost::filesystem::is_directory(directory)) {
      ...
      for (std::vector<std::string>::iterator it = requestedPaths.begin(),
        ...
        if (is_directory(f)) {
          std::cerr << "either one directory or a list of files are allowed as arguments"
                    << std::endl;
          return 1;
        }
        ...
        while (ci && fi != fe) {
          ...
          try {
            try {
              ...
              // PluginCapabilities is special, the plugins do not call it.  Instead, for
              // each shared library load
              ...
              if (iException.category() == "PluginLibraryLoadError") {
                std::cerr << "Caught exception " << iException.what() << " will ignore "
                          << files[fi] << " and continue." << std::endl;
              } else {
                throw;
              }
            }
    ...
    for (CacheParser::LoadableToPlugins::iterator itFile = ltp.begin(); itFile != ltp.end();
         ++itFile) {
