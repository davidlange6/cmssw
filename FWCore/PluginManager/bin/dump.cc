...
int main(int argc, char** argv) {
  ...
  desc.add_options()(kHelpCommandOpt, "produce help message")(
      kFilesCommandOpt, "list the file from which a plugin will come")(
  ...
  try {
    ...
    for (CatToInfos::const_iterator it = catToInfos.begin(), itEnd = catToInfos.end(); it != itEnd;
         ++it) {
      std::cout << "Category " << it->first << ":" << std::endl;
      std::string prevPluginName;
      for (edmplugin::PluginManager::Infos::const_iterator itInfo = it->second.begin(),
                                                           itInfoEnd = it->second.end();
