...
namespace edmplugin {
  ...
  public:
    ...
    /**The container is ordered by category, then plugin name and then by precidence order of the
      plugin files.
    ...
    // If can not find iPlugin in category iCategory return null pointer, any other failure will
    // cause a throw
    ...
    /// if the value returned from loadingFile matches this string then the file is statically
    /// linked
  ...
  private:
    ...
    tbb::concurrent_unordered_map<boost::filesystem::path, std::shared_ptr<SharedLibrary>,
