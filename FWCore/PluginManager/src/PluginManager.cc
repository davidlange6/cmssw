...
namespace edmplugin {
  ...
  PluginManager::PluginManager(const PluginManager::Config& iConfig)
    ...
    // read in the files
    // Since we are looping in the 'precidence' order then the lists in categoryToInfos_ will also
    // be
    // in that order
  ...
  const boost::filesystem::path& PluginManager::loadableFor_(const std::string& iCategory,
    ...
    if (itFound == categoryToInfos_.end()) {
      if (throwIfFail) {
        throw cms::Exception("PluginNotFound")
            << "Unable to find plugin '" << iPlugin << "' because the category '" << iCategory
    ...
    std::pair<PIItr, PIItr> range =
        std::equal_range(itFound->second.begin(), itFound->second.end(), i, PICompare());

    if (range.first == range.second) {
      if (throwIfFail) {
        throw cms::Exception("PluginNotFound")
            << "Unable to find plugin '" << iPlugin << "' in category '" << iCategory
      ...
      if (range.first->loadable_.branch_path() == (range.first + 1)->loadable_.branch_path()) {
        ...
        throw cms::Exception("MultiplePlugins")
            ...
            << "'.\n"
               ...
               "You will need to remove the macro which registers the plugin so it only "
               "appears in"
               " one of these files.\n"
               "  If none of these files register such a plugin, "
               "then the problem originates in a library to which all these files link.\n"
               "The plugin registration must be removed from that library since plugins are "
               "not allowed in regular libraries.";
      }
    }
  ...
  namespace {
    class Sentry {
    public:
      Sentry(std::string& iPath, const std::string& iNewPath) : path_(iPath), oldPath_(iPath) {
        path_ = iNewPath;
      }
