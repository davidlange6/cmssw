...
namespace edmplugin {
    ...
    if (itFound == m_plugins.end()) {
      ...
      if (itFound == m_plugins.end()) {
        throw cms::Exception("PluginCacheError")
            ...
            << "'\n but was not there.  This means the plugin cache is incorrect.  Please run "
               "'EdmPluginRefresh "
            << lib << "'";
      }
    } else {
      // The item in the container can still be under construction so wait until the m_ptr has
      // been set since that is done last
  ...
  void* PluginFactoryBase::tryToFindPMaker(const std::string& iName) const {
    ...
    if (itFound == m_plugins.end()) {
      ...
      if (0 != slib) {
        ...
        if (itFound == m_plugins.end()) {
          throw cms::Exception("PluginCacheError")
              << "The plugin '" << iName << "' should have been in loadable\n '" << lib
              << "'\n but was not there.  This means the plugin cache is incorrect.  Please "
    ...
    return itFound != m_plugins.end()
               ? itFound->second.front().m_ptr.load(std::memory_order_acquire)
               : nullptr;
  }

  void PluginFactoryBase::fillInfo(const PMakers& makers, PluginInfo& iInfo,
