...
namespace edmplugin {
  ...
  static void checkForError(const std::istream& iIn, unsigned long iRecordNumber,
    ...
    if (iIn.eof()) {
      throw cms::Exception("PluginCacheParseFailed")
          << "Unexpectedly reached end of file for line " << iRecordNumber << " just after '"
          << iContext << "'";
    }
    if (iIn.bad()) {
      throw cms::Exception("PluginCacheParseFailed")
  ...
  bool CacheParser::readline(std::istream& iIn, const boost::filesystem::path& iDirectory,
  ...
  void CacheParser::read(std::istream& iIn, const boost::filesystem::path& iDirectory,
    ...
    for (CacheParser::CategoryToInfos::iterator it = iOut.begin(), itEnd = iOut.end(); it != itEnd;
         ++it) {
