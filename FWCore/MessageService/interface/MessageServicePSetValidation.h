...
//
/**\class MessageLogger MessageServicePSetValidation.h
 FWCore/MessageService/interface/MessageServicePSetValidation.h
...
namespace edm {
  namespace service {
    ...
    private:
      ...
      void noDuplicates(vString const& v1, vString const& v2, std::string const& psetName,
      ...
      void noNonPSetUsage(ParameterSet const& pset, vString const& v, std::string const& psetName,
                          std::string const& parameterLabel);
      void noBadParams(vString const& v, vString const& params, std::string const& psetName,
      ...
      void categoryPSet(ParameterSet const& pset, std::string const& OuterPsetName,
                        std::string const& categoryName);
      void catInts(ParameterSet const& pset, std::string const& psetName,
                   std::string const& categoryName);
      void catNoPSets(ParameterSet const& pset, std::string const& psetName,
                      std::string const& categoryName);
      void catBoolRestriction(ParameterSet const& pset, std::string const& psetName,
                              std::string const& categoryName, std::string const& type);

      template <typename T>
      T check(ParameterSet const& pset, std::string const& psetName,
              std::string const& parameterLabel) {
        T val = T();
        try {
      ...
      template <typename T>
      void noneExcept(ParameterSet const& pset, std::string const& psetName,
      ...
      template <typename T>
      void noneExcept(ParameterSet const& pset, std::string const& psetName,
                      std::string const& type, std::string const& ok) {
        vString x = pset.template getParameterNamesForType<T>(false);
        vString::const_iterator end = x.end();
        for (vString::const_iterator i = x.begin(); i != end; ++i) {
          std::string val = (*i);
      ...
      template <typename T>
      void noneExcept(ParameterSet const& pset, std::string const& psetName,
                      std::string const& type, T const& ok1, T const& ok2) {
        vString x = pset.template getParameterNamesForType<T>(false);
        vString::const_iterator end = x.end();
        for (vString::const_iterator i = x.begin(); i != end; ++i) {
          std::string val = (*i);
      ...
      template <typename T>
      void noneExcept(ParameterSet const& pset, std::string const& psetName,
                      std::string const& type, vString const& vok) {
        vString x = pset.template getParameterNamesForType<T>(false);
        vString::const_iterator end = x.end();
      ...
      template <typename T>
      void catNone(ParameterSet const& pset, std::string const& psetName,
                   std::string const& categoryName, std::string const& type) {
        vString x = pset.template getParameterNamesForType<T>(false);
        vString::const_iterator end = x.end();
        for (vString::const_iterator i = x.begin(); i != end; ++i) {
