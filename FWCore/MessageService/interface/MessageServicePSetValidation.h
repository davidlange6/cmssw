...
namespace edm {
  namespace service {
    ...
    private:
      ...
      void noDuplicates(vString const& v1, vString const& v2, std::string const& psetName, std::string const& p1,
                        std::string const& p2);
      ...
      void catBoolRestriction(ParameterSet const& pset, std::string const& psetName, std::string const& categoryName,
      ...
      template <typename T>
      void noneExcept(ParameterSet const& pset, std::string const& psetName, std::string const& type, T const& ok1,
                      T const& ok2) {
        vString x = pset.template getParameterNamesForType<T>(false);
        vString::const_iterator end = x.end();
        for (vString::const_iterator i = x.begin(); i != end; ++i) {
      ...
      template <typename T>
      void catNone(ParameterSet const& pset, std::string const& psetName, std::string const& categoryName,
                   std::string const& type) {
        vString x = pset.template getParameterNamesForType<T>(false);
        vString::const_iterator end = x.end();
        for (vString::const_iterator i = x.begin(); i != end; ++i) {
