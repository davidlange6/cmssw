...
namespace edm {
  namespace service {
    ...
    void MessageLoggerScribe::runCommand(  // changeLog 32
      ...
      switch (opcode) {  // interpret the work item
        ...
        case MessageLoggerQ::CONFIGURE: {  // changelog 17
          if (singleThread) {
            job_pset_p = std::shared_ptr<PSet>(
                static_cast<PSet*>(operand));  // propagate_const<T> has no reset() function
            configure_errorlog();
            break;
          } else {
            ConfigurationHandshake* h_p = static_cast<ConfigurationHandshake*>(operand);
            job_pset_p = std::shared_ptr<PSet>(
                static_cast<PSet*>(h_p->p));         // propagate_const<T> has no reset() function
    ...
    void MessageLoggerScribe::configure_dest(std::shared_ptr<ELdestination> dest_ctrl,
      ...
      for (vString::const_iterator sev_it = severities.begin(); sev_it != severities.end();
           ++sev_it) {
        String sevID = *sev_it;
