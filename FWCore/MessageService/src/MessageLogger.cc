...
namespace {
  char const* const s_globalTransitionNames[] = {"@beginJob", "@beginRun", "@beginLumi",
                                                 "@endLumi",  "@endRun",   "@endJob",
                                                 "@writeRun", "@writeLumi"};
...
namespace edm {
  ...
  namespace service {
          ...
          nonModule_errorEnabled(true)  // change log 20
    {
      ...
      for (vString::const_iterator it = suppressWarning.begin(); it != suppressWarning.end();
           ++it) {
                                        ...
                                        const char* whichPhase)  // ChangeLog 13, 17
    {
      ...
      if (it != suppression_levels_.end()) {
        messageDrop->debugEnabled =
            messageDrop->debugEnabled && (it->second < ELseverityLevel::ELsev_success);
                                        ...
                                        const char* whichPhase)  // ChangeLog 13, 17
    {
      ...
      messageDrop->setModuleWithPhase(desc->moduleName(), desc->moduleLabel(), desc->id(),
                                      whichPhase);
    ...
    void MessageLogger::postModuleConstruction(
        const ModuleDescription& iDescription) {  // it is now guaranteed that this will be called
                                                  // even if the module throws
    ...
    void MessageLogger::preModuleGlobalBeginRun(GlobalContext const& context,
                                                ModuleCallingContext const& mod) {
      establishModule(
          runInfoBegin_ + context.runIndex().value(), mod,
