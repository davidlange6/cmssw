...
namespace edm {
  namespace service {
    ...
    ELstatistics::~ELstatistics() {
      ...
      if (updatedStats && printAtTermination) summary(termStream, "Termination Summary");
    ...
    ELstring ELstatistics::formSummary(ELmap_stats& stats) {
      ...
      for (ELmap_stats::const_iterator i = stats.begin(); i != stats.end(); ++i) {
        ...
        s << right << std::setw(5) << ++n << ' ' << left << std::setw(20)
          << (*i).first.id.substr(0, 20) << ' ' << left << std::setw(2)
          << (*i).first.severity.getSymbol() << ' ' << left << std::setw(16)
          << (*i).first.module.substr(0, 16) << ' ' << left << std::setw(16)
          << (*i).first.subroutine.substr(0, 16) << right << std::setw(7) << (*i).second.n << left
          << std::setw(1) << ((*i).second.ignoredFlag ? '*' : ' ') << right << std::setw(8)
          << (*i).second.aggregateN << '\n';
      ...
      for (std::set<std::string>::iterator g = groupedCategories.begin(); g != gcEnd; ++g) {
        ...
        if (groupTotal > 0) {
          // -----  Emit detailed message information:
          //
          s << right << std::setw(5) << ++n << ' ' << left << std::setw(20) << (*g).substr(0, 20)
            << ' ' << left << std::setw(2) << severityLevel.getSymbol() << ' ' << left
            << std::setw(16) << "  <Any Module>  " << ' ' << left << std::setw(16)
      ...
      for (ELmap_stats::const_iterator i = stats.begin(); i != stats.end(); ++i) {
        ...
        s << right << std::setw(5) << ++n << ' ' << left << std::setw(20) << (*i).first.id.c_str()
          << ' ' << left << std::setw(16) << (*i).second.context1.c_str() << ' ' << left
          << std::setw(16) << (*i).second.context2.c_str() << ' ' << (*i).second.contextLast.c_str()
          << '\n';
      }  // for
      ...
      for (int k = 0; k < ELseverityLevel::nLevels; ++k) {
        if (p3[k].n != 0 || p3[k].t != 0) {
          s << left << std::setw(8) << ELseverityLevel(ELseverityLevel::ELsev_(k)).getName().c_str()
            << right << std::setw(17) << p3[k].n << right << std::setw(20) << p3[k].t << '\n';
        }
      }  // for
