...
//
/**\class EventSetupCacheIdentifierChecker EventSetupCacheIdentifierChecker.cc
 FWCore/Modules/src/EventSetupCacheIdentifierChecker.cc
...
namespace edm {
  ...
  private:
    ...
    // ----------member data ---------------------------
    ParameterSet m_pset;
    std::map<eventsetup::EventSetupRecordKey, std::vector<unsigned int> >
...
void EventSetupCacheIdentifierChecker::beginRun(edm::Run const&, edm::EventSetup const& iSetup) {
  check(iSetup);
}
...
// void
// EventSetupCacheIdentifierChecker::endLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup
// const&)
//{
//}
...
void EventSetupCacheIdentifierChecker::check(edm::EventSetup const& iSetup) {
  ...
  for (auto it = m_recordKeysToExpectedCacheIdentifiers.begin(),
            itEnd = m_recordKeysToExpectedCacheIdentifiers.end();
       it != itEnd; ++it) {
    EventSetupRecord const* pRecord = iSetup.find(it->first);
    if (0 == pRecord) {
      edm::LogWarning("RecordNotInIOV")
          << "The EventSetup Record '" << it->first.name() << "' is not available for this IOV.";
    }
    if (it->second.size() <= m_index) {
      throw cms::Exception("TooFewCacheIDs") << "The vector of cacheIdentifiers for the record "
                                             << it->first.name() << " is too short";
    }
    if (0 != pRecord && pRecord->cacheIdentifier() != it->second[m_index]) {
      throw cms::Exception("IncorrectCacheID")
          << "The Record " << it->first.name()
          << " was supposed to have cacheIdentifier: " << it->second[m_index] << " but instead has "
...
void EventSetupCacheIdentifierChecker::initialize() {
  std::vector<std::string> recordNames{
      m_pset.getParameterNamesForType<std::vector<unsigned int> >(false)};

  for (auto const& name : recordNames) {
    eventsetup::EventSetupRecordKey recordKey(
        eventsetup::EventSetupRecordKey::TypeTag::findType(name));
...
void EventSetupCacheIdentifierChecker::fillDescriptions(
    edm::ConfigurationDescriptions& descriptions) {
  // The following says we do not know what parameters are allowed so do no validation
  // Please change this to state exactly what you do use, even if it is no parameters
  edm::ParameterSetDescription desc;
  desc.addWildcardUntracked<std::vector<unsigned int> >("*")->setComment(
      "The label is the name of an EventSetup Record while the vector contains the expected "
      "cacheIdentifier values for each beginRun, beginLuminosityBlock and event transition");
