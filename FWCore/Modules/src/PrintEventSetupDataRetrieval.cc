...
//
// Implementation:
//     A service which prints which data from the EventSetup have been retrieved since the last time
//     it checked
//
...
namespace edm {
  ...
  // PrintEventSetupDataRetrieval::PrintEventSetupDataRetrieval(const PrintEventSetupDataRetrieval&
  // rhs)
  // {
  ...
  void PrintEventSetupDataRetrieval::fillDescriptions(
  ...
  //
  // const PrintEventSetupDataRetrieval& PrintEventSetupDataRetrieval::operator=(const
  // PrintEventSetupDataRetrieval& rhs)
  // {
  ...
  void PrintEventSetupDataRetrieval::beginLuminosityBlock(LuminosityBlock const&,
  ...
  void PrintEventSetupDataRetrieval::check(EventSetup const& iES) {
    ...
    for (std::vector<eventsetup::EventSetupRecordKey>::const_iterator it = m_recordKeys.begin(),
        ...
        itRetrievedData =
            m_retrievedDataMap
                .insert(std::make_pair(
                    *it, std::pair<unsigned long long, std::map<eventsetup::DataKey, bool> >()))
                .first;
        itRetrievedData->second.first = r->cacheIdentifier();
        std::vector<eventsetup::DataKey> keys;
        r->fillRegisteredDataKeys(keys);
        for (std::vector<eventsetup::DataKey>::const_iterator itData = keys.begin(),
                                                              itDataEnd = keys.end();
      ...
      if (itRetrievedData->second.first != r->cacheIdentifier()) {
        itRetrievedData->second.first = r->cacheIdentifier();
        for (std::map<eventsetup::DataKey, bool>::iterator
                 itDatum = retrievedData.second.second.begin(),
