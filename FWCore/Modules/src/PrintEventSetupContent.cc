...
namespace edm {
  ...
  PrintEventSetupContent::PrintEventSetupContent(ParameterSet const& config)
    ...
    // neededEventSetupRecordDataGetter::EventSetupRecordDataGetter(ParameterSet const& iConfig):
  ...
  void PrintEventSetupContent::print(EventSetup const& iSetup) {
    ...
    for (Records::iterator itrecords = records.begin(), itrecordsend = records.end();
      ...
      if (0 != rec && cacheIdentifiers_[*itrecords] != rec->cacheIdentifier()) {
        ...
        if (compact_) {
          for (Data::iterator itdata = data.begin(), itdataend = data.end(); itdata != itdataend;
               ++itdata) {
            if (not msg)
