...
LogErrorFilter::LogErrorFilter(edm::ParameterSet const& iConfig)
  ...
  if (!atLeastOneError_ && !atLeastOneWarning_) {
    throw edm::Exception(edm::errors::Configuration) << "Useless configuration of the "
                                                        "error/warning filter. Need to select on "
                                                        "an error or a warning or both.\n";
  }
  ...
  if (errorsAndWarnings.failedToGet()) {
    ...
    if (useThresholdsPerKind_) {
      ...
      for (unsigned int iE = 0; iE != errorsAndWarnings->size(); ++iE) {
        ...
        if (std::find(avoidCategories_.begin(), avoidCategories_.end(), iSummary.category) !=
        ...
        if (avoidCategories_.size() != 0) {
          for (unsigned int iE = 0; iE != errorsAndWarnings->size(); ++iE) {
            ...
            if (std::find(avoidCategories_.begin(), avoidCategories_.end(),
                          ((*errorsAndWarnings)[iE]).category) != avoidCategories_.end()) {
              continue;
            } else {
              return true;
            }
