...
/**\class EventSetupRecordDataGetter EventSetupRecordDataGetter.cc
 ...
 Description: Can be configured to 'get' any Data in any EventSetup Record.  Primarily used for
 testing.
...
namespace edm {
  ...
  private:
    ...
    typedef std::map<eventsetup::EventSetupRecordKey, std::vector<eventsetup::DataKey> >
        RecordToDataKeys;
  ...
  void EventSetupRecordDataGetter::fillDescriptions(ConfigurationDescriptions& descriptions) {
    ...
    desc.addVPSet("toGet", toGet, emptyVect)
        ->setComment(
            ...
            "A 'string' named 'record' that holds the name of an EventSetup record holding the "
  ...
  void EventSetupRecordDataGetter::doGet(EventSetup const& iSetup) {
    if (0 == recordToDataKeys_.size()) {
      ...
      for (Parameters::const_iterator itToGet = toGet.begin(), itToGetEnd = toGet.end();
        ...
        for (Strings::iterator itDatum = dataNames.begin(), itDatumEnd = dataNames.end();
          ...
          if (datumType == eventsetup::TypeTag()) {
            // not found
            edm::LogWarning("DataGetter")
                << "data item of type \"" << datumName << "\" does not exist" << std::endl;
    ...
    for (RecordToDataKeys::iterator itRecord = recordToDataKeys_.begin(),
      ...
      if (0 == pRecord) {
        edm::LogWarning("RecordNotInIOV") << "The EventSetup Record '" << itRecord->first.name()
      ...
      if (0 != pRecord && pRecord->cacheIdentifier() != recordToCacheIdentifier_[itRecord->first]) {
        ...
        for (Keys::const_iterator itKey = keys.begin(), itKeyEnd = keys.end(); itKey != itKeyEnd;
             ++itKey) {
          if (!pRecord->doGet(*itKey)) {
            edm::LogWarning("DataGetter")
                << "No data of type \"" << itKey->type().name() << "\" with name \""
                << itKey->name().value() << "\" in record " << itRecord->first.type().name()
                << " found " << std::endl;
          } else {
            if (verbose_) {
              edm::LogSystem("DataGetter")
                  << "got data of type \"" << itKey->type().name() << "\" with name \""
                  << itKey->name().value() << "\" in record " << itRecord->first.type().name()
                  << std::endl;
            }
          }
