...
namespace edm {
  ...
  EDLooperBase::Status NavigateEventsLooper::duringLoop(Event const&, EventSetup const&,
    ...
    if (pc.forwardState() == ProcessingController::kEventsAheadInFile) {
      ...
      std::cout << "(0) process the next event if it exists (at last event in the open file. "
    ...
    if (pc.canRandomAccess()) {
      if (pc.reverseState() == ProcessingController::kEventsBackwardsInFile) {
        ...
        std::cout << "(1) process the previous event if there are any (at first event in the "
                     "open file. there are previous files)\n";
    ...
    do {
      ...
      if (!(std::cin >> x) || x < 0 || x > 4) {
        ...
        std::cout << "Please enter numeric characters only. The value must be in the range 0 "
                     "to 4 (inclusive). Please try again."
                  << std::endl;
      }
      if (!pc.canRandomAccess() && (x == 1 || x == 2)) {
        inputFailed = true;
        std::cout << "The source cannot do random access. 1 and 2 are illegal values. Please "
                     "try again."
                  << std::endl;
      }
    } while (inputFailed);
  ...
  EDLooperBase::Status NavigateEventsLooper::endOfLoop(EventSetup const&,
