  ...
  void EmptyESSource::delaySettingRecords() {
    ...
    if (recordKey == eventsetup::EventSetupRecordKey()) {
      throw edm::Exception(errors::Configuration)
          ...
          << "If the spelling is fine, then no module in the job requires this Record and "
             "therefore EmptyESSource can not function.\n"
             "In such a case please either remove the EmptyESSource with label'"
          << descriptionForFinder().label_
          << "' from your job or add a module which needs the Record to your job.";
    }
    findingRecordWithKey(recordKey);
  }

  void EmptyESSource::setIntervalFor(eventsetup::EventSetupRecordKey const&,
                                     IOVSyncValue const& iTime, ValidityInterval& oInterval) {
