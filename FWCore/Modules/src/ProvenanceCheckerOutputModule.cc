...
namespace edm {
  ...
  // ProvenanceCheckerOutputModule::operator=(ProvenanceCheckerOutputModule const& rhs)
  // {
  ...
  namespace {
    void markAncestors(EventForOutput const& e, ProductProvenance const& iInfo,
                       std::map<BranchID, bool>& oMap, std::set<BranchID>& oMapperMissing) {
      for (BranchID const id : iInfo.parentage().parents()) {
        ...
        if (oMap.find(id) == oMap.end()) {
          // use side effect of calling operator[] which is if the item isn't there it will add
          // it as 'false'
          oMap[id];
  ...
  void ProvenanceCheckerOutputModule::write(EventForOutput const& e) {
    ...
    for (auto const product : keptProducts()[InEvent]) {
      ...
      markAncestors(e, *(bh.provenance()->productProvenance()), seenParentInPrincipal,
                    missingFromMapper);
