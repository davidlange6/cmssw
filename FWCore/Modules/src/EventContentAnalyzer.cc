...
namespace edm {
  ...
  namespace {
    ...
    void doPrint<char>(std::string const& iName, ObjectWithDict const& iObject,
      ...
      LogAbsolute("EventContent") << iIndent << iName << kNameValueSep
                                  << static_cast<int>(
                                         *reinterpret_cast<char*>(iObject.address()));  // << "\n";
    }

    template <>
    void doPrint<unsigned char>(std::string const& iName, ObjectWithDict const& iObject,
                                std::string const& iIndent) {
      LogAbsolute("EventContent") << iIndent << iName << kNameValueSep
    ...
    void doPrint<bool>(std::string const& iName, ObjectWithDict const& iObject,
                       std::string const& iIndent) {
      LogAbsolute("EventContent") << iIndent << iName << kNameValueSep
                                  << ((*reinterpret_cast<bool*>(iObject.address()))
                                          ? "true"
                                          : "false");  // << "\n";
    }
    ...
    void printObject(std::string const& iName, ObjectWithDict const& iObject,
      ...
      if (iObject.typeOf().isPointer()) {
        LogAbsolute("EventContent")
            << iIndent << iName << kNameValueSep << formatClassName(iObject.typeOf().name())
            << std::hex << iObject.address() << std::dec;  // << "\n";
        TypeWithDict pointedType =
            iObject.typeOf().toType();  // for Pointers, I get the real type this way
        if (TypeWithDict::byName("void") == pointedType || pointedType.isPointer() ||
            iObject.address() == 0) {
          return;
        }
        return;
      ...
      LogAbsolute("EventContent") << indent << printName << " "
    ...
    bool printAsContainer(std::string const& iName, ObjectWithDict const& iObject,
      ...
      try {
        ...
        // std::cout << "size of type '" << sizeObj.name() << "' " << sizeObj.typeName() <<
        // std::endl;
        ...
        // atReturnType.SizeOf()
        // << " pointer? " << atReturnType.isPointer() << " ref? " << atReturnType.isReference()
        // << std::endl;

        // Return by reference must be treated differently since reflex will not properly create
        // memory for a ref (which should just be a pointer to the object and not the object
        // itself)
        ...
        size_t index = 0;
        // The argument to the 'at' function is the index. Since the argument list holds
        // pointers to the arguments
        // we only need to create it once and then when the value of index changes the pointer
        // already
        // gets the new value
        ...
        for (; index != size; ++index) {
          ...
          if (isRef) {
            ...
            // Although to hold the return value from a reference reflex requires you to pass
            // it a
            // void** when it tries to call methods on the reference it expects to be given a
            // void*
    ...
    void printObject(Event const& iEvent, std::string const& iClassName,
                     std::string const& iModuleLabel, std::string const& iInstanceLabel,
  ...
  EventContentAnalyzer::EventContentAnalyzer(ParameterSet const& iConfig)
      : indentation_(iConfig.getUntrackedParameter("indentation", std::string("++"))),
        ...
        moduleLabels_(
            iConfig.getUntrackedParameter("verboseForModuleLabels", std::vector<std::string>())),
        verbose_(iConfig.getUntrackedParameter("verbose", false) || moduleLabels_.size() > 0),
        getModuleLabels_(
            iConfig.getUntrackedParameter("getDataForModuleLabels", std::vector<std::string>())),
        ...
        if (getModuleLabels_.empty()) {
          ...
          if (iBranch.className() != kPathStatus && iBranch.className() != kEndPathStatus) {
            this->consumes(edm::TypeToGet{iBranch.unwrappedTypeID(), PRODUCT_TYPE},
                           edm::InputTag{iBranch.moduleLabel(), iBranch.productInstanceName(),
                                         iBranch.processName()});
          }
        } else {
  ...
  void EventContentAnalyzer::analyze(Event const& iEvent, EventSetup const&) {
    ...
    if (listContent_) {
      LogAbsolute("EventContent")
          << "\n"
          << indentation_ << "Event " << std::setw(5) << evno_ << " contains " << provenances.size()
    ...
    for (auto const& provenance : provenances) {
      ...
      bool doVerbose =
          verbose_ && (moduleLabels_.empty() || binary_search_all(moduleLabels_, modLabel));

      if (listContent_ || doVerbose) {
        LogAbsolute("EventContent")
            << indentation_ << friendlyName << " \"" << modLabel << "\" \"" << instanceName
            << "\" \"" << processName << "\""
            << " (productId = " << provenance->productID() << ")" << std::endl;
      }

      std::string key = friendlyName + std::string(" + \"") + modLabel + std::string("\" + \"") +
                        instanceName + "\" \"" + processName + "\"";
      ++cumulates_[key];

      if (doVerbose) {
        // indent one level before starting to print
        printObject(iEvent, className, modLabel, instanceName, processName, startIndent,
                    verboseIndentation_);
        continue;
      }
      if (getData_) {
  ...
  void EventContentAnalyzer::endJob() {
    ...
    LogAbsolute("EventContent") << "\nSummary for key being the concatenation of "
                                   "friendlyClassName, moduleLabel, productInstanceName and "
                                   "processName"
                                << std::endl;
    for (nameMap::const_iterator it = cumulates_.begin(), itEnd = cumulates_.end(); it != itEnd;
         ++it) {
      LogAbsolute("EventContent") << std::setw(6) << it->second << " occurrences of key "
                                  << it->first << std::endl;
    }
  ...
  void EventContentAnalyzer::fillDescriptions(ConfigurationDescriptions& descriptions) {
    ...
    np->setComment(
        "This string is used to further indent lines when printing the contents of products in "
        "verbose mode.");

    std::vector<std::string> defaultVString;

    np = desc.addOptionalUntracked<std::vector<std::string> >("verboseForModuleLabels",
                                                              defaultVString);
    np->setComment(
        "If this vector is not empty, then only products with module labels on this list are "
        "printed.");

    np = desc.addOptionalUntracked<bool>("getData", false);
    np->setComment("If true the products will be retrieved using getByLabel.");

    np = desc.addOptionalUntracked<std::vector<std::string> >("getDataForModuleLabels",
                                                              defaultVString);
    np->setComment(
        "If this vector is not empty, then only products with module labels on this list are "
        "retrieved by getByLabel.");
