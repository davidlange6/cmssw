...
namespace edmtest {
  ...
  TestTriggerNames::TestTriggerNames(edm::ParameterSet const& ps)
      : iEvent_(0U),
        ...
        expectedTriggerResultsHLT_(ps.getUntrackedParameter<std::vector<unsigned int> >(
            "expectedTriggerResultsHLT", std::vector<unsigned int>())),
        expectedTriggerResultsPROD_(ps.getUntrackedParameter<std::vector<unsigned int> >(
  ...
  void TestTriggerNames::analyze(edm::Event const& e, edm::EventSetup const&) {
    ...
    if (expected_trigger_previous_.size() > 0) {
      ...
      while ((index < prod.size()) and
             (moduleDescription().processName() == prod[index].provenance()->processName())) {
        ++index;
      }
    ...
    if (expectedTriggerResultsPROD_.size() > iEvent_) {
      ...
      edm::LogAbsolute("TEST") << "Event " << iEvent_ << "  " << resultsByNamePROD.accept("p1")
                               << std::endl;
    }
    ++iEvent_;
  }
