...
namespace edmtest {
  namespace limited {
    ...
    public:
      ...
      ~StreamIntFilter() {
        if (m_count != trans_) {
          throw cms::Exception("transitions") << "StreamIntFilter transitions " << m_count
    ...
    class RunIntFilter
        : public edm::limited::EDFilter<edm::StreamCache<UnsafeCache>, edm::RunCache<Cache>> {
    public:
      ...
      void globalEndRun(edm::Run const& iRun, edm::EventSetup const&) const override {
        ...
        if (rCache->value != cvalue_) {
          throw cms::Exception("cache value") << "RunIntFilter cache value " << rCache->value
    ...
    class LumiIntFilter : public edm::limited::EDFilter<edm::StreamCache<UnsafeCache>,
                                                        edm::LuminosityBlockCache<Cache>> {
    public:
      explicit LumiIntFilter(edm::ParameterSet const& p)
          : edm::limited::EDFilterBase(p),
            edm::limited::EDFilter<edm::StreamCache<UnsafeCache>, edm::LuminosityBlockCache<Cache>>(
                p),
            trans_(p.getParameter<int>("transitions")),
            cvalue_(p.getParameter<int>("cachevalue")) {
        produces<unsigned int>();
      ...
      void streamBeginLuminosityBlock(edm::StreamID iID, edm::LuminosityBlock const& iLB,
        ...
        if (lCache->lumi == 0) {
          throw cms::Exception("out of sequence")
              << "streamBeginLuminosityBlock seen before globalBeginLuminosityBlock in "
                 "LuminosityBlock"
              << iLB.luminosityBlock();
        }
      }
      ...
      void globalEndLuminosityBlock(edm::LuminosityBlock const& iLB,
        ...
        if (lCache->lumi != 0) {
          throw cms::Exception("end out of sequence")
              << "globalEndLuminosityBlock seen before globalBeginLuminosityBlock in "
                 "LuminosityBlock"
              << iLB.luminosityBlock();
        }
        if (lCache->value != cvalue_) {
          throw cms::Exception("cache value") << "LumiIntFilter cache value " << lCache->value
                                              << " but it was supposed to be " << cvalue_;
        }
      }
    ...
    class RunSummaryIntFilter : public edm::limited::EDFilter<edm::StreamCache<Cache>,
    ...
    public:
      ...
      std::shared_ptr<UnsafeCache> globalBeginRunSummary(edm::Run const&,
                                                         edm::EventSetup const&) const override {
        ++m_count;
      ...
      void globalEndRunSummary(edm::Run const&, edm::EventSetup const&,
        ...
        if (gCache->value != cvalue_) {
          throw cms::Exception("cache value") << "RunSummaryIntFilter cache value " << gCache->value
                                              << " but it was supposed to be " << cvalue_;
        }
        --(gCache->run);
      }
      ...
      ~RunSummaryIntFilter() {
        if (m_count != trans_) {
          throw cms::Exception("transitions") << "RunSummaryIntFilter transitions " << m_count
                                              << " but it was supposed to be " << trans_;
        }
      }
    };
    ...
    class LumiSummaryIntFilter
        : public edm::limited::EDFilter<edm::StreamCache<Cache>,
                                        edm::LuminosityBlockSummaryCache<UnsafeCache>> {
    public:
      explicit LumiSummaryIntFilter(edm::ParameterSet const& p)
          : edm::limited::EDFilterBase(p),
            edm::limited::EDFilter<edm::StreamCache<Cache>,
                                   edm::LuminosityBlockSummaryCache<UnsafeCache>>(p),
            trans_(p.getParameter<int>("transitions")),
            cvalue_(p.getParameter<int>("cachevalue")) {
        produces<unsigned int>();
      }
      ...
      void streamEndLuminosityBlockSummary(edm::StreamID iID, edm::LuminosityBlock const&,
                                           edm::EventSetup const&,
                                           UnsafeCache* gCache) const override {
        ++m_count;
        if (gCache->lumi == 0) {
          throw cms::Exception("out of sequence")
              << "streamEndLuminosityBlockSummary after globalEndLuminosityBlockSummary in "
                 "Stream "
              << iID.value();
        }
        auto sCache = streamCache(iID);
      ...
      ~LumiSummaryIntFilter() {
        if (m_count != trans_) {
          throw cms::Exception("transitions") << "LumiSummaryIntFilter transitions " << m_count
                                              << " but it was supposed to be " << trans_;
        }
      }
    };

    class TestBeginRunFilter
        : public edm::limited::EDFilter<edm::RunCache<Dummy>, edm::BeginRunProducer> {
    public:
      ...
      ~TestBeginRunFilter() {
        if (m_count != trans_) {
          throw cms::Exception("transitions") << "TestBeginRunFilter transitions " << m_count
                                              << " but it was supposed to be " << trans_;
        }
      }
    };
    ...
    class TestEndRunFilter
        : public edm::limited::EDFilter<edm::RunCache<Dummy>, edm::EndRunProducer> {
    public:
      ...
      ~TestEndRunFilter() {
        if (m_count != trans_) {
          throw cms::Exception("transitions") << "TestEndRunFilter transitions " << m_count
                                              << " but it was supposed to be " << trans_;
        }
      }
    };
    ...
    class TestBeginLumiBlockFilter
        : public edm::limited::EDFilter<edm::LuminosityBlockCache<Dummy>,
                                        edm::BeginLuminosityBlockProducer> {
    public:
      explicit TestBeginLumiBlockFilter(edm::ParameterSet const& p)
          : edm::limited::EDFilterBase(p),
            edm::limited::EDFilter<edm::LuminosityBlockCache<Dummy>,
                                   edm::BeginLuminosityBlockProducer>(p),
            trans_(p.getParameter<int>("transitions")) {
        produces<unsigned int>();
      }
      const unsigned int trans_;
      ...
      void globalBeginLuminosityBlockProduce(edm::LuminosityBlock&,
                                             edm::EventSetup const&) const override {
        ++m_count;
        gblp = true;
      }
      ...
      ~TestBeginLumiBlockFilter() {
        if (m_count != trans_) {
          throw cms::Exception("transitions") << "TestBeginLumiBlockFilter transitions " << m_count
                                              << " but it was supposed to be " << trans_;
        }
      }
    };

    class TestEndLumiBlockFilter : public edm::limited::EDFilter<edm::LuminosityBlockCache<Dummy>,
    ...
    public:
      ...
      ~TestEndLumiBlockFilter() {
        if (m_count != trans_) {
          throw cms::Exception("transitions") << "TestEndLumiBlockFilter transitions " << m_count
                                              << " but it was supposed to be " << trans_;
        }
      }
    };
