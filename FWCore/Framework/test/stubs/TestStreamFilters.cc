...
namespace edmtest {
  namespace stream {
    ...
    public:
      ...
      static std::shared_ptr<Cache> globalBeginRun(edm::Run const& iRun, edm::EventSetup const&,
                                                   GlobalCache const*) {
        ++m_count;
        gbr = true;
        ger = false;
    ...
    public:
      ...
      static void globalEndLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&,
        ...
        if (iLBContext->luminosityBlock()->value != cvalue_) {
          throw cms::Exception("cache value")
              << "LumiIntFilter cache value " << iLBContext->luminosityBlock()->value
    ...
    class RunSummaryIntFilter
        : public edm::stream::EDFilter<edm::RunCache<Cache>, edm::RunSummaryCache<Cache>> {
    public:
      static std::atomic<unsigned int> m_count;
      unsigned int trans_;
      static std::atomic<unsigned int> cvalue_;
      ...
      static std::shared_ptr<Cache> globalBeginRunSummary(edm::Run const&, edm::EventSetup const&,
                                                          GlobalCache const*) {
        ++m_count;
        gbrs = true;
        gers = false;
    ...
    class TestBeginRunFilter
        : public edm::stream::EDFilter<edm::RunCache<Cache>, edm::BeginRunProducer> {
    public:
      static std::atomic<unsigned int> m_count;
      unsigned int trans_;
      static std::atomic<unsigned int> cvalue_;
    ...
    class TestEndLumiBlockFilter : public edm::stream::EDFilter<edm::LuminosityBlockCache<Cache>,
