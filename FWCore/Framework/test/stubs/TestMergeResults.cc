...
namespace edmtest {
  ...
  private:
    void checkExpectedLumiProducts(unsigned int index, std::vector<int> const& expectedValues,
                                   edm::InputTag const& tag, char const* functionName,
                                   edm::LuminosityBlock const& lumi);

    void checkExpectedRunProducts(unsigned int index, std::vector<int> const& expectedValues,
                                  edm::InputTag const& tag, char const* functionName,
                                  edm::Run const& run);
  ...
  TestMergeResults::TestMergeResults(edm::ParameterSet const& ps)
      : default_(),
        ...
        expectedBeginRunProd_(
            ps.getUntrackedParameter<std::vector<int> >("expectedBeginRunProd", default_)),
        expectedEndRunProd_(
            ps.getUntrackedParameter<std::vector<int> >("expectedEndRunProd", default_)),
        expectedBeginLumiProd_(
            ps.getUntrackedParameter<std::vector<int> >("expectedBeginLumiProd", default_)),
        expectedEndLumiProd_(
            ps.getUntrackedParameter<std::vector<int> >("expectedEndLumiProd", default_)),

        expectedBeginRunNew_(
            ps.getUntrackedParameter<std::vector<int> >("expectedBeginRunNew", default_)),
        expectedEndRunNew_(
            ps.getUntrackedParameter<std::vector<int> >("expectedEndRunNew", default_)),
        expectedBeginLumiNew_(
            ps.getUntrackedParameter<std::vector<int> >("expectedBeginLumiNew", default_)),
        expectedEndLumiNew_(
            ps.getUntrackedParameter<std::vector<int> >("expectedEndLumiNew", default_)),

        expectedParents_(ps.getUntrackedParameter<std::vector<std::string> >("expectedParents",
                                                                             defaultvstring_)),
        expectedProcessHistoryInRuns_(ps.getUntrackedParameter<std::vector<std::string> >(
            "expectedProcessHistoryInRuns", defaultvstring_)),

        expectedDroppedEvent_(
            ps.getUntrackedParameter<std::vector<int> >("expectedDroppedEvent", default_)),
        expectedDroppedEvent1_(
            ps.getUntrackedParameter<std::vector<int> >("expectedDroppedEvent1", default_)),
        ...
        expectedInputFileNames_(ps.getUntrackedParameter<std::vector<std::string> >(
  ...
  void TestMergeResults::endRun(edm::Run const& run, edm::EventSetup const&) {
    ...
    for (edm::ProcessHistory::const_iterator iter = ph.begin(), iEnd = ph.end(); iter != iEnd;
         ++iter) {
  ...
  void TestMergeResults::abortWithMessage(char const* whichFunction, char const* type,
