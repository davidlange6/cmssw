...
namespace edmtest {
  namespace limited {
    ...
    public:
      ...
      ~StreamIntProducer() {
        if (m_count != trans_) {
          throw cms::Exception("transitions") << "StreamIntProducer transitions " << m_count
    ...
    class RunIntProducer
        : public edm::limited::EDProducer<edm::StreamCache<UnsafeCache>, edm::RunCache<Cache>> {
    public:
      ...
      void globalEndRun(edm::Run const& iRun, edm::EventSetup const&) const override {
        ...
        if (rCache->value != cvalue_) {
          throw cms::Exception("cache value") << "RunIntProducer cache value " << rCache->value
      ...
      ~RunIntProducer() {
        if (m_count != trans_) {
          throw cms::Exception("transitions") << "RunIntProducer transitions " << m_count
                                              << " but it was supposed to be " << trans_;
        }
      }
    };

    class LumiIntProducer : public edm::limited::EDProducer<edm::StreamCache<UnsafeCache>,
    ...
    public:
      explicit LumiIntProducer(edm::ParameterSet const& p)
          : edm::limited::EDProducerBase(p),
            edm::limited::EDProducer<edm::StreamCache<UnsafeCache>,
                                     edm::LuminosityBlockCache<Cache>>(p),
            trans_(p.getParameter<int>("transitions")),
            cvalue_(p.getParameter<int>("cachevalue")) {
        produces<unsigned int>();
      }
      ...
      void globalEndLuminosityBlock(edm::LuminosityBlock const& iLB,
        ...
        if (lCache->value != cvalue_) {
          throw cms::Exception("cache value") << "LumiIntProducer cache value " << lCache->value
                                              << " but it was supposed to be " << cvalue_;
        }
      }
      ...
      ~LumiIntProducer() {
        if (m_count != trans_) {
          throw cms::Exception("transitions") << "LumiIntProducer transitions " << m_count
                                              << " but it was supposed to be " << trans_;
        }
      }
    };
    ...
    class RunSummaryIntProducer
        : public edm::limited::EDProducer<edm::StreamCache<UnsafeCache>,
                                          edm::RunSummaryCache<UnsafeCache>> {
    public:
      explicit RunSummaryIntProducer(edm::ParameterSet const& p)
          : edm::limited::EDProducerBase(p),
            edm::limited::EDProducer<edm::StreamCache<UnsafeCache>,
                                     edm::RunSummaryCache<UnsafeCache>>(p),
            trans_(p.getParameter<int>("transitions")),
            cvalue_(p.getParameter<int>("cachevalue")) {
        produces<unsigned int>();
      }
      ...
      std::shared_ptr<UnsafeCache> globalBeginRunSummary(edm::Run const& iRun,
                                                         edm::EventSetup const&) const override {
        ++m_count;
      ...
      ~RunSummaryIntProducer() {
        if (m_count != trans_) {
          throw cms::Exception("transitions") << "RunSummaryIntProducer transitions " << m_count
                                              << " but it was supposed to be " << trans_;
        }
      }
    };
    ...
    public:
      ...
      ~LumiSummaryIntProducer() {
        if (m_count != trans_) {
          throw cms::Exception("transitions") << "LumiSummaryIntProducer transitions " << m_count
                                              << " but it was supposed to be " << trans_;
        }
      }
    };
    ...
    class TestBeginRunProducer
        : public edm::limited::EDProducer<edm::RunCache<Dummy>, edm::BeginRunProducer> {
    public:
      ...
      ~TestBeginRunProducer() {
        if (m_count != trans_) {
          throw cms::Exception("transitions") << "TestBeginRunProducer transitions " << m_count
                                              << " but it was supposed to be " << trans_;
        }
      }
    };
    ...
    class TestEndRunProducer
        : public edm::limited::EDProducer<edm::RunCache<Dummy>, edm::EndRunProducer> {
    public:
      ...
      ~TestEndRunProducer() {
        if (m_count != trans_) {
          throw cms::Exception("transitions") << "TestEndRunProducer transitions " << m_count
                                              << " but it was supposed to be " << trans_;
        }
      }
    };
    ...
    class TestBeginLumiBlockProducer
        : public edm::limited::EDProducer<edm::LuminosityBlockCache<void>,
                                          edm::BeginLuminosityBlockProducer> {
    public:
      explicit TestBeginLumiBlockProducer(edm::ParameterSet const& p)
          : edm::limited::EDProducerBase(p),
            edm::limited::EDProducer<edm::LuminosityBlockCache<void>,
      ...
      ~TestBeginLumiBlockProducer() {
        if (m_count != trans_) {
          throw cms::Exception("transitions") << "TestBeginLumiBlockProducer transitions "
    ...
    public:
      ...
      ~TestEndLumiBlockProducer() {
        if (m_count != trans_) {
          throw cms::Exception("transitions") << "TestEndLumiBlockProducer transitions " << m_count
                                              << " but it was supposed to be " << trans_;
        }
      }
    };
