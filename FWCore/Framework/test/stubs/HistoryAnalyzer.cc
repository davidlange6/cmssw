...
namespace edmtest {
  ...
  HistoryAnalyzer::HistoryAnalyzer(edm::ParameterSet const& params)
      : expectedSize_(params.getParameter<int>("expectedSize")),
        ...
        expectedSelectEventsInfo_(
            params.getParameter<std::vector<edm::ParameterSet> >("expectedSelectEventsInfo")),
        ...
        expectedModulesOnEndPaths_(
            params.getParameter<edm::ParameterSet>("expectedModulesOnEndPaths")) {}
  ...
  void HistoryAnalyzer::analyze(edm::Event const& event, edm::EventSetup const&) {
    ...
    if (eventCount_ == 0) {
      ...
      if (!expectedEndPaths_.empty()) {
        ...
        for (vstring::const_iterator i = end_paths.begin(), iEnd = end_paths.end(); i != iEnd;
             ++i) {
      ...
      if (!expectedModules_.empty()) {
        ...
        if (all_modules != expectedModules_) {
          std::copy(all_modules.begin(), all_modules.end(),
                    std::ostream_iterator<std::string>(std::cout, " "));
          std::cout << std::endl;
        ...
        for (vstring::const_iterator i = all_modules.begin(), iEnd = all_modules.end(); i != iEnd;
             ++i) {
          // Make sure the ParameterSet for the module is also present
          edm::ParameterSet const& pset = proc_pset.getParameterSet(*i);
          // This is probably overkill, but also check it can be retrieved by ID from the
          // registry
      ...
      for (vstring::const_iterator i = expectedDroppedEndPaths_.begin(),
                                   iEnd = expectedDroppedEndPaths_.end();
           i != iEnd; ++i) {
        assert(!proc_pset.exists(*i));
      }

      for (vstring::const_iterator i = expectedDroppedModules_.begin(),
                                   iEnd = expectedDroppedModules_.end();
           i != iEnd; ++i) {
        assert(!proc_pset.exists(*i));

        for (edm::pset::Registry::const_iterator j = reg->begin(), jEnd = reg->end(); j != jEnd;
             ++j) {
      ...
      for (vstring::const_iterator i = expectedDropFromProcPSet_.begin(),
                                   iEnd = expectedDropFromProcPSet_.end();
           i != iEnd; ++i) {
