...
namespace edmtest {
  ...
  RunLumiEventAnalyzer::RunLumiEventAnalyzer(edm::ParameterSet const& pset)
    ...
    if (pset.existsAs<std::vector<unsigned int> >("expectedRunLumiEvents", false)) {
      std::vector<unsigned int> temp =
          pset.getUntrackedParameter<std::vector<unsigned int> >("expectedRunLumiEvents");
    ...
    if (pset.existsAs<std::vector<unsigned int> >("expectedRunLumiEvents1", false)) {
      std::vector<unsigned int> temp =
          pset.getUntrackedParameter<std::vector<unsigned int> >("expectedRunLumiEvents1");
  ...
  void RunLumiEventAnalyzer::analyze(edm::Event const& event, edm::EventSetup const&) {
    if (verbose_) {
      edm::LogAbsolute("RunLumiEvent") << "RUN_LUMI_EVENT " << event.run() << ", "
                                       << event.luminosityBlock() << ", " << event.id().event();
    }
  ...
  void RunLumiEventAnalyzer::beginLuminosityBlock(edm::LuminosityBlock const& lumi,
                                                  edm::EventSetup const&) {
    if (verbose_) {
      edm::LogAbsolute("RunLumiEvent")
          << "RUN_LUMI_EVENT " << lumi.run() << ", " << lumi.luminosityBlock() << ", " << 0;
    }
  ...
  void RunLumiEventAnalyzer::endLuminosityBlock(edm::LuminosityBlock const& lumi,
                                                edm::EventSetup const&) {
    if (verbose_) {
      edm::LogAbsolute("RunLumiEvent")
          << "RUN_LUMI_EVENT " << lumi.run() << ", " << lumi.luminosityBlock() << ", " << 0;
