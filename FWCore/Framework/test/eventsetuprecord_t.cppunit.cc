...
static eventsetup::EventSetupRecordProviderFactoryTemplate<eventsetuprecord_t::DummyRecord> const
    s_factory;
...
void testEventsetupRecord::transientTest() {
  ...
  // do another transient access which should not do a reset since we have a non-transient access
  // outstanding
  dummyRecord.get(hDummy);
  dummyRecord.get(hTDummy);
