...
namespace {
  ...
  struct Responder {
    ...
    Responder(std::string const& iName, edm::ConstProductRegistry& iConstReg,
              edm::ProductRegistry& iReg)
    ...
    void respond(edm::BranchDescription const& iDesc) {
      ...
      edm::BranchDescription prod(iDesc.branchType(), name_, iDesc.processName(),
                                  iDesc.fullClassName(), iDesc.friendlyClassName(),
...
void testProductRegistry::setUp() {
  ...
  pset.registerIt();
  intBranch_.reset(new edm::BranchDescription(edm::InEvent, "labeli", "PROD", "int", "int", "int",
                                              "", pset.id(), edm::TypeWithDict(typeid(int))));

  floatBranch_.reset(new edm::BranchDescription(edm::InEvent, "labelf", "PROD", "float", "float",
                                                "float", "", pset.id(),
...
void testProductRegistry::testAddAlias() {
  ...
  CPPUNIT_ASSERT(v.at(0).first == "aliasf" && v.at(0).second == "labelf" &&
                 v.at(1).first == "aliasi" && v.at(1).second == "labeli");
}
