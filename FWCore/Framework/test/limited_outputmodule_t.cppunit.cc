...
private:
  ...
  public:
    ...
    FileOutputModule(edm::ParameterSet const& iPSet)
        : edm::limited::OutputModuleBase(iPSet),
          edm::limited::OutputModule<edm::WatchInputFiles>(iPSet) {}
    std::atomic<unsigned int> m_count{0};
    void write(edm::EventForOutput const&) override { ++m_count; }
    void writeRun(edm::RunForOutput const&) override { ++m_count; }
    void writeLuminosityBlock(edm::LuminosityBlockForOutput const&) override { ++m_count; }
...
namespace {
  ...
  void testTransition(std::shared_ptr<T> iMod, edm::Worker* iWorker,
                      edm::OutputModuleCommunicator* iComm, testLimitedOutputModule::Trans iTrans,
    ...
    if (count != iMod->m_count) {
      std::cout << "For trans "
                << static_cast<std::underlying_type<testLimitedOutputModule::Trans>::type>(iTrans)
...
template <typename T>
void testLimitedOutputModule::testTransitions(std::shared_ptr<T> iMod,
...
void testLimitedOutputModule::fileTest() {
  ...
  CPPUNIT_ASSERT(0 == testProd->m_count);
  testTransitions(testProd,
                  {Trans::kGlobalOpenInputFile, Trans::kEvent, Trans::kGlobalEndLuminosityBlock,
