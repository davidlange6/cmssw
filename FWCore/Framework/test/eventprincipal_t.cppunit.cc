...
std::shared_ptr<edm::ProcessConfiguration> test_ep::fake_single_module_process(
    std::string const& tag, std::string const& processName, edm::ParameterSet const& moduleParams,
    std::string const& release, std::string const& pass) {
  ...
  auto result =
      std::make_shared<edm::ProcessConfiguration>(processName, processParams.id(), release, pass);
...
std::shared_ptr<edm::BranchDescription> test_ep::fake_single_process_branch(
  ...
  auto result = std::make_shared<edm::BranchDescription>(
      edm::InEvent, moduleLabel, processName, productClassName, friendlyProductClassName,
      productInstanceName, moduleClass, modParams.id(), dummyType);
...
void test_ep::setUp() {
  ...
  // Put products we'll look for into the EventPrincipal.
  {
    ...
    auto rp = std::make_shared<edm::RunPrincipal>(runAux, pProductRegistry_, *process,
                                                  &historyAppender_, 0);
    auto lumiAux = std::make_shared<edm::LuminosityBlockAuxiliary>(rp->run(), 1, now, now);
    auto lbp = std::make_shared<edm::LuminosityBlockPrincipal>(lumiAux, pProductRegistry_, *process,
                                                               &historyAppender_, 0);
    lbp->setRunPrincipal(rp);
    edm::EventAuxiliary eventAux(eventID_, uuid, now, true);
    pEvent_.reset(new edm::EventPrincipal(pProductRegistry_, branchIDListHelper,
                                          thinnedAssociationsHelper, *process, &historyAppender_,
...
void clear_map(MAP& m) {
  for (typename MAP::iterator i = m.begin(), e = m.end(); i != e; ++i) i->second.reset();
}
...
void test_ep::failgetbyLabelTest() {
  ...
  edm::BasicHandle h(pEvent_->getByLabel(edm::PRODUCT_TYPE, tid, label, std::string(),
