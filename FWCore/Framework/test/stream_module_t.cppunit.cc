...
private:
  ...
  public:
    ...
    static std::shared_ptr<int> globalBeginRun(edm::Run const&, edm::EventSetup const&,
                                               GlobalCache const*) {
      ++m_count;
  ...
  public:
    ...
    static std::shared_ptr<int> globalBeginRunSummary(edm::Run const&, edm::EventSetup const&,
                                                      GlobalCache const*) {
      ++m_count;
      return std::shared_ptr<int>{};
    }
    ...
    static void globalEndRunSummary(edm::Run const&, edm::EventSetup const&, RunContext const*,
                                    int*) {
      ++m_count;
    }
  };
  ...
  class EndRunSummaryProd
      : public edm::stream::EDProducer<edm::EndRunProducer, edm::RunSummaryCache<int>> {
  public:
    static unsigned int m_count;
  ...
  class EndLumiSummaryProd : public edm::stream::EDProducer<edm::EndLuminosityBlockProducer,
...
testStreamModule::testStreamModule()
  ...
  m_ep.reset(new edm::EventPrincipal(m_prodReg, m_idHelper, m_associationsHelper, m_procConfig,
                                     nullptr, *pID));
...
namespace {
  ...
  void testTransition(edm::Worker* iWorker, testStreamModule::Trans iTrans,
    ...
    if (count != T::m_count) {
      std::cout << "For trans "
                << static_cast<std::underlying_type<testStreamModule::Trans>::type>(iTrans)
...
void testStreamModule::endRunSummaryProdTest() {
  runTest<EndRunSummaryProd>({Trans::kGlobalEndRun, Trans::kEvent, Trans::kGlobalBeginRun,
