...
private:
  ...
  class EndRunSummaryProd
      : public edm::global::EDProducer<edm::EndRunProducer, edm::RunSummaryCache<int>> {
  public:
    mutable unsigned int m_count = 0;
    void produce(edm::StreamID, edm::Event&, edm::EventSetup const&) const override { ++m_count; }
  ...
  class EndLumiSummaryProd : public edm::global::EDProducer<edm::EndLuminosityBlockProducer,
...
namespace {
  ...
  void testTransition(std::shared_ptr<T> iMod, edm::Worker* iWorker, testGlobalModule::Trans iTrans,
    ...
    if (count != iMod->m_count) {
      std::cout << "For trans "
                << static_cast<std::underlying_type<testGlobalModule::Trans>::type>(iTrans)
