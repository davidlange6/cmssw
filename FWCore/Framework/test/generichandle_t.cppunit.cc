...
void testGenericHandle::failgetbyLabelTest() {
  ...
  edm::ProcessConfiguration pc("PROD", edm::ParameterSetID(), edm::getReleaseVersion(),
                               edm::getPassID());
  ...
  auto lbp =
      std::make_shared<edm::LuminosityBlockPrincipal>(lumiAux, preg, pc, &historyAppender_, 0);
...
void testGenericHandle::getbyLabelTest() {
  ...
  edm::BranchDescription product(edm::InEvent, label, processName, dummytype.userClassName(),
                                 className, productInstanceName, "", pset.id(), dummytype);
  ...
  std::string uuid = edm::createGlobalIdentifier();
  edm::ProcessConfiguration pc("PROD", dummyProcessPset.id(), edm::getReleaseVersion(),
                               edm::getPassID());
  ...
  auto lbp =
      std::make_shared<edm::LuminosityBlockPrincipal>(lumiAux, pregc, pc, &historyAppender_, 0);
  lbp->setRunPrincipal(rp);
  edm::EventAuxiliary eventAux(col, uuid, fakeTime, true);
  edm::EventPrincipal ep(pregc, branchIDListHelper, thinnedAssociationsHelper, pc,
