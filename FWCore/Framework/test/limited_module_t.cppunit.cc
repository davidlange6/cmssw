...
private:
  ...
  public:
    StreamProd()
        : edm::limited::EDProducerBase(s_pset),
          edm::limited::EDProducer<edm::StreamCache<int>>(s_pset) {}
  ...
  public:
    RunProd()
        : edm::limited::EDProducerBase(s_pset),
          edm::limited::EDProducer<edm::RunCache<int>>(s_pset) {}
    mutable unsigned int m_count = 0;
    void produce(edm::StreamID, edm::Event&, edm::EventSetup const&) const override { ++m_count; }
  ...
  public:
    LumiProd()
        : edm::limited::EDProducerBase(s_pset),
          edm::limited::EDProducer<edm::LuminosityBlockCache<int>>(s_pset) {}
    mutable unsigned int m_count = 0;
    void produce(edm::StreamID, edm::Event&, edm::EventSetup const&) const override { ++m_count; }
  ...
  public:
    RunSummaryProd()
        : edm::limited::EDProducerBase(s_pset),
          edm::limited::EDProducer<edm::RunSummaryCache<int>>(s_pset) {}
    mutable unsigned int m_count = 0;
    void produce(edm::StreamID, edm::Event&, edm::EventSetup const&) const override { ++m_count; }

    std::shared_ptr<int> globalBeginRunSummary(edm::Run const&,
                                               edm::EventSetup const&) const override {
      ++m_count;
      return std::shared_ptr<int>{};
    }

    void streamEndRunSummary(edm::StreamID, edm::Run const&, edm::EventSetup const&,
                             int*) const override {
      ++m_count;
    }
  ...
  public:
    ...
    void streamEndLuminosityBlockSummary(edm::StreamID, edm::LuminosityBlock const&,
  ...
  public:
    BeginRunProd()
        : edm::limited::EDProducerBase(s_pset),
          edm::limited::EDProducer<edm::BeginRunProducer>(s_pset) {}
    mutable unsigned int m_count = 0;
    void produce(edm::StreamID, edm::Event&, edm::EventSetup const&) const override { ++m_count; }
  ...
  public:
    BeginLumiProd()
        : edm::limited::EDProducerBase(s_pset),
          edm::limited::EDProducer<edm::BeginLuminosityBlockProducer>(s_pset) {}
    mutable unsigned int m_count = 0;
    void produce(edm::StreamID, edm::Event&, edm::EventSetup const&) const override { ++m_count; }
  ...
  public:
    EndRunProd()
        : edm::limited::EDProducerBase(s_pset),
          edm::limited::EDProducer<edm::EndRunProducer>(s_pset) {}
    mutable unsigned int m_count = 0;
    void produce(edm::StreamID, edm::Event&, edm::EventSetup const&) const override { ++m_count; }
  ...
  public:
    EndLumiProd()
        : edm::limited::EDProducerBase(s_pset),
          edm::limited::EDProducer<edm::EndLuminosityBlockProducer>(s_pset) {}
    mutable unsigned int m_count = 0;
    void produce(edm::StreamID, edm::Event&, edm::EventSetup const&) const override { ++m_count; }
  ...
  class EndRunSummaryProd
      : public edm::limited::EDProducer<edm::EndRunProducer, edm::RunSummaryCache<int>> {
  public:
    EndRunSummaryProd()
        : edm::limited::EDProducerBase(s_pset),
  ...
  class EndLumiSummaryProd
      : public edm::limited::EDProducer<edm::EndLuminosityBlockProducer,
                                        edm::LuminosityBlockSummaryCache<int>> {
  public:
    EndLumiSummaryProd()
        : edm::limited::EDProducerBase(s_pset),
          edm::limited::EDProducer<edm::EndLuminosityBlockProducer,
                                   edm::LuminosityBlockSummaryCache<int>>(s_pset) {}
    mutable unsigned int m_count = 0;
    void produce(edm::StreamID, edm::Event&, edm::EventSetup const&) const override { ++m_count; }
    ...
    void globalEndLuminosityBlockProduce(edm::LuminosityBlock&, edm::EventSetup const&,
...
testLimitedModule::testLimitedModule()
  ...
  m_lbp.reset(
      new edm::LuminosityBlockPrincipal(lumiAux, m_prodReg, m_procConfig, &historyAppender_, 0));
  m_lbp->setRunPrincipal(m_rp);
  edm::EventAuxiliary eventAux(eventID, uuid, now, true);

  m_ep.reset(
      new edm::EventPrincipal(m_prodReg, m_idHelper, m_associationsHelper, m_procConfig, nullptr));
  ...
  m_transToFunc[Trans::kGlobalBeginLuminosityBlock] = [this](edm::Worker* iBase) {
    typedef edm::OccurrenceTraits<edm::LuminosityBlockPrincipal, edm::BranchActionGlobalBegin>
        Traits;
    edm::ParentContext nullParentContext;
  ...
  m_transToFunc[Trans::kStreamBeginLuminosityBlock] = [this](edm::Worker* iBase) {
    typedef edm::OccurrenceTraits<edm::LuminosityBlockPrincipal, edm::BranchActionStreamBegin>
        Traits;
    edm::ParentContext nullParentContext;
    iBase->doWork<Traits>(*m_lbp, *m_es, s_streamID0, nullParentContext, nullptr);
  };
...
namespace {
  ...
  void testTransition(std::shared_ptr<T> iMod, edm::Worker* iWorker,
    ...
    if (count != iMod->m_count) {
      std::cout << "For trans "
                << static_cast<std::underlying_type<testLimitedModule::Trans>::type>(iTrans)
...
void testLimitedModule::streamTest() {
  ...
  testTransitions(
      testProd,
      {Trans::kBeginStream, Trans::kStreamBeginRun, Trans::kStreamBeginLuminosityBlock,
