...
void TestEDConsumerBase::testRegularType() {
  ...
  const auto vint_c =
      helper.index(edm::PRODUCT_TYPE, typeID_vint, "labelC", "instanceC", "processC");
  const auto vint_c_no_proc =
      helper.index(edm::PRODUCT_TYPE, typeID_vint, "labelC", "instanceC", 0);
  const auto vint_blank =
      helper.index(edm::PRODUCT_TYPE, typeID_vint, "label", "instance", "process");
  const auto vint_blank_no_proc =
      helper.index(edm::PRODUCT_TYPE, typeID_vint, "label", "instance", 0);
  {
    std::vector<edm::InputTag> vTags = {{"label", "instance", "process"},
                                        {"labelC", "instanceC", "processC"}};
    ...
    CPPUNIT_ASSERT(indices.end() != std::find(indices.begin(), indices.end(),
                                              edm::ProductResolverIndexAndSkipBit(vint_c, false)));
  ...
  {
    std::vector<edm::InputTag> vTagsRev = {{"labelC", "instanceC", "processC"},
  ...
  }
  {
    ...
    std::vector<edm::InputTag> vTags = {{"label", "instance"},
                                        {"labelC", "instanceC", "@skipCurrentProcess"}};
    IntsConsumer intConsumer{vTags};
    intConsumer.updateLookup(edm::InEvent, helper, false);
...
void TestEDConsumerBase::testViewType() {
  ...
  const auto v_simple =
      helper.index(edm::ELEMENT_TYPE, typeID_Simple, "labelC", "instanceC", "processC");

  const auto v_int_no_proc = helper.index(edm::ELEMENT_TYPE, typeID_int, "label", "instance");
  const auto v_simple_no_proc =
      helper.index(edm::ELEMENT_TYPE, typeID_Simple, "labelC", "instanceC");

  {
...

    {
      ...
      CPPUNIT_ASSERT(2 == indices.size());
      CPPUNIT_ASSERT(indices.end() != std::find(indices.begin(), indices.end(),
                                                edm::ProductResolverIndexAndSkipBit(v_int, false)));
      CPPUNIT_ASSERT(indices.end() !=
                     std::find(indices.begin(), indices.end(),
                               edm::ProductResolverIndexAndSkipBit(v_simple, false)));
...

  {
    std::vector<std::pair<edm::TypeToGet, edm::InputTag>> vT = {
        ...
        {edm::TypeToGet::make<edm::View<edmtest::Simple>>(),
         {"labelC", "instanceC", "@skipCurrentProcess"}}};
    TypeToGetConsumer consumer{vT};
...
void TestEDConsumerBase::testMany() {
  ...
  const auto productIndex =
      helper.index(edm::PRODUCT_TYPE, typeID_EventID, "labelB", "instanceB", "processB");

  {
    ...
    CPPUNIT_ASSERT(indices.end() !=
                   std::find(indices.begin(), indices.end(),
                             edm::ProductResolverIndexAndSkipBit(productIndex, false)));
  }
}
...
void TestEDConsumerBase::testMay() {
  ...
  {
    std::vector<edm::InputTag> vTags;
    std::vector<edm::InputTag> vMayTags = {{"label", "instance", "process"},
                                           {"labelC", "instanceC", "processC"}};
    IntsMayConsumer consumer{vTags, vMayTags};
    consumer.updateLookup(edm::InEvent, helper, false);
    ...
    CPPUNIT_ASSERT(indicesMay.end() !=
                   std::find(indicesMay.begin(), indicesMay.end(),
                             edm::ProductResolverIndexAndSkipBit(vint_c, false)));
    CPPUNIT_ASSERT(indicesMay.end() !=
                   std::find(indicesMay.begin(), indicesMay.end(),
                             edm::ProductResolverIndexAndSkipBit(vint_blank, false)));
  ...
  {
    std::vector<edm::InputTag> vTags;
    std::vector<edm::InputTag> vMayTags = {{"label", "instance", ""},
                                           {"labelC", "instanceC", "@skipCurrentProcess"}};
    IntsMayConsumer consumer{vTags, vMayTags};
    consumer.updateLookup(edm::InEvent, helper, false);
