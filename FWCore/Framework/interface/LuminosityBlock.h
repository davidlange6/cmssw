...
namespace edm {
  ...
  public:
    ...
    /**\return Reusable index which can be used to separate data for different simultaneous
     * LuminosityBlocks.
     */
  ...
  bool LuminosityBlock::getByLabel(std::string const& label, std::string const& productInstanceName,
    ...
    if (!provRecorder_.checkIfComplete<PROD>()) {
      principal_get_adapter_detail::throwOnPrematureRead("Lumi", TypeID(typeid(PROD)), label,
                                                         productInstanceName);
    }
    result.clear();
  ...
  bool LuminosityBlock::getByLabel(InputTag const& tag, Handle<PROD>& result) const {
    if (!provRecorder_.checkIfComplete<PROD>()) {
      principal_get_adapter_detail::throwOnPrematureRead("Lumi", TypeID(typeid(PROD)), tag.label(),
                                                         tag.instance());
    }
    result.clear();
