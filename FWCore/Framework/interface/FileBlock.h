...
namespace edm {
  ...
  public:
    ...
    FileBlock(FileFormatVersion const& version, TTree const* ev, TTree const* meta,
              TTree const* lumi, TTree const* lumiMeta, TTree const* run, TTree const* runMeta,
              int whyNotFastClonable, std::array<bool, NumBranchTypes> const& hasNewlyDroppedBranch,
              std::string const& fileName, bool branchListIndexesUnchanged, bool modifiedIDs,
