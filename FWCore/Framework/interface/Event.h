...
namespace edm {
  ...
  public:
    ...
    TriggerResultsByName triggerResultsByName(
    ...
    typedef std::vector<
        std::pair<edm::propagate_const<std::unique_ptr<WrapperBase>>, BranchDescription const*>>
        ProductPtrVec;
  ...
  private:
    ...
    void commit_(std::vector<edm::ProductResolverIndex> const& iShouldPut,
                 std::vector<BranchID>* previousParentage = nullptr,
  ...
  struct RecordInParentless {
    ...
    void do_it(ptrvec_t& /*ignored*/, ptrvec_t& used, std::unique_ptr<Wrapper<PROD>> wp,
  ...
  struct RecordInParentfull {
    ...
    void do_it(ptrvec_t& used, ptrvec_t& /*ignored*/, std::unique_ptr<Wrapper<PROD>> wp,
               BranchDescription const* desc) const {
      used.emplace_back(std::move(wp), desc);
    }
  };
  ...
  OrphanHandle<PROD> Event::put(std::unique_ptr<PROD> product,
    ...
    std::conditional_t<detail::has_donotrecordparents<PROD>::value, RecordInParentless<PROD>,
                       RecordInParentfull<PROD>>
        parentage_recorder;
    parentage_recorder.do_it(putProducts(), putProductsWithoutParents(), std::move(wp), &desc);
  ...
  template <typename PROD>
  bool Event::getByLabel(std::string const& label, std::string const& productInstanceName,
                         Handle<PROD>& result) const {
    result.clear();
    BasicHandle bh = provRecorder_.getByLabel_(TypeID(typeid(PROD)), label, productInstanceName,
  ...
  bool Event::getByToken(EDGetToken token, Handle<PROD>& result) const {
    result.clear();
    BasicHandle bh =
        provRecorder_.getByToken_(TypeID(typeid(PROD)), PRODUCT_TYPE, token, moduleCallingContext_);
    convert_handle(std::move(bh), result);  // throws on conversion error
    if (result.failedToGet()) {
      return false;
    }
  ...
  bool Event::getByLabel(std::string const& moduleLabel, std::string const& productInstanceName,
                         Handle<View<ELEMENT>>& result) const {
    result.clear();
    BasicHandle bh = provRecorder_.getMatchingSequenceByLabel_(TypeID(typeid(ELEMENT)), moduleLabel,
