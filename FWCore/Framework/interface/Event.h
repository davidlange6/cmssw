...
namespace edm {
  ...
  private:
    ...
    void commit_(std::vector<edm::ProductResolverIndex> const& iShouldPut,
                 std::vector<BranchID>* previousParentage = nullptr, ParentageID* previousParentageId = nullptr);
    void commit_aux(ProductPtrVec& products, bool record_parents, std::vector<BranchID>* previousParentage = nullptr,
  ...
  OrphanHandle<PROD> Event::put(std::unique_ptr<PROD> product, std::string const& productInstanceName) {
    ...
    std::conditional_t<detail::has_donotrecordparents<PROD>::value, RecordInParentless<PROD>, RecordInParentfull<PROD>>
        parentage_recorder;
  ...
  RefProd<PROD> Event::getRefBeforePut(std::string const& productInstanceName) {
    PROD* p = nullptr;
    BranchDescription const& desc = provRecorder_.getBranchDescription(TypeID(*p), productInstanceName);
