...
/**\class data_default_record_trait data_default_record_trait.h
 ...
 Usage:
    Many types of data in a EventSetup are only available in one Record type.  For example, the HCal
 Alignment
  data only appears in the HCal Alignment Record. For such cases, it is annoying for users to have
 to specify
  both the Record and the Data type in order to get the data.  In such a case, a specialization of
  data_default_record_trait for that data type can be assigned to allow access 'direct' access to
 that data
  from the EventSetup
  ...
  eventSetup.get<MyDataRecord>.get(pMyData);
    ...
    To specify the default record, you must use the macro EVENTSETUP_DATA_DEFAULT_RECORD in the
 header file for
