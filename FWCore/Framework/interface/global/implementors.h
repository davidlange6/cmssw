  ...
  namespace global {
    namespace impl {
      ...
      private:
        ...
        virtual void doStreamEndRun_(StreamID id, Run const& rp,
                                     EventSetup const& c) override final {
          streamEndRun(id, rp, c);
        }
        virtual void doStreamBeginLuminosityBlock_(StreamID id, LuminosityBlock const& lbp,
                                                   EventSetup const& c) override final {
          streamBeginLuminosityBlock(id, lbp, c);
        }
        virtual void doStreamEndLuminosityBlock_(StreamID id, LuminosityBlock const& lbp,
                                                 EventSetup const& c) override final {
      ...
      private:
        ...
        virtual std::shared_ptr<C> globalBeginRun(edm::Run const&,
      ...
      public:
        ...
        LuminosityBlockCacheHolder<T, C>& operator=(LuminosityBlockCacheHolder<T, C> const&) =
            delete;
      ...
      private:
        ...
        void doStreamEndRunSummary_(StreamID id, Run const& rp,
                                    EventSetup const& c) override final {
        ...
        virtual std::shared_ptr<C> globalBeginRunSummary(edm::Run const&,
                                                         edm::EventSetup const&) const = 0;
        virtual void streamEndRunSummary(StreamID, edm::Run const&, edm::EventSetup const&,
                                         C*) const = 0;
      ...
      public:
        ...
        LuminosityBlockSummaryCacheHolder<T, C>& operator=(
            LuminosityBlockSummaryCacheHolder<T, C> const&) = delete;
        ~LuminosityBlockSummaryCacheHolder() noexcept(false){};

      private:
        friend class EndLuminosityBlockSummaryProducer<T, C>;

        void doBeginLuminosityBlockSummary_(edm::LuminosityBlock const& lb,
                                            EventSetup const& c) override final {
          cache_ = globalBeginLuminosityBlockSummary(lb, c);
        }

        virtual void doStreamEndLuminosityBlockSummary_(StreamID id, LuminosityBlock const& lb,
                                                        EventSetup const& c) override final {
          std::lock_guard<std::mutex> guard(mutex_);
          streamEndLuminosityBlockSummary(id, lb, c, cache_.get());
        }
        void doEndLuminosityBlockSummary_(LuminosityBlock const& lb,
                                          EventSetup const& c) override final {
          globalEndLuminosityBlockSummary(lb, c, cache_.get());
        }

        virtual std::shared_ptr<C> globalBeginLuminosityBlockSummary(
            edm::LuminosityBlock const&, edm::EventSetup const&) const = 0;
        virtual void streamEndLuminosityBlockSummary(StreamID, edm::LuminosityBlock const&,
                                                     edm::EventSetup const&, C*) const = 0;

        virtual void globalEndLuminosityBlockSummary(edm::LuminosityBlock const&,
                                                     edm::EventSetup const&, C*) const = 0;
      ...
      private:
        void doBeginLuminosityBlockProduce_(LuminosityBlock& lb,
                                            EventSetup const& c) override final;
        virtual void globalBeginLuminosityBlockProduce(edm::LuminosityBlock&,
                                                       edm::EventSetup const&) const = 0;
      };
      ...
      private:
        ...
        virtual void globalEndLuminosityBlockProduce(edm::LuminosityBlock&,
                                                     edm::EventSetup const&) const = 0;
      };
      ...
      public:
        ...
        EndLuminosityBlockSummaryProducer& operator=(EndLuminosityBlockSummaryProducer const&) =
            delete;
        ~EndLuminosityBlockSummaryProducer() noexcept(false){};

      private:
        void doEndLuminosityBlockProduce_(LuminosityBlock& lb, EventSetup const& c) override final {
          globalEndLuminosityBlockProduce(lb, c,
                                          LuminosityBlockSummaryCacheHolder<T, S>::cache_.get());
        }

        virtual void globalEndLuminosityBlockProduce(edm::LuminosityBlock&, edm::EventSetup const&,
                                                     S const*) const = 0;
      };
    }
  }
