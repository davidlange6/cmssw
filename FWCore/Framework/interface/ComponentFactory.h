...
/**\class ComponentFactory ComponentFactory.h FWCore/Framework/interface/ComponentFactory.h
 ...
 Description: Factory for building the Factories for the various 'plug-in' components needed for the
 EventSetup
      ...
      std::shared_ptr<base_type> addTo(EventSetupsController& esController,
        ...
        if (it == makers_.end()) {
          std::shared_ptr<Maker> wm(
              edmplugin::PluginFactory<ComponentMakerBase<T>*()>::get()->create(modtype));

          if (wm.get() == nullptr) {
            Exception::throwThis(errors::Configuration, "UnknownModule", T::name().c_str(),
...
#define COMPONENTFACTORY_GET(_type_)                                                             \
  EDM_REGISTER_PLUGINFACTORY(                                                                    \
      edmplugin::PluginFactory<edm::eventsetup::ComponentMakerBase<_type_>*()>, _type_::name()); \
