...
FWCore/Framework/interface/eventsetup_dependsOn.h
 ...
 Usage:
    The function dependsOn takes a variable number of pointers to member methods as arguments.
These methods are then called
  for the ESProducer whenever the dependent Record (determined by looking at the methods lone
argument) has changed since the
...
/*
 ...
 The dependsOn function does not have enough information to convert the list of member function
 pointers directly into the
 appropriate Decorator class since it is missing the Record type of the Decorator (all it knows are
 the Record types the
 Decorator depends on).  Therefore we must defer the creation of the Decorator until that Record
 type is known (which
 happens within the body of the ESProducer::setWhatProduced method).  To allow the deferred
 construction,
 the dependsOn method returns a compile time linked list created via the TwoHolder class (if there
 is only one
 node you get a OneHolder).  The dependsOn method always makes sure that the second type of the
 TwoHolder is the member
 function pointer which is needed for the later construction stage.

 Within the body of ESProducer::setWhatProduced, the proper Decorator is created by calling
 'createDecoratorFrom' which is given
 a pointer to the Producer, a dummy pointer to the proper Record and the linked list of member
 function pointers.  The
 'createDecoratorFrom' uses the makeCaller and createDependsOnCaller functions to recursively create
 the proper DependsOnCaller
 functor which is then used by the ESPreFunctorDecorator to do the work.  We use  HolderToCaller
 template class merely to define
...
namespace edm {
  namespace eventsetup {
    ...
    // and if so, calls the appropriate member method.  Multiple callers can be chained together
    // using the
    ...
    DependsOnCaller<T, TRecord, TDependsOnRecord, TCallerChain> createDependsOnCaller(
        T* iCallee, const TRecord*, void (T::*iMethod)(const TDependsOnRecord&),
        const TCallerChain& iChain) {
    ...
    // put implementation details used to get the dependsOn method to work into their own
    // namespace
    namespace depends_on {
      ...
      // HolderToCaller is used to state how a OneHolder or TwoHolder is converted into the
      // appropriate
      // DependsOnCaller.  This class is needed to define the return value of the makeCaller
      // function
      ...
      struct HolderToCaller<TRecord, OneHolder<T, TDependsOnRecord> > {
        typedef DependsOnCaller<T, TRecord, TDependsOnRecord, DependsOnDoNothingCaller<TRecord> >
            Caller_t;
      };
      template <class TRecord, class T, class T1, class T2>
      struct HolderToCaller<TRecord, TwoHolder<T1, void (T::*)(const T2&)> > {
        typedef DependsOnCaller<T, TRecord, T2, typename HolderToCaller<TRecord, T1>::Caller_t>
            Caller_t;
      };
    ...
    // DecoratorFromArg is used to declare the return type of 'createDecoratorFrom' based on the
    ...
    struct DecoratorFromArg<T, TRecord, depends_on::OneHolder<T, TDependsOnRecord> > {
      typedef ESPreFunctorDecorator<TRecord, DependsOnCaller<T, TRecord, TDependsOnRecord,
                                                             DependsOnDoNothingCaller<TRecord> > >
          Decorator_t;
    };
    ...
    inline ESPreFunctorDecorator<
        TRecord, DependsOnCaller<T, TRecord, TDependsOnRecord, DependsOnDoNothingCaller<TRecord> > >
    createDecoratorFrom(T* iT, const TRecord* iRec,
      ...
      ESPreFunctorDecorator<TRecord, DependsOnCaller<T, TRecord, TDependsOnRecord,
                                                     DependsOnDoNothingCaller<TRecord> > >
    ...
    struct DecoratorFromArg<T, TRecord, depends_on::TwoHolder<T1, T2> > {
      typedef ESPreFunctorDecorator<TRecord, typename depends_on::HolderToCaller<
                                                 TRecord, depends_on::TwoHolder<T1, T2> >::Caller_t>
          Decorator_t;
    };
    template <typename T, typename TRecord, typename T1, typename T2>
    inline ESPreFunctorDecorator<TRecord, typename depends_on::HolderToCaller<
                                              TRecord, depends_on::TwoHolder<T1, T2> >::Caller_t>
    createDecoratorFrom(T* iT, const TRecord* iRec, const depends_on::TwoHolder<T1, T2>& iHolder) {
      return ESPreFunctorDecorator<TRecord, typename depends_on::HolderToCaller<
                                                TRecord, depends_on::TwoHolder<T1, T2> >::Caller_t>(
    ...
    depends_on::TwoHolder<depends_on::OneHolder<T, T1>, T2> dependsOn(void (T::*iT1)(const T1&),
                                                                      T2 iT2) {
