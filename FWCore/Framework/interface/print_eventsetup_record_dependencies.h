...
namespace edm {
  ...
  void print_eventsetup_record_dependencies(std::ostream& oStream, std::string iIndent,
                                            TFirst const*, TEnd const* iEnd) {
    iIndent += " ";
    print_eventsetup_record_dependencies<typename boost::mpl::deref<TFirst>::type>(oStream,
                                                                                   iIndent);
  ...
  template <typename RecordT>
  void print_eventsetup_record_dependencies_recursive(std::ostream&, std::string const&,
                                                      boost::mpl::false_) {
    return;
  }
  ...
  void print_eventsetup_record_dependencies(std::ostream& oStream, std::string const& iIndent) {
    oStream << iIndent << edm::eventsetup::EventSetupRecordKey::makeKey<RecordT>().name()
            << std::endl;

    print_eventsetup_record_dependencies_recursive<RecordT>(
        oStream, iIndent,
        rec_dep::inherits_from_DependentRecordTag(static_cast<RecordT const*>(nullptr)));
  }
}
