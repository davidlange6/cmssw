...
namespace edm {
  ...
  public:
    ...
    EventPrincipal(std::shared_ptr<ProductRegistry const> reg,
                   ...
                   ProcessConfiguration const& pc, HistoryAppender* historyAppender, unsigned int streamIndex = 0,
                   bool isForPrimaryProcess = true);
    ~EventPrincipal() override {}

    void fillEventPrincipal(EventAuxiliary const& aux, ProcessHistoryRegistry const& processHistoryRegistry,
    ...
    void fillEventPrincipal(EventAuxiliary const& aux, ProcessHistoryRegistry const& processHistoryRegistry,
                            EventSelectionIDVector&& eventSelectionIDs, BranchListIndexes&& branchListIndexes,
                            ProductProvenanceRetriever const& provRetriever, DelayedReader* reader = nullptr,
    ...
    bool luminosityBlockPrincipalPtrValid() const { return (luminosityBlockPrincipal_) ? true : false; }
    ...
    void putOnRead(BranchDescription const& bd, std::unique_ptr<WrapperBase> edp,
                   ProductProvenance const* productProvenance) const;
    ...
    void getThinnedProducts(ProductID const& pid, std::vector<WrapperBase const*>& foundContainers,
