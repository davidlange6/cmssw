  ...
  namespace eventsetup {
    ...
    public:
      ...
      void storeReturnedValues(TReturn iReturn) {
        ...
        setData(iReturn, static_cast<typename type::head_type*>(nullptr),
                static_cast<const typename type::tail_type*>(nullptr));
      }
      ...
      void setData(ProductsT& iProducts, const RemainingContainerT*, const DataT*) {
        DataT* temp =
            reinterpret_cast<DataT*>(proxyData_[produce::find_index<TReturn, DataT>::value]);
        if (0 != temp) {
      ...
      void setData(ProductsT& iProducts, const produce::Null*, const DataT*) {
        DataT* temp =
            reinterpret_cast<DataT*>(proxyData_[produce::find_index<TReturn, DataT>::value]);
        // std::cout <<" setData["<< produce::find_index<TReturn,DataT>::value<<"] "<< temp
        // <<std::endl;
        if (0 != temp) {
          copyFromTo(iProducts, *temp);
        }
