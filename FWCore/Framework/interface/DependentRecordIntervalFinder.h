...
/**\class DependentRecordIntervalFinder DependentRecordIntervalFinder.h
 ...
 Usage:
    This class is used internally to a EventSetupRecordProvider which delivers a Record that is
 dependent on other Records.

    If no Providers are given, then Finder will always report an invalid ValidityInterval for all
 IOVSyncValues
...
namespace edm {
  namespace eventsetup {
    ...
    private:
      ...
      // ---------- member data --------------------------------
      typedef std::vector<edm::propagate_const<std::shared_ptr<EventSetupRecordProvider>>>
          Providers;
