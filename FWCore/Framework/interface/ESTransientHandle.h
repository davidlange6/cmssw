...
/**\class ESTransientHandle ESTransientHandle.h FWCore/Framework/interface/ESTransientHandle.h
 ...
 Usage:
    This handle is used to setup a memory optimization.  Data obtained via this handle are allowed
to be discarded before
the end of the actual IOV for the data.  In this way the system can claim back some memory
 ...
 2) you only access this EventSetup data once per IOV change of the Record [i.e. you do NOT read it
every Event]
    (failure to do this will cause the EventSetup data to be created each access which can
drastically slow the system)

 If you are unsure whether to use this handle or not then it is best not to and just use the regular
ESHandle.
...
namespace edm {
  ...
  template <class T>
  ESTransientHandle<T>::ESTransientHandle(
