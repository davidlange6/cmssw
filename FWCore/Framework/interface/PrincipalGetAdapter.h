...
namespace edm {
  ...
  public:
    ...
    BranchDescription const& getBranchDescription(TypeID const& type,
    ...
    BasicHandle getMatchingSequenceByLabel_(TypeID const& typeID, std::string const& label,
                                            std::string const& instance, std::string const& process,
                                            ModuleCallingContext const* mcc) const;
  ...
  private:
    ...
    SharedResourcesAcquirer* resourcesAcquirer_;  // We do not use propagate_const because the
                                                  // acquirer is itself mutable.
  };
  ...
  namespace detail {
    ...
    struct has_postinsert {
      static constexpr bool value =
          std::is_same<decltype(has_postinsert_helper<T>(nullptr)), yes_tag>::value &&
