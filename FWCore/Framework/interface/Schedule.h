...
namespace edm {
  ...
  public:
    ...
    typedef std::vector<edm::propagate_const<std::shared_ptr<OutputModuleCommunicator>>>
        AllOutputModuleCommunicators;

    typedef std::vector<Worker*> Workers;

    Schedule(ParameterSet& proc_pset, service::TriggerNamesService const& tns,
             ...
             std::shared_ptr<ProcessConfiguration> processConfiguration, bool hasSubprocesses,
             PreallocationConfiguration const& config, ProcessContext const* processContext);

    void processOneEventAsync(WaitingTaskHolder iTask, unsigned int iStreamID,
                              EventPrincipal& principal, EventSetup const& eventSetup);

    template <typename T>
    void processOneGlobalAsync(WaitingTaskHolder iTask, typename T::MyPrincipal& principal,
                               EventSetup const& eventSetup, bool cleaningUpAfterException = false);

    template <typename T>
    void processOneStreamAsync(WaitingTaskHolder iTask, unsigned int iStreamID,
                               typename T::MyPrincipal& principal, EventSetup const& eventSetup,
    ...
    /// hint is a performance optimization if you might know the position of the module in the
    /// path
    void moduleDescriptionsInPath(std::string const& iPathLabel,
                                  std::vector<ModuleDescription const*>& descriptions,
                                  unsigned int hint) const;

    /// adds the ModuleDescriptions into the vector for the modules scheduled in path
    /// iEndPathLabel
    /// hint is a performance optimization if you might know the position of the module in the
    /// path
  ...
  template <typename T>
  void Schedule::processOneStreamAsync(WaitingTaskHolder iTaskHolder, unsigned int iStreamID,
                                       typename T::MyPrincipal& ep, EventSetup const& es,
                                       bool cleaningUpAfterException) {
    assert(iStreamID < streamSchedules_.size());
    streamSchedules_[iStreamID]->processOneStreamAsync<T>(std::move(iTaskHolder), ep, es,
                                                          cleaningUpAfterException);
  }

  template <typename T>
  void Schedule::processOneGlobalAsync(WaitingTaskHolder iTaskHolder, typename T::MyPrincipal& ep,
