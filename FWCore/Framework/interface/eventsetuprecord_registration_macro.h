...
FWCore/Framework/interface/eventsetuprecord_registration_macro.h
 ...
 Usage:
    Special code is needed to 'register' a new class that inherits from EventSetupRecord with they
system.  The
macro EVENTSETUP_RECORD_REG is used to create that code.

     Example: You have a new record defined called 'DummyRecord'.  Then to register that record with
the system
     you place the line

     EVENTSETUP_RECORD_REG(DummyRecord);

     at 'file scope' (i.e., not within a namespace, class or function) in the .cc file for
DummyRecord
*/
//
// Author:      Chris Jones
...
#define EVENTSETUP_RECORD_REG(_recordclassname_)                                           \
  ...
  static const edm::eventsetup::EventSetupRecordProviderFactoryTemplate<_recordclassname_> \
