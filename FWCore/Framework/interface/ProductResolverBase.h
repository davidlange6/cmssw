...
namespace edm {
  ...
  public:
    ...
    public:
      ...
      ProductData const* data() const {
        return reinterpret_cast<ProductData const*>(kAmbiguityMask &
    ...
    // Retrieves pointer to a class containing both the event independent and the per even
    // provenance.
    ...
    // Initializes the event independent portion of the provenance, plus the process history ID,
    // the product ID, and the provRetriever.
  ...
  private:
    ...
    virtual void setProvenance_(ProductProvenanceRetriever const* provRetriever,
                                ProcessHistory const& ph, ProductID const& pid) = 0;
