...
Description: This is the primary interface for accessing per run EDProducts and inserting new
derived products.
...
namespace edm {
  ...
  public:
    ...
    // bool
    // getProcessParameterSet(std::string const& processName, std::vector<ParameterSet>& ps)
    // const;
  ...
  private:
    ...
    SharedResourcesAcquirer* sharedResourcesAcquirer_;  // We do not use propagate_const because
                                                        // the acquirer is itself mutable.
  ...
  template <typename PROD>
  bool Run::getByLabel(std::string const& label, std::string const& productInstanceName,
                       Handle<PROD>& result) const {
    if (!provRecorder_.checkIfComplete<PROD>()) {
      principal_get_adapter_detail::throwOnPrematureRead("Run", TypeID(typeid(PROD)), label,
                                                         productInstanceName);
    }
    result.clear();
  ...
  bool Run::getByLabel(InputTag const& tag, Handle<PROD>& result) const {
    if (!provRecorder_.checkIfComplete<PROD>()) {
      principal_get_adapter_detail::throwOnPrematureRead("Run", TypeID(typeid(PROD)), tag.label(),
                                                         tag.instance());
    }
    result.clear();
