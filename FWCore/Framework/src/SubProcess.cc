...
namespace edm {
  ...
  SubProcess::SubProcess(ParameterSet& parameterSet, ParameterSet const& topLevelParameterSet,
    ...
    // if the top level process has a maxEvents or maxLuminosityBlocks parameter set, add them to
    // this process.
    ...
    ParameterSet const& optionsPset(
        processParameterSet_->getUntrackedParameterSet("options", ParameterSet()));
    ...
    ServiceToken newToken =
        items.initServices(serviceSets, *processParameterSet_, token, iLegacy, false);
    ...
    schedule_ = items.initSchedule(*processParameterSet_, hasSubProcesses, preallocConfig,
                                   &processContext_);
    ...
    // CMS-THREADING this only works since Run/Lumis are synchronous so when principalCache asks
    // for
    // the reducedProcessHistoryID from a full ProcessHistoryID that registry will not be in use
    // by
    ...
    for (unsigned int index = 0; index < preallocConfig.numberOfStreams(); ++index) {
      auto ep = std::make_shared<EventPrincipal>(
          preg_, branchIDListHelper(), thinnedAssociationsHelper(), *processConfiguration_,
          &(historyAppenders_[index]), index, false /*not primary process*/);
    ...
    for (auto& subProcessPSet : subProcessVParameterSet) {
      subProcesses_.emplace_back(subProcessPSet, topLevelParameterSet, preg_, branchIDListHelper(),
                                 *thinnedAssociationsHelper_, *subProcessParentageHelper_,
                                 esController, *items.actReg_, newToken, iLegacy, preallocConfig,
                                 &processContext_);
    }
  }
  ...
  void SubProcess::selectProducts(ProductRegistry const& preg,
    ...
    parentThinnedAssociationsHelper.selectAssociationProducts(associationDescriptions,
                                                              keptProductsInEvent, keepAssociation);
    ...
    // Now fill in a mapping needed in the case that a branch was dropped while its EDAlias was
    // kept.
  ...
  void SubProcess::processAsync(WaitingTaskHolder iHolder, EventPrincipal const& principal) {
    ...
    ep.fillEventPrincipal(aux, processHistoryRegistry, std::move(esids), std::move(bli),
                          *(principal.productProvenanceRetrieverPtr()),  // NOTE: this transfers
                                                                         // the per product
                                                                         // provenance
    ...
    WaitingTaskHolder finalizeEventTask(make_waiting_task(
        tbb::task::allocate_root(), [this, &ep, iHolder](std::exception_ptr const* iPtr) mutable {
    ...
    schedule_->processOneEventAsync(std::move(afterProcessTask), ep.streamID().value(), ep,
                                    esp_->eventSetup());
  }

  void SubProcess::doBeginRunAsync(WaitingTaskHolder iHolder, RunPrincipal const& principal,
    ...
    auto rpp = std::make_shared<RunPrincipal>(
        ...
        &(historyAppenders_[historyRunOffset_ + principal.index()]), principal.index(), false);
    ...
    beginGlobalTransitionAsync<Traits>(std::move(iHolder), *schedule_, rp, ts,
                                       esp_->eventSetupForInstance(ts), subProcesses_);
  }

  void SubProcess::doEndRunAsync(WaitingTaskHolder iHolder, RunPrincipal const& principal,
  ...
  void SubProcess::doBeginLuminosityBlockAsync(WaitingTaskHolder iHolder,
    ...
    auto lbpp = std::make_shared<LuminosityBlockPrincipal>(
        aux, preg_, *processConfiguration_,
        &(historyAppenders_[historyLumiOffset_ + principal.index()]), principal.index(), false);
  ...
  void SubProcess::writeLumi(ProcessHistoryID const& parentPhID, int runNumber, int lumiNumber) {
    ...
    assert(it != parentToChildPhID_.end());
    auto const& childPhID = it->second;
    schedule_->writeLumi(principalCache_.lumiPrincipal(childPhID, runNumber, lumiNumber),
                         &processContext_);
  ...
  void SubProcess::deleteLumiFromCache(ProcessHistoryID const& parentPhID, int runNumber,
                                       int lumiNumber) {
    std::map<ProcessHistoryID, ProcessHistoryID>::const_iterator it =
        parentToChildPhID_.find(parentPhID);
    assert(it != parentToChildPhID_.end());
  ...
  void SubProcess::doStreamBeginRunAsync(WaitingTaskHolder iHolder, unsigned int id,
                                         RunPrincipal const& principal, IOVSyncValue const& ts) {
    ServiceRegistry::Operate operate(serviceToken_);
  ...
  void SubProcess::doStreamBeginLuminosityBlockAsync(WaitingTaskHolder iHolder, unsigned int id,
    ...
    beginStreamTransitionAsync<Traits>(std::move(iHolder), *schedule_, id, lbp, ts,
                                       esp_->eventSetupForInstance(ts), subProcesses_);
  }
  ...
  void SubProcess::propagateProducts(BranchType type, Principal const& parentPrincipal,
