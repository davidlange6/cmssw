...
namespace edm {
  ...
  namespace {
    ...
    public:
      CallbackWrapper(ProducerBase* iProd, std::function<void(BranchDescription const&)> iCallback,
                      ProductRegistry* iReg, const ModuleDescription& iDesc)
          : prod_(iProd),
            callback_(iCallback),
            reg_(iReg),
            mdesc_(iDesc),
  ...
  void ProducerBase::registerProducts(ProducerBase* producer, ProductRegistry* iReg,
    ...
    if (registrationCallback()) {
      ...
      regService->watchProductAdditions(
          CallbackWrapper(producer, registrationCallback(), iReg, md));
    }
  }

  void ProducerBase::resolvePutIndicies(
