...
namespace edm {
  ...
  bool EDAnalyzer::doEvent(EventPrincipal const& ep, EventSetup const& c, ActivityRegistry* act,
  ...
  bool EDAnalyzer::doBeginRun(RunPrincipal const& rp, EventSetup const& c,
                              ModuleCallingContext const* mcc) {
  ...
  bool EDAnalyzer::doEndRun(RunPrincipal const& rp, EventSetup const& c,
                            ModuleCallingContext const* mcc) {
    Run r(rp, moduleDescription_, mcc);
    r.setConsumer(this);
    this->endRun(r, c);
  ...
  bool EDAnalyzer::doBeginLuminosityBlock(LuminosityBlockPrincipal const& lbp, EventSetup const& c,
                                          ModuleCallingContext const* mcc) {
  ...
  bool EDAnalyzer::doEndLuminosityBlock(LuminosityBlockPrincipal const& lbp, EventSetup const& c,
                                        ModuleCallingContext const* mcc) {
    LuminosityBlock lb(lbp, moduleDescription_, mcc);
    lb.setConsumer(this);
  ...
  void EDAnalyzer::callWhenNewProductsRegistered(
  ...
  void EDAnalyzer::prevalidate(ConfigurationDescriptions& iConfig) {
