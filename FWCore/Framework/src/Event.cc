...
namespace edm {
      ...
      : provRecorder_(ep, md),
        ...
        luminosityBlock_(
            ep.luminosityBlockPrincipalPtrValid()
                ? new LuminosityBlock(ep.luminosityBlockPrincipal(), md, moduleCallingContext)
                : nullptr),
        gotBranchIDs_(),
        gotViews_(),
  ...
  void Event::setSharedResourcesAcquirer(SharedResourcesAcquirer* iResourceAcquirer) {
    ...
    const_cast<LuminosityBlock*>(luminosityBlock_.get())
        ->setSharedResourcesAcquirer(iResourceAcquirer);
  }
  ...
  void Event::commit_aux(Event::ProductPtrVec& products, bool record_parents,
    ...
    // Check that previousParentageId is still good by seeing if previousParentage matches
    // gotBranchIDs_
    bool sameAsPrevious =
        ((nullptr != previousParentage) && (previousParentage->size() == gotBranchIDs_.size()));
  ...
  size_t Event::size() const {
    return putProducts().size() + provRecorder_.principal().size() +
           putProductsWithoutParents().size();
  }

  BasicHandle Event::getByLabelImpl(std::type_info const&, std::type_info const& iProductType,
  ...
  TriggerResultsByName Event::triggerResultsByName(
      edm::TriggerResults const& triggerResults) const {
    edm::TriggerNames const* names = triggerNames_(triggerResults);
