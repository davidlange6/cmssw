...
namespace edm {
  ...
  void Event::commit_aux(Event::ProductPtrVec& products, bool record_parents, std::vector<BranchID>* previousParentage,
    ...
    if (record_parents && !gotBranchIDs_.empty()) {
      ...
      for (BranchIDSet::const_iterator it = gotBranchIDs_.begin(), itEnd = gotBranchIDs_.end(); it != itEnd; ++it) {
  ...
  void Event::addToGotBranchIDs(Provenance const& prov) const { gotBranchIDs_.insert(prov.originalBranchID()); }
