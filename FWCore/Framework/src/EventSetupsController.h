...
namespace edm {
  ...
  namespace eventsetup {
    ...
    public:
      ...
      void putESProducer(ParameterSet const& pset, std::shared_ptr<DataProxyProvider> const& component,
                         unsigned subProcessIndex);

      std::shared_ptr<EventSetupRecordIntervalFinder> getESSourceAndRegisterProcess(ParameterSet const& pset,
                                                                                    unsigned subProcessIndex);
      void putESSource(ParameterSet const& pset, std::shared_ptr<EventSetupRecordIntervalFinder> const& component,
                       unsigned subProcessIndex);
      ...
      void lookForMatches(ParameterSetID const& psetID, unsigned subProcessIndex, unsigned precedingProcessIndex,
                          bool& firstProcessWithThisPSet, bool& precedingHasMatchingPSet) const;

      bool isFirstMatch(ParameterSetID const& psetID, unsigned subProcessIndex, unsigned precedingProcessIndex) const;

      bool isLastMatch(ParameterSetID const& psetID, unsigned subProcessIndex, unsigned precedingProcessIndex) const;

      bool isMatchingESSource(ParameterSetID const& psetID, unsigned subProcessIndex,
                              unsigned precedingProcessIndex) const;

      bool isMatchingESProducer(ParameterSetID const& psetID, unsigned subProcessIndex,
                                unsigned precedingProcessIndex) const;
