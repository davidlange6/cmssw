...
namespace edm {
  EventPrincipal::EventPrincipal(
      ...
      ProcessConfiguration const& pc, HistoryAppender* historyAppender, unsigned int streamIndex,
  ...
  void EventPrincipal::fillEventPrincipal(EventAuxiliary const& aux,
                                          ...
                                          DelayedReader* reader, bool deepCopyRetriever) {
  ...
  void EventPrincipal::fillEventPrincipal(EventAuxiliary const& aux,
    ...
    if (aux.event() == invalidEventNumber) {
      throw Exception(errors::LogicError) << "EventPrincipal::fillEventPrincipal, Invalid event "
                                             "number provided in EventAuxiliary, It is illegal "
                                             "for the event number to be 0\n";
    }
    ...
    if (branchListIndexes_.empty() and branchIDListHelper_->hasProducedProducts()) {
      // Add index into BranchIDListRegistry for products produced this process
      //  if it hasn't already been filled in by the other fillEventPrincipal or by an earlier
  ...
  void EventPrincipal::put(BranchDescription const& bd, std::unique_ptr<WrapperBase> edp,
    ...
    // assert commented out for DaqSource.  When DaqSource no longer uses put(), the assert can be
    // restored.
    // assert(produced());
    if (edp.get() == nullptr) {
  ...
  static void throwProductDeletedException(ProductID const& pid,
    ...
    exception << "get by product ID: The product with given id: " << pid
              ...
              << "\nwas already deleted. This is a configuration error. Please change the "
                 "configuration of the module which caused this exception to state it reads this "
                 "data.";
    throw exception;
  }
    ...
    if (phb == nullptr) {
      return BasicHandle(makeHandleExceptionFactory([pid]() -> std::shared_ptr<cms::Exception> {
        std::shared_ptr<cms::Exception> whyFailed(
            std::make_shared<Exception>(errors::ProductNotFound, "InvalidID"));
  ...
  WrapperBase const* EventPrincipal::getThinnedProduct(ProductID const& pid,
    ...
    // Loop over thinned containers which were made by selecting elements from the parent
    // container
  ...
  Provenance EventPrincipal::getProvenance(ProductID const& pid,
  ...
  edm::ThinnedAssociation const* EventPrincipal::getThinnedAssociation(
    ...
    if (phb == nullptr) {
      throw Exception(errors::LogicError)
          << "EventPrincipal::getThinnedAssociation, ThinnedAssociation ProductResolver cannot "
             "be found\n"
    ...
    if (!(typeid(edm::ThinnedAssociation) == product->dynamicTypeInfo())) {
      throw Exception(errors::LogicError) << "EventPrincipal::getThinnedProduct, product has "
                                             "wrong type, not a ThinnedAssociation.\n";
    }
