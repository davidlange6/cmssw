...
namespace edm {
  ...
  void EDLooperBase::prepareForNextLoop(eventsetup::EventSetupProvider* esp) {
    ...
    for_all(keys, std::bind(&eventsetup::EventSetupProvider::resetRecordPlusDependentRecords, esp,
                            std::placeholders::_1));
  }
  ...
  void EDLooperBase::doBeginRun(RunPrincipal& iRP, EventSetup const& iES,
    ...
    GlobalContext globalContext(
        GlobalContext::Transition::kBeginRun, LuminosityBlockID(iRP.run(), 0), iRP.index(),
  ...
  void EDLooperBase::doEndRun(RunPrincipal& iRP, EventSetup const& iES,
                              ProcessContext* processContext) {
    GlobalContext globalContext(GlobalContext::Transition::kEndRun, LuminosityBlockID(iRP.run(), 0),
  ...
  void EDLooperBase::doBeginLuminosityBlock(LuminosityBlockPrincipal& iLB, EventSetup const& iES,
                                            ProcessContext* processContext) {
    GlobalContext globalContext(GlobalContext::Transition::kBeginLuminosityBlock, iLB.id(),
                                iLB.runPrincipal().index(), iLB.index(), iLB.beginTime(),
                                processContext);
    ParentContext parentContext(&globalContext);
    ModuleContextSentry moduleContextSentry(&moduleCallingContext_, parentContext);
  ...
  void EDLooperBase::doEndLuminosityBlock(LuminosityBlockPrincipal& iLB, EventSetup const& iES,
                                          ProcessContext* processContext) {
    GlobalContext globalContext(GlobalContext::Transition::kEndLuminosityBlock, iLB.id(),
                                iLB.runPrincipal().index(), iLB.index(), iLB.beginTime(),
                                processContext);
    ParentContext parentContext(&globalContext);
    ModuleContextSentry moduleContextSentry(&moduleCallingContext_, parentContext);
