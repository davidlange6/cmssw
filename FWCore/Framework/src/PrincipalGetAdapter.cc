...
namespace edm {
  ...
  void principal_get_adapter_detail::throwOnPrematureRead(char const* principalType,
    ...
    LogWarning("LogicError") << "::getByLabel: An attempt was made to read a " << principalType
                             << " product before end" << principalType << "() was called.\n"
                             << "The product is of type '" << productType
                             << "'.\nThe specified ModuleLabel was '" << moduleLabel
  ...
  void principal_get_adapter_detail::throwOnPrematureRead(char const* principalType,
                                                          ...
                                                          EDGetToken token) {
    throw Exception(errors::LogicError)
        << "::getByToken: An attempt was made to read a " << principalType << " product before end"
    ...
    return BasicHandle(makeHandleExceptionFactory(
        [labels, kindOfType, productType]() -> std::shared_ptr<cms::Exception> {
          ...
          if (kindOfType == PRODUCT_TYPE) {
            *exception << "Principal::getByToken: Found zero products matching all "
                          "criteria\nLooking for type: "
                       << productType << "\n"
  ...
  BasicHandle PrincipalGetAdapter::getByToken_(TypeID const& id, KindOfType kindOfType,
                                               EDGetToken token,
                                               ModuleCallingContext const* mcc) const {
    ...
    BasicHandle h = principal_.getByToken(kindOfType, id, index, skipCurrentProcess, ambiguous,
                                          resourcesAcquirer_, mcc);
    if (ambiguous) {
  ...
  BasicHandle PrincipalGetAdapter::getMatchingSequenceByLabel_(
      TypeID const& typeID, InputTag const& tag, ModuleCallingContext const* mcc) const {
  ...
  BasicHandle PrincipalGetAdapter::getMatchingSequenceByLabel_(
    ...
    auto h = principal_.getByLabel(ELEMENT_TYPE, typeID, label, instance, process, consumer_,
                                   resourcesAcquirer_, mcc);
    return h;
  }
  ...
  BranchDescription const& PrincipalGetAdapter::getBranchDescription(
    ...
    ProductResolverIndex index =
        productResolverIndexHelper.index(PRODUCT_TYPE, type, md_.moduleLabel().c_str(),
    ...
    if (index == ProductResolverIndexInvalid) {
      ...
      throw edm::Exception(edm::errors::InsertFailure)
          ...
          << "To correct the problem:\n"
             "   1) make sure the proper 'produce' call is being made in the module's "
             "constructor,\n"
             "   2) if 'produce' exists and uses a product instance name make sure that same "
             "name is used during the 'put' call.";
    }
