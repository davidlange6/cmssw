  ...
  namespace eventsetup {
      ...
      if (nullptr != multipleFinders_.get()) {
        ...
        if (nullptr != oldFinder.get()) {
          cms::Exception("EventSetupMultipleSources")
              ...
              << "after all the other sources have been dealt with.  This is a logic error, "
                 "please send email to the framework group.";
        }
      }
    }
    ...
    void EventSetupRecordProvider::setDependentProviders(
      ...
      std::shared_ptr<DependentRecordIntervalFinder> newFinder =
          std::make_shared<DependentRecordIntervalFinder>(key());

      std::shared_ptr<EventSetupRecordIntervalFinder> old = swapFinder(newFinder);
      for_all(iProviders,
              std::bind(std::mem_fun(&DependentRecordIntervalFinder::addProviderWeAreDependentOn),
                        &(*newFinder), _1));
      // if a finder was already set, add it as a depedency.  This is done to ensure that the
      // IOVs properly change even if the
    ...
    void EventSetupRecordProvider::usePreferred(const DataToPreferredProviderMap& iMap) {
      using std::placeholders::_1;
      for_all(providers_,
              std::bind(&EventSetupRecordProvider::addProxiesToRecordHelper, this, _1, iMap));
    ...
    bool EventSetupRecordProvider::setValidityIntervalFor(const IOVSyncValue& iTime) {
      ...
      // we reset any transients just in case some modules get their data at beginRun or
      // beginLumi
