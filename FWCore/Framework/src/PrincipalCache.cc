...
namespace edm {
  ...
  RunPrincipal& PrincipalCache::runPrincipal(ProcessHistoryID const& phid, RunNumber_t run) const {
    if (phid != reducedInputProcessHistoryID_ || run != run_ || runPrincipal_.get() == 0) {
      throwRunMissing();
    }
  ...
  LuminosityBlockPrincipal& PrincipalCache::lumiPrincipal(ProcessHistoryID const& phid,
                                                          RunNumber_t run,
  ...
  void PrincipalCache::merge(std::shared_ptr<RunAuxiliary> aux,
    ...
    if (inputProcessHistoryID_ != aux->processHistoryID()) {
      if (reducedInputProcessHistoryID_ !=
          processHistoryRegistry_->reducedProcessHistoryID(aux->processHistoryID())) {
  ...
  void PrincipalCache::insert(std::shared_ptr<RunPrincipal> rp) {
    ...
    if (inputProcessHistoryID_ != rp->aux().processHistoryID()) {
      reducedInputProcessHistoryID_ =
          processHistoryRegistry_->reducedProcessHistoryID(rp->aux().processHistoryID());
  ...
  void PrincipalCache::deleteLumi(ProcessHistoryID const& phid, RunNumber_t run,
    ...
    if (reducedInputProcessHistoryID_ != phid || run != run_ || lumi != lumi_) {
      throw edm::Exception(edm::errors::LogicError)
          << "PrincipalCache::deleteLumi\n"
          << "Illegal attempt to delete luminosity block from cache\n"
          << "Run number, lumi numbers, or reduced ProcessHistoryID inconsistent with those in "
             "cache\n"
          << "Contact a Framework Developer\n";
    }
