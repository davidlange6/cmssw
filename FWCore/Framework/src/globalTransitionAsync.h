...
namespace edm {
  ...
  void beginGlobalTransitionAsync(WaitingTaskHolder iWait, Schedule& iSchedule, P& iPrincipal,
    ...
    auto subs = make_waiting_task(
        ...
        [&iSubProcesses, iWait, &iPrincipal, iTS, token](std::exception_ptr const* iPtr) mutable {
          if (iPtr) {
  ...
  template <typename Traits, typename P, typename SC>
  void endGlobalTransitionAsync(WaitingTaskHolder iWait, Schedule& iSchedule, P& iPrincipal,
        ...
        [&iSubProcesses, iWait, &iPrincipal, iTS, token,
         cleaningUpAfterException](std::exception_ptr const* iPtr) mutable {
          ...
          for_all(iSubProcesses,
                  [&iWait, &iPrincipal, iTS, cleaningUpAfterException](auto& subProcess) {
                    subProcessDoGlobalEndTransitionAsync(iWait, subProcess, iPrincipal, iTS,
                                                         cleaningUpAfterException);
                  });
        });
