...
namespace edm {
  namespace {
    ...
    StreamSchedule::WorkerPtr makeInserter(ExceptionToActionTable const& actions,
      ...
      StreamSchedule::WorkerPtr ptr(new edm::WorkerT<TriggerResultInserter::ModuleType>(
    ...
    void initializeBranchToReadingWorker(
      ...
      auto vBranchesToDeleteEarly = opts.getUntrackedParameter<std::vector<std::string>>(
      ...
      if (not vBranchesToDeleteEarly.empty()) {
        ...
        if (temp.size() != vBranchesToDeleteEarly.size()) {
          ...
          std::set_difference(temp.begin(), temp.end(), vBranchesToDeleteEarly.begin(),
                              vBranchesToDeleteEarly.end(), std::back_inserter(missingProducts));
          LogInfo l("MissingProductsForCanDeleteEarly");
          l << "The following products in the 'canDeleteEarly' list are not available in this "
               "job and will be ignored.";
  ...
  StreamSchedule::StreamSchedule(
      ...
      service::TriggerNamesService const& tns, PreallocationConfiguration const& prealloc,
      ...
      std::shared_ptr<ProcessConfiguration> processConfiguration, bool allowEarlyDelete,
    ...
    for (auto const& trig_name : pathNames) {
      fillTrigPath(proc_pset, preg, &prealloc, processConfiguration, trig_bitpos, trig_name,
    ...
    if (!unusedLabels.empty()) {
      ...
      for (auto const& label : unusedLabels) {
        ...
        workerManager_.addToUnscheduledWorkers(*modulePSet, preg, &prealloc, processConfiguration,
  ...
  void StreamSchedule::initializeEarlyDelete(ModuleRegistry& modReg, edm::ParameterSet const& opts,
    ...
    }
    {
      ...
      if (not unusedBranches.empty()) {
        ...
        l << "The following products in the 'canDeleteEarly' list are not used in this job and "
             "will be ignored.\n"
             " If possible, remove the producer from the job or add the product to the "
             "producer's own 'mightGet' list.";
    ...
    if (0 != branchToReadingWorker.size()) {
      ...
      for (auto& branchAndWorker : branchToReadingWorker) {
        ...
        if (alreadySeenWorkers.end() == found) {
          // NOTE: we will set aside enough space in earlyDeleteHelperToBranchIndicies_ to
          // accommodate
          // all the branches that might be read by this worker. However, initially we will
          // only tell the
          // EarlyDeleteHelper about the first one. As additional branches are added via
          // 'appendIndex' the
          ...
          earlyDeleteHelpers_.emplace_back(beginAddress + index, beginAddress + index + 1,
                                           &earlyDeleteBranchToCount_);
          branchAndWorker.second->setEarlyDeleteHelper(&(earlyDeleteHelpers_.back()));
          alreadySeenWorkers.insert(
              std::make_pair(branchAndWorker.second, &(earlyDeleteHelpers_.back())));
      ...
      // Now we can compactify the earlyDeleteHelperToBranchIndicies_ since we may have over
      // estimated the
      ...
      for (auto it = earlyDeleteHelpers_.begin() + 1; it != earlyDeleteHelpers_.end(); ++it) {
        if (itLast->end() != it->begin()) {
          // figure the offset for next Worker since it hasn't been moved yet so it has the
          // original address
  ...
  void StreamSchedule::fillWorkers(ParameterSet& proc_pset, ProductRegistry& preg,
                                   ...
                                   std::string const& pathName, bool ignoreFilters,
                                   PathWorkers& out, std::vector<std::string> const& endPathNames) {
    ...
    for (auto const& name : modnames) {
      ...
      if (modpset == 0) {
        ...
        throw Exception(errors::Configuration)
            << "The unknown module label \"" << moduleLabel << "\" appears in " << pathType << " \""
            << pathName << "\"\n please check spelling or remove that label from the path.";
      }
      assert(isTracked);

      Worker* worker =
          workerManager_.getWorker(*modpset, preg, prealloc, processConfiguration, moduleLabel);
      if (ignoreFilters && filterAction != WorkerInPath::Ignore &&
        ...
        if (!search_all(allowed_filters, worker->description().moduleName())) {
          ...
          LogWarning("FilterOnEndPath")
              << "The EDFilter '" << worker->description().moduleName() << "' with module label '"
              << moduleLabel << "' appears on EndPath '" << pathName << "'.\n"
  ...
  void StreamSchedule::fillTrigPath(
      ParameterSet& proc_pset, ProductRegistry& preg, PreallocationConfiguration const* prealloc,
      std::shared_ptr<ProcessConfiguration const> processConfiguration, int bitpos,
      std::string const& name, TrigResPtr trptr, std::vector<std::string> const& endPathNames) {
    PathWorkers tmpworkers;
    fillWorkers(proc_pset, preg, prealloc, processConfiguration, name, false, tmpworkers,
                endPathNames);

    // an empty path will cause an extra bit that is not used
    if (!tmpworkers.empty()) {
      trig_paths_.emplace_back(bitpos, name, tmpworkers, trptr, actionTable(), actReg_,
                               &streamContext_, &skippingEvent_, PathContext::PathType::kPath);
    } else {
  ...
  void StreamSchedule::fillEndPath(ParameterSet& proc_pset, ProductRegistry& preg,
    ...
    PathWorkers tmpworkers;
    fillWorkers(proc_pset, preg, prealloc, processConfiguration, name, true, tmpworkers,
                endPathNames);

    if (!tmpworkers.empty()) {
      // EndPaths are not supposed to stop if SkipEvent type exception happens
      end_paths_.emplace_back(bitpos, name, tmpworkers, TrigResPtr(), actionTable(), actReg_,
                              &streamContext_, nullptr, PathContext::PathType::kEndPath);
    } else {
  ...
  void StreamSchedule::finishedPaths(std::exception_ptr iExcept, WaitingTaskHolder iWait,
  ...
  std::exception_ptr StreamSchedule::finishProcessOneEvent(std::exception_ptr iExcept) {
    ...
    if (iExcept) {
      ...
      try {
        ...
        if (ex.context().empty()) {
          addContextAndPrintException("Calling function StreamSchedule::processOneEvent", ex,
      ...
      actReg_->preStreamEarlyTerminationSignal_(streamContext_,
                                                TerminationOrigin::ExceptionFromThisContext);
    }
  ...
  void StreamSchedule::modulesInPath(std::string const& iPathLabel,
  ...
  void StreamSchedule::moduleDescriptionsInPath(std::string const& iPathLabel,
    ...
    if (!found) {
      ...
      itFound = std::find_if(trig_paths_.begin(), trig_paths_.end(),
                             std::bind(std::equal_to<std::string>(), iPathLabel,
                                       std::bind(&Path::name, std::placeholders::_1)));
  ...
  void StreamSchedule::moduleDescriptionsInEndPath(
      std::string const& iEndPathLabel, std::vector<ModuleDescription const*>& descriptions,
      unsigned int hint) const {
    descriptions.clear();
    bool found = false;
    TrigPaths::const_iterator itFound;
    ...
    if (!found) {
      // if the hint did not work, do it the slow way
      itFound = std::find_if(end_paths_.begin(), end_paths_.end(),
