...
namespace edm {
  ...
  template <typename T>
  void OutputModuleCommunicatorT<T>::writeRun(edm::RunPrincipal const& rp,
                                              ProcessContext const* processContext) {
    GlobalContext globalContext(
        GlobalContext::Transition::kWriteRun, LuminosityBlockID(rp.run(), 0), rp.index(),
  ...
  void OutputModuleCommunicatorT<T>::writeLumi(edm::LuminosityBlockPrincipal const& lbp,
                                               ProcessContext const* processContext) {
    GlobalContext globalContext(GlobalContext::Transition::kWriteLuminosityBlock, lbp.id(),
                                lbp.runPrincipal().index(), lbp.index(), lbp.beginTime(),
                                processContext);
    ParentContext parentContext(&globalContext);
    ModuleCallingContext mcc(&description());
    ModuleContextSentry moduleContextSentry(&mcc, parentContext);
  ...
  namespace impl {
    ...
    std::unique_ptr<edm::OutputModuleCommunicator> createCommunicatorIfNeeded(
        ::edm::limited::OutputModuleBase* iMod) {
