...
SystemTimeKeeper::SystemTimeKeeper(unsigned int iNumStreams,
  ...
  std::copy(iNamesService.getTrigPaths().begin(), iNamesService.getTrigPaths().end(),
            std::back_inserter(m_pathNames));
  std::copy(iNamesService.getEndPaths().begin(), iNamesService.getEndPaths().end(),
            std::back_inserter(m_pathNames));
...
void SystemTimeKeeper::stopPath(StreamContext const& iStream, PathContext const& iPath,
...
void SystemTimeKeeper::startModuleEvent(StreamContext const& iStream,
  ...
  if (checkBounds(iModule.moduleDescription()->id())) {
    auto& mod = m_streamModuleTiming[iStream.streamID().value()]
...
void SystemTimeKeeper::stopModuleEvent(StreamContext const& iStream,
  ...
  if (checkBounds(iModule.moduleDescription()->id())) {
    ...
    if (iModule.type() == ParentContext::Type::kPlaceInPath) {
      ...
      auto& modTiming =
          pathTiming(iStream, *(place->pathContext())).m_moduleTiming[place->placeInPath()];
...
static void fillPathSummary(
    unsigned int iStartIndex, unsigned int iEndIndex, std::vector<std::string> const& iPathNames,
...
void SystemTimeKeeper::fillTriggerTimingReport(TriggerTimingReport& rep) const {
  ...
  // Per path summary
  {
    fillPathSummary(0, m_endPathOffset, m_pathNames, m_modulesOnPaths, m_streamPathTiming,
                    rep.trigPathSummaries);
    fillPathSummary(m_endPathOffset, m_streamPathTiming[0].size(), m_pathNames, m_modulesOnPaths,
                    m_streamPathTiming, rep.endPathSummaries);
  }
}
