...
namespace edm {
  namespace eventsetup {
    ...
    void DependentRecordIntervalFinder::setIntervalFor(const EventSetupRecordKey& iKey,
      ...
      // NOTE: oInterval is the last value that was used so if nothing changed do not modify
      // oInterval
      ...
      // assume that lumi sections are 23 seconds long and therefore if we take a difference
      // between
      ...
      if (hadChangedIOV) {
        if (closestID.run() != 0) {
          if (closestTimeStamp.value() == 0) {
            // no time
            oInterval =
                ValidityInterval(IOVSyncValue(closestID), IOVSyncValue::invalidIOVSyncValue());
          } else {
            if (closestID.run() == iTime.eventID().run()) {
              // can compare time to lumi
              const unsigned long long kLumiTimeLength = 23;

              if ((iTime.eventID().luminosityBlock() - closestID.luminosityBlock()) *
                      kLumiTimeLength <
          ...
          oInterval =
              ValidityInterval(IOVSyncValue(closestTimeStamp), IOVSyncValue::invalidIOVSyncValue());
        }
      }
    }
