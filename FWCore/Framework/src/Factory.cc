...
namespace edm {
  ...
  std::shared_ptr<maker::ModuleHolder> Factory::makeModule(
      const MakeModuleParams& p, signalslot::Signal<void(const ModuleDescription&)>& pre,
  ...
  std::shared_ptr<maker::ModuleHolder> Factory::makeReplacementModule(
      const edm::ParameterSet& p) const {
