...
namespace edm {
  ...
  EventSelector::EventSelector(ParameterSet const& config, Strings const& pathNames)
      : pathspecs_(config.empty() ? Strings()
                                  : initPathSpecs(config.getParameter<Strings>("SelectEvents"))),
        results_from_current_process_(true),
        accept_all_(initAcceptAll()),
        absolute_acceptors_(),
        conditional_acceptors_(),
  ...
  void EventSelector::initPathNames(Strings const& pathNames) {
    ...
    absolute_acceptors_.clear(), conditional_acceptors_.clear(), exception_acceptors_.clear(),
  ...
  bool EventSelector::acceptOneBit(Bits const& b, HLTGlobalStatus const& tr,
                                   hlt::HLTState const& s) const {
    bool lookForException = (s == hlt::Exception);
    for (auto const& bit : b) {
      hlt::HLTState bstate =
          lookForException ? hlt::Exception : bit.accept_state_ ? hlt::Pass : hlt::Fail;
    ...
    for (unsigned int idx = 0; idx < pathspecs.size(); idx++) {
      ...
      try {
        ...
        for (unsigned int iPath = 0; iPath < fullPathCount; iPath++) {
          ...
          for (int iState = static_cast<int>(hlt::Pass); iState <= static_cast<int>(hlt::Exception);
               iState++) {
  ...
  evtSel::OverlapResult EventSelector::testSelectionOverlap(Strings const& pathspec1,
    ...
    try {
      ...
      if (overlap) return identical(a, b, N) ? evtSel::ExactMatch : evtSel::PartialOverlap;
   ...
   *         full trigger list.
   */
  std::shared_ptr<TriggerResults> EventSelector::maskTriggerResults(
   ...
   */
  std::shared_ptr<TriggerResults> EventSelector::maskTriggerResults(
      Strings const& pathspecs, TriggerResults const& inputResults, Strings const& fullPathList) {
    // fetch and validate the total number of paths
    unsigned int fullPathCount = fullPathList.size();
    if (fullPathCount != inputResults.size()) {
      throw edm::Exception(errors::EventCorruption)
  ...
  std::vector<bool> EventSelector::expandDecisionList(Bits const& b, bool PassOrFail,
                                                      unsigned int n) {
