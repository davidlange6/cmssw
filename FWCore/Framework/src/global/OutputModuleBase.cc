...
namespace edm {
  namespace global {
    ...
    void OutputModuleBase::selectProducts(
        ProductRegistry const& preg, ThinnedAssociationsHelper const& thinnedAssociationsHelper) {
      ...
      thinnedAssociationsHelper.selectAssociationProducts(associationDescriptions,
                                                          keptProductsInEvent, keepAssociation_);
    ...
    void OutputModuleBase::doPreallocate(PreallocationConfiguration const& iPC) {
      ...
      for (auto& s : selectors_) {
        if (seenFirst) {
          detail::configureEventSelector(selectEvents_, process_name_, getAllTriggerNames(), s,
                                         consumesCollector());
        } else {
          seenFirst = true;
        }
        ...
        *branchIDLists_ = *origBranchIDLists_;
        ...
        for (BranchIDList& branchIDList : *branchIDLists_) {
          for (BranchID::value_type& branchID : branchIDList) {
            // Replace BranchID of each dropped branch with that of the kept alias, so the
            // alias branch will have the product ID of the original branch.
