...
namespace edm {
  ...
  public:
    ...
    std::shared_ptr<maker::ModuleHolder> makeModule(
        MakeModuleParams const&, signalslot::Signal<void(ModuleDescription const&)>& iPre,
  ...
  std::shared_ptr<maker::ModuleHolder> WorkerMaker<T>::makeModule(
    ...
    return std::shared_ptr<maker::ModuleHolder>(std::make_shared<maker::ModuleHolderT<ModuleType> >(
