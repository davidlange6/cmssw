...
namespace edm {
  ...
  inline void subProcessDoStreamBeginTransitionAsync(WaitingTaskHolder iHolder,
                                                     SubProcess& iSubProcess, unsigned int i,
  ...
  inline void subProcessDoStreamEndTransitionAsync(WaitingTaskHolder iHolder,
    ...
    iSubProcess.doStreamEndLuminosityBlockAsync(std::move(iHolder), i, iPrincipal, iTS,
  ...
  void beginStreamTransitionAsync(WaitingTaskHolder iWait, Schedule& iSchedule,
                                  unsigned int iStreamIndex, P& iPrincipal, IOVSyncValue const& iTS,
    ...
    auto subs = make_waiting_task(
        tbb::task::allocate_root(), [&iSubProcesses, iWait, iStreamIndex, &iPrincipal, iTS,
          ...
          for_all(iSubProcesses, [&iWait, iStreamIndex, &iPrincipal, iTS](auto& subProcess) {
            subProcessDoStreamBeginTransitionAsync(iWait, subProcess, iStreamIndex, iPrincipal,
                                                   iTS);
          });
        });
  ...
  template <typename Traits, typename P, typename SC>
  void beginStreamsTransitionAsync(WaitingTask* iWait, Schedule& iSchedule, unsigned int iNStreams,
                                   P& iPrincipal, IOVSyncValue const& iTS, EventSetup const& iES,
                                   SC& iSubProcesses) {
    WaitingTaskHolder holdUntilAllStreamsCalled(iWait);
    for (unsigned int i = 0; i < iNStreams; ++i) {
      beginStreamTransitionAsync<Traits>(WaitingTaskHolder(iWait), iSchedule, i, iPrincipal, iTS,
                                         iES, iSubProcesses);
    }
  }
  ...
  template <typename Traits, typename P, typename SC>
  void endStreamsTransitionAsync(WaitingTask* iWait, Schedule& iSchedule, unsigned int iNStreams,
                                 P& iPrincipal, IOVSyncValue const& iTS, EventSetup const& iES,
                                 SC& iSubProcesses, bool cleaningUpAfterException) {
    WaitingTaskHolder holdUntilAllStreamsCalled(iWait);
    for (unsigned int i = 0; i < iNStreams; ++i) {
      endStreamTransitionAsync<Traits>(WaitingTaskHolder(iWait), iSchedule, i, iPrincipal, iTS, iES,
