...
namespace edm {
  Path::Path(int bitpos, std::string const& path_name, WorkersInPath const& workers, TrigResPtr trptr,
             ExceptionToActionTable const& actions, std::shared_ptr<ActivityRegistry> areg,
             StreamContext const* streamContext, std::atomic<bool>* stopProcessingEvent, PathContext::PathType pathType)
      : timesRun_(),
        timesPassed_(),
        timesFailed_(),
        timesExcept_(),
  ...
  bool Path::handleWorkerFailure(cms::Exception& e, int nwrwue, bool isEvent, bool begin, BranchType branchType,
  ...
  void Path::exceptionContext(cms::Exception& ex, bool isEvent, bool begin, BranchType branchType,
                              ModuleDescription const& desc, std::string const& id, PathContext const& pathContext) {
    std::ostringstream ost;
  ...
  void Path::processOneOccurrenceAsync(WaitingTask* iTask, EventPrincipal const& iEP, EventSetup const& iES,
                                       StreamID const& iStreamID, StreamContext const* iStreamContext) {
    waitingTasks_.reset();
    ++timesRun_;
  ...
  void Path::workerFinished(std::exception_ptr const* iException, unsigned int iModuleIndex, EventPrincipal const& iEP,
  ...
  void Path::finished(int iModuleIndex, bool iSucceeded, std::exception_ptr iException, StreamContext const* iContext,
  ...
  void Path::runNextWorkerAsync(unsigned int iNextModuleIndex, EventPrincipal const& iEP, EventSetup const& iES,
    ...
    auto nextTask = make_waiting_task(tbb::task::allocate_root(), [this, iNextModuleIndex, &iEP, &iES, iID, iContext,
