...
namespace edm {
  Path::Path(int bitpos, std::string const& path_name, WorkersInPath const& workers,
             TrigResPtr trptr, ExceptionToActionTable const& actions,
             std::shared_ptr<ActivityRegistry> areg, StreamContext const* streamContext,
             std::atomic<bool>* stopProcessingEvent, PathContext::PathType pathType)
      : timesRun_(),
        timesPassed_(),
        timesFailed_(),
        timesExcept_(),
  ...
  bool Path::handleWorkerFailure(cms::Exception& e, int nwrwue, bool isEvent, bool begin,
                                 BranchType branchType, ModuleDescription const& desc,
                                 std::string const& id) {
    if (e.context().empty()) {
    ...
    exception_actions::ActionCodes action =
        (isEvent ? act_table_->find(e.category()) : exception_actions::Rethrow);
    switch (action) {
  ...
  void Path::exceptionContext(cms::Exception& ex, bool isEvent, bool begin, BranchType branchType,
                              ModuleDescription const& desc, std::string const& id,
  ...
  void Path::setEarlyDeleteHelpers(
      std::map<const Worker*, EarlyDeleteHelper*> const& iWorkerToDeleter) {
  ...
  void Path::processOneOccurrenceAsync(WaitingTask* iTask, EventPrincipal const& iEP,
                                       EventSetup const& iES, StreamID const& iStreamID,
  ...
  void Path::workerFinished(std::exception_ptr const* iException, unsigned int iModuleIndex,
                            EventPrincipal const& iEP, EventSetup const& iES, StreamID const& iID,
    ...
    if (iException) {
      ...
      try {
        std::ostringstream ost;
        ost << iEP.id();
        shouldContinue = handleWorkerFailure(*pEx, iModuleIndex, /*isEvent*/ true, /*isBegin*/ true,
                                             InEvent, worker.getWorker()->description(), ost.str());
  ...
  void Path::finished(int iModuleIndex, bool iSucceeded, std::exception_ptr iException,
                      StreamContext const* iContext, EventPrincipal const& iEP,
  ...
  void Path::runNextWorkerAsync(unsigned int iNextModuleIndex, EventPrincipal const& iEP,
    ...
    auto nextTask = make_waiting_task(
        tbb::task::allocate_root(), [this, iNextModuleIndex, &iEP, &iES, iID, iContext,
                                     token](std::exception_ptr const* iException) {
          ServiceRegistry::Operate guard(token);
          this->workerFinished(iException, iNextModuleIndex, iEP, iES, iID, iContext);
        });

    workers_[iNextModuleIndex]
        .runWorkerAsync<OccurrenceTraits<EventPrincipal, BranchActionStreamBegin>>(
            nextTask, iEP, iES, iID, iContext);
  }
}
