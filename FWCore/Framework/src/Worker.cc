...
namespace edm {
  ...
  bool Worker::shouldRethrowException(std::exception_ptr iPtr, ParentContext const& parentContext,
    ...
    try {
      ...
      ModuleCallingContext tempContext(&description(), ModuleCallingContext::State::kInvalid,
                                       parentContext, nullptr);

      // If we are processing an endpath and the module was scheduled, treat SkipEvent or
      // FailPath
      // as IgnoreCompletely, so any subsequent OutputModules are still run.
      // For unscheduled modules only treat FailPath as IgnoreCompletely but still allow
      // SkipEvent to throw
  ...
  void Worker::prefetchAsync(WaitingTask* iTask, ParentContext const& parentContext,
                             Principal const& iPrincipal) {
    // Prefetch products the module declares it consumes (not including the products it maybe
    // consumes)
    ...
    if (iPrincipal.branchType() == InEvent) {
      actReg_->preModuleEventPrefetchingSignal_.emit(*moduleCallingContext_.getStreamContext(),
