...
namespace edm {
  ...
  namespace {
    ...
    std::shared_ptr<TriggerResultInserter> makeInserter(
        ParameterSet& proc_pset, PreallocationConfiguration const& iPrealloc, ProductRegistry& preg,
      ...
      ModuleDescription md(trig_pset->id(), "TriggerResultInserter", "TriggerResults",
      ...
      try {
        ...
        if (!postCalled) {
          try {
            ...
            // If post throws an exception ignore it because we are already handling another
            // exception
          }
        }
        throw;
    ...
    void checkAndInsertAlias(std::string const& friendlyClassName, std::string const& moduleLabel,
      ...
      if (preg.productList().find(key) == preg.productList().end()) {
        ...
        // We throw an exception only if a module with the specified module label was created in
        // this process.
      ...
      std::string const& theInstanceAlias(instanceAlias == star ? productInstanceName
                                                                : instanceAlias);
      ...
      if (iter != aliasKeys.end()) {
        // The alias matches a previous one.  If the same alias is used for different product,
        // throw.
        if (iter->second != key) {
          throw Exception(errors::Configuration, "EDAlias conflict\n")
              << "The module label alias '" << alias << "' and product instance alias '"
              << theInstanceAlias << "'\n"
              << "are used for multiple products of type '" << friendlyClassName << "'\n"
              << "One has module label '" << moduleLabel << "' and product instance name '"
              << productInstanceName << "',\n"
        ...
        if (prodIter != preg.productList().end()) {
          if (!prodIter->second.produced()) {
            throw Exception(errors::Configuration, "EDAlias\n")
                ...
                << "with module label '" << moduleLabel << "' and product instance name '"
                << productInstanceName << "',\n"
                << "An EDAlias can only be used for products produced in the current "
    ...
    void processEDAliases(ParameterSet const& proc_pset, std::string const& processName,
          ...
          for (ParameterSet& pset : vPSet) {
            ...
            if (productInstanceName == star) {
              ...
              for (ProductRegistry::ProductList::const_iterator it =
                ...
                checkAndInsertAlias(friendlyClassName, moduleLabel, it->first.productInstanceName(),
              ...
              checkAndInsertAlias(friendlyClassName, moduleLabel, productInstanceName, processName,
    ...
    void reduceParameterSet(ParameterSet& proc_pset, vstring const& end_path_name_list,
      ...
      for_all(labelsToBeDropped,
              std::bind(&ParameterSet::eraseOrSetUntrackedParameterSet, std::ref(proc_pset), _1));
      ...
      for (vstring::const_iterator iEndPath = end_path_name_list.begin(),
        ...
        for (vstring::iterator iLabel = labels.begin(), iEnd = labels.end(); iLabel != iEnd;
             ++iLabel) {
      ...
      endAfterRemove =
          std::remove_if(scheduledPaths.begin(), scheduledPaths.end(),
      ...
      endAfterRemove =
          std::remove_if(scheduledEndPaths.begin(), scheduledEndPaths.end(),
                         std::bind(binary_search_string, std::ref(endPathsToBeDropped), _1));
  ...
  Schedule::Schedule(ParameterSet& proc_pset, service::TriggerNamesService const& tns,
                     ProductRegistry& preg, BranchIDListHelper& branchIDListHelper,
    ...
    makePathStatusInserters(pathStatusInserters_, *pathNames_, prealloc, preg, areg,
                            processConfiguration, std::string("PathStatusInserter"));

    makePathStatusInserters(endPathStatusInserters_, *endPathNames_, prealloc, preg, areg,
                            processConfiguration, std::string("EndPathStatusInserter"));
    ...
    for (unsigned int i = 0; i < prealloc.numberOfStreams(); ++i) {
      streamSchedules_.emplace_back(std::make_shared<StreamSchedule>(
          resultsInserter(), pathStatusInserters_, endPathStatusInserters_, moduleRegistry(),
          proc_pset, tns, prealloc, preg, branchIDListHelper, actions, areg, processConfiguration,
          !hasSubprocesses, StreamID{i}, processContext));
    }
    ...
    globalSchedule_ = std::make_unique<GlobalSchedule>(
        resultsInserter(), pathStatusInserters_, endPathStatusInserters_, moduleRegistry(),
        modulesToUse, proc_pset, preg, prealloc, actions, areg, processConfiguration,
        processContext);
    ...
    if (wantSummary_) {
      ...
      std::transform(
          workers.begin(), workers.end(), std::back_inserter(modDesc),
          [](const Worker* iWorker) -> const ModuleDescription* { return iWorker->descPtr(); });

      // propagate_const<T> has no reset() function
      summaryTimeKeeper_ = std::make_unique<SystemTimeKeeper>(prealloc.numberOfStreams(), modDesc,
                                                              tns, processContext);
      ...
      // areg->preModuleEventSignal_.connect([timeKeeperPtr](StreamContext const& iContext,
      // ModuleCallingContext const& iMod) {
  ...
  void Schedule::endJob(ExceptionCollector& collector) {
    ...
    if (wantSummary_ == false) return;
    {
      ...
      for (auto const& p : tr.trigPathSummaries) {
        LogVerbatim("FwkSummary") << "TrigReport " << std::right << std::setw(5) << 1 << std::right
                                  << std::setw(5) << p.bitPosition << " " << std::right
                                  << std::setw(10) << p.timesRun << " " << std::right
                                  << std::setw(10) << p.timesPassed << " " << std::right
                                  << std::setw(10) << p.timesFailed << " " << std::right
                                  << std::setw(10) << p.timesExcept << " " << p.name << "";
      }
      ...
      for (auto const& p : tr.endPathSummaries) {
        LogVerbatim("FwkSummary") << "TrigReport " << std::right << std::setw(5) << 0 << std::right
                                  << std::setw(5) << p.bitPosition << " " << std::right
                                  << std::setw(10) << p.timesRun << " " << std::right
                                  << std::setw(10) << p.timesPassed << " " << std::right
                                  << std::setw(10) << p.timesFailed << " " << std::right
      ...
      for (auto const& worker : tr.workerSummaries) {
        LogVerbatim("FwkSummary") << "TrigReport " << std::right << std::setw(10)
                                  << worker.timesVisited << " " << std::right << std::setw(10)
                                  << worker.timesRun << " " << std::right << std::setw(10)
                                  << worker.timesPassed << " " << std::right << std::setw(10)
                                  << worker.timesFailed << " " << std::right << std::setw(10)
    ...
    LogVerbatim("FwkSummary") << "TimeReport" << std::setprecision(6) << std::fixed
                              << "      event loop Real/event = "
                              << tr.eventSummary.realTime / totalEvents;
    LogVerbatim("FwkSummary") << "TimeReport" << std::setprecision(6) << std::fixed
                              << "     sum Streams Real/event = "
    ...
    LogVerbatim("FwkSummary") << "TimeReport " << std::right << std::setw(kColumn1Size)
                              << "per event"
    ...
    for (auto const& p : tr.trigPathSummaries) {
      ...
      for (auto const& mod : p.moduleInPathSummaries) {
        LogVerbatim("FwkSummary") << "TimeReport " << std::setprecision(6) << std::fixed
                                  << std::right << std::setw(kColumn1Size)
                                  << mod.realTime / totalEvents << " " << std::right
                                  << std::setw(kColumn2Size)
    ...
    for (auto const& worker : tr.workerSummaries) {
      LogVerbatim("FwkSummary") << "TimeReport " << std::setprecision(6) << std::fixed << std::right
                                ...
                                << worker.realTime / std::max(1, worker.timesRun) << " "
  ...
  void Schedule::processOneEventAsync(WaitingTaskHolder iTask, unsigned int iStreamID,
  ...
  bool Schedule::changeModule(std::string const& iLabel, ParameterSet const& iPSet,
