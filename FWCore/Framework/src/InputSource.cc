...
namespace edm {
  ...
  bool InputSource::readEvent(EventPrincipal& ep, EventID const& eventID, StreamContext& streamContext) {
    ...
    if (not limitReached()) {
      ...
      // result = callWithTryCatchAndPrint<bool>( [this,&eventID,&ep](){ return readIt(eventID, ep); }, "Calling
      // InputSource::readIt" );
  ...
  void InputSource::issueReports(EventID const& eventID) {
    if (isInfoEnabled()) {
      LogVerbatim("FwkReport") << "Begin processing the " << readCount_ << suffix(readCount_) << " record. Run "
                               ...
                               << eventID.luminosityBlock() << " at " << std::setprecision(3) << TimeOfDay();
    }
  ...
  void InputSource::doEndRun(RunPrincipal& rp, bool cleaningUpAfterException, ProcessContext const*) {
    Run run(rp, moduleDescription(), nullptr);
  ...
  void InputSource::doEndLumi(LuminosityBlockPrincipal& lbp, bool cleaningUpAfterException, ProcessContext const*) {
    LuminosityBlock lb(lbp, moduleDescription(), nullptr);
    callWithTryCatchAndPrint<void>([this, &lb]() { endLuminosityBlock(lb); }, "Calling InputSource::endLuminosityBlock",
                                   cleaningUpAfterException);
    lb.commit_(std::vector<edm::ProductResolverIndex>());
  }
