...
namespace edm {
  GlobalSchedule::GlobalSchedule(
      ...
      std::shared_ptr<ModuleRegistry> modReg, std::vector<std::string> const& iModulesToUse,
    ...
    for (auto const& moduleLabel : iModulesToUse) {
      ...
      if (modpset != nullptr) {  // It will be null for PathStatusInserters, it should
        ...
        addToAllWorkers(workerManager_.getWorker(*modpset, pregistry, &prealloc,
                                                 processConfiguration, moduleLabel));
      }
    }
    if (inserter) {
