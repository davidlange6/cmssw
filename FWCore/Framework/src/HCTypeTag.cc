...
// moved type lookup used by EventSetup to FWCore/Utilities to avoid unneeded external dependencies
// from FWCore/Framework
//
...
namespace edm {
  namespace eventsetup {
    namespace heterocontainer {
      ...
      //
      HCTypeTag HCTypeTag::findType(const std::string& iTypeName) {
      ...
      HCTypeTag HCTypeTag::findType(const char* iTypeName) {
        ...
        // need to take name from the 'findType' since that address is guaranteed to be long
        // lived
