...
namespace edm {
  ...
  public:
    ...
    GlobalSchedule(
        ...
        ParameterSet& proc_pset, ProductRegistry& pregistry,
        PreallocationConfiguration const& prealloc, ExceptionToActionTable const& actions,
  ...
  private:
    ...
    public:
      ...
      ~SendTerminationSignalIfException() {
        if (reg_) {
          reg_->preGlobalEarlyTerminationSignal_(*context_,
  ...
  void GlobalSchedule::processOneGlobalAsync(WaitingTaskHolder iHolder, typename T::MyPrincipal& ep,
    ...
    auto doneTask = make_waiting_task(
        tbb::task::allocate_root(), [this, iHolder, cleaningUpAfterException, globalContext,
    ...
    for (auto& worker : boost::adaptors::reverse((allWorkers()))) {
      worker->doWorkAsync<T>(doneTask, ep, es, StreamID::invalidStreamID(), parentContext,
                             globalContext.get());
    }
  }
}
