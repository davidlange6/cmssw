...
namespace edm {
  namespace eventsetup {
    ...
    bool EventSetupRecord::add(const DataKey& iKey, const DataProxy* iProxy) {
      ...
      if (0 != proxy) {
        ...
        if (proxy->providerDescription()->isSource_ == iProxy->providerDescription()->isSource_) {
          // should lookup to see if there is a specified 'chosen' one and only if not, throw
          // the exception
          throw cms::Exception("EventSetupConflict")
              ...
              << "Please either\n   remove one of these "
              << (proxy->providerDescription()->isSource_ ? "Sources" : "Producers")
              << "\n   or find a way of configuring one of them so it does not deliver this "
                 "data"
    ...
    edm::eventsetup::ComponentDescription const* EventSetupRecord::providerDescription(
        const DataKey& aKey) const {
      const DataProxy* proxy = find(aKey);
      if (0 != proxy) {
    ...
    void EventSetupRecord::fillRegisteredDataKeys(std::vector<DataKey>& oToFill) const {
      ...
      for (std::map<DataKey, const DataProxy*>::const_iterator it = proxies_.begin(),
                                                               itEnd = proxies_.end();
           it != itEnd; ++it) {
      ...
      if (iDesc && iTag.module().size()) {
        ...
        if (!matched) {
          throw cms::Exception("EventSetupWrongModule")
              ...
              << "  dataLabel = '" << iTag.data() << "'\n"
              << "but the data matching the C++ class type and dataLabel comes from module "
                 "type="
    ...
    void EventSetupRecord::addTraceInfoToCmsException(cms::Exception& iException, const char* iName,
      ...
      ost << "Using EventSetup component " << iDescription->type_ << "/'" << iDescription->label_
          << "' to make data " << iKey.type().name() << "/'" << iName << "' in record "
          << this->key().type().name();
