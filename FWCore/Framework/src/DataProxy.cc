...
namespace edm {
  namespace eventsetup {
    ...
    const void* DataProxy::get(const EventSetupRecord& iRecord, const DataKey& iKey,
      ...
      // We need to set the AccessType for each request so this can't be called in the if block
      // above.
      // This also must be before the cache_ check since we want to setCacheIsValid before a
      // possible
      // exception throw. If we don't, 'getImpl' will be called again on a second request for the
      // data.
      if (!iTransiently) {
