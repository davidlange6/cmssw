...
namespace edm {
  ...
  void OutputModule::keepThisBranch(
    ...
    if (!checkDictionary(missingDictionaries, desc.className(), desc.unwrappedType())) {
      std::string context(
          "Calling OutputModule::keepThisBranch, checking dictionaries for kept types");
      ...
      *branchIDLists_ = *origBranchIDLists_;
      ...
      for (BranchIDList& branchIDList : *branchIDLists_) {
        for (BranchID::value_type& branchID : branchIDList) {
          // Replace BranchID of each dropped branch with that of the kept alias, so the alias
          // branch will have the product ID of the original branch.
