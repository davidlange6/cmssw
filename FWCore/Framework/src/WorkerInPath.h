...
namespace edm {
  ...
  public:
    ...
    template <typename T>
    void runWorkerAsync(WaitingTask* iTask, typename T::MyPrincipal const&, EventSetup const&,
                        StreamID streamID, typename T::Context const* context);
  ...
  template <typename T>
  void WorkerInPath::runWorkerAsync(WaitingTask* iTask, typename T::MyPrincipal const& ep,
