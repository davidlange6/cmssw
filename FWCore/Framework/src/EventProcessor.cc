...
namespace {
  ...
  private:
    edm::ActivityRegistry*
        reg_;  // We do not use propagate_const because the registry itself is mutable.
  };
}
...
namespace edm {
  ...
  std::unique_ptr<InputSource> makeInput(
      ParameterSet& params, CommonParams const& common, std::shared_ptr<ProductRegistry> preg,
    ...
    if (main_input == nullptr) {
      throw Exception(errors::Configuration)
          ...
          << "It is missing (or there are sufficient syntax errors such that it is not "
    ...
    try {
      ...
      std::shared_ptr<int> sentry(nullptr,
                                  [areg, &md](void*) { areg->postSourceConstructionSignal_(md); });
  ...
  EventProcessor::EventProcessor(std::string const& config, ServiceToken const& iToken,
      ...
      : actReg_(),
        ...
        sourceResourcesAcquirer_(
            SharedResourcesRegistry::instance()->createAcquirerForSourceDelayedReader().first),
        sourceMutex_(
  ...
  EventProcessor::EventProcessor(std::shared_ptr<ProcessDesc> processDesc,
                                 ServiceToken const& token, serviceregistry::ServiceLegacy legacy)
      : actReg_(),
        preg_(),
        branchIDListHelper_(),
        serviceToken_(),
  ...
  void EventProcessor::init(std::shared_ptr<ProcessDesc>& processDesc, ServiceToken const& iToken,
    ...
    // If there are subprocesses, pop the subprocess parameter sets out of the process parameter
    // set
    ...
    forceESCacheClearOnNewRun_ =
        optionsPset.getUntrackedParameter<bool>("forceEventSetupCacheClearOnNewRun", false);
    // threading
    ...
    */
    ...
    preallocations_ =
        PreallocationConfiguration{nThreads, nStreams, nConcurrentLumis, nConcurrentRuns};

    // initialize the input source
    input_ = makeInput(*parameterSet, *common, items.preg(), items.branchIDListHelper(),
                       items.thinnedAssociationsHelper(), items.actReg_,
                       items.processConfiguration(), preallocations_);

    // intialize the Schedule
    schedule_ =
        items.initSchedule(*parameterSet, hasSubProcesses, preallocations_, &processContext_);
    ...
    for (unsigned int index = 0; index < preallocations_.numberOfStreams(); ++index) {
      ...
      auto ep = std::make_shared<EventPrincipal>(
          preg(), branchIDListHelper(), thinnedAssociationsHelper(), *processConfiguration_,
    ...
    for (auto& subProcessPSet : subProcessVParameterSet) {
      subProcesses_.emplace_back(
          subProcessPSet, *parameterSet, preg(), branchIDListHelper(), *thinnedAssociationsHelper_,
          SubProcessParentageHelper(), *espController_, *actReg_, token,
          serviceregistry::kConfigurationOverrides, preallocations_, &processContext_);
    }
  }
  ...
  void EventProcessor::endJob() {
    ...
    ExceptionCollector c(
        "Multiple exceptions were thrown while executing endJob. An exception message follows "
        "for each.\n");
...
#include "TransitionProcessors.icc"
  ...
  void EventProcessor::respondToCloseInputFile() {
    if (fb_.get() != nullptr) {
      ...
      for_all(subProcesses_,
              [this](auto& subProcess) { subProcess.respondToCloseInputFile(*fb_); });
    }
  ...
  void EventProcessor::beginRun(ProcessHistoryID const& phid, RunNumber_t run) {
    ...
    }
    {
      ...
      beginGlobalTransitionAsync<Traits>(WaitingTaskHolder(globalWaitTask.get()), *schedule_,
    ...
    }
    {
      ...
      beginStreamsTransitionAsync<Traits>(streamLoopWaitTask.get(), *schedule_,
                                          preallocations_.numberOfStreams(), runPrincipal, ts, es,
                                          subProcesses_);
  ...
  void EventProcessor::endRun(ProcessHistoryID const& phid, RunNumber_t run,
    ...
    IOVSyncValue ts(EventID(runPrincipal.run(), LuminosityBlockID::maxLuminosityBlockNumber(),
    ...
    EventSetup const& es = esp_->eventSetup();
    {
      ...
      endStreamsTransitionAsync<Traits>(streamLoopWaitTask.get(), *schedule_,
                                        preallocations_.numberOfStreams(), runPrincipal, ts, es,
    ...
    }
    {
      ...
      endGlobalTransitionAsync<Traits>(WaitingTaskHolder(globalWaitTask.get()), *schedule_,
                                       runPrincipal, ts, es, subProcesses_,
  ...
  void EventProcessor::beginLumi(ProcessHistoryID const& phid, RunNumber_t run,
    ...
    IOVSyncValue ts(EventID(lumiPrincipal.run(), lumiPrincipal.luminosityBlock(), 0),
    ...
    }
    {
      ...
      beginStreamsTransitionAsync<Traits>(streamLoopWaitTask.get(), *schedule_,
                                          preallocations_.numberOfStreams(), lumiPrincipal, ts, es,
                                          subProcesses_);
      streamLoopWaitTask->wait_for_all();
      if (streamLoopWaitTask->exceptionPtr() != nullptr) {
        std::rethrow_exception(*(streamLoopWaitTask->exceptionPtr()));
  ...
  void EventProcessor::endLumi(ProcessHistoryID const& phid, RunNumber_t run,
    ...
    // lumi blocks know their start and end times why not also start and end events?
    IOVSyncValue ts(
        EventID(lumiPrincipal.run(), lumiPrincipal.luminosityBlock(), EventID::maxEventNumber()),
  ...
  int EventProcessor::readLuminosityBlock() {
    ...
    auto lbp = std::make_shared<LuminosityBlockPrincipal>(input_->luminosityBlockAuxiliary(),
  ...
  void EventProcessor::deleteRunFromCache(ProcessHistoryID const& phid, RunNumber_t run) {
    ...
    for_all(subProcesses_,
            [run, phid](auto& subProcess) { subProcess.deleteRunFromCache(phid, run); });
  ...
  void EventProcessor::writeLumi(ProcessHistoryID const& phid, RunNumber_t run,
    ...
    for_all(subProcesses_,
            [&phid, run, lumi](auto& subProcess) { subProcess.writeLumi(phid, run, lumi); });
  ...
  InputSource::ItemType EventProcessor::readAndProcessEvents() {
    ...
    for (; iStreamIndex < kNumStreams - 1; ++iStreamIndex) {
      ...
      tbb::task::enqueue(*make_waiting_task(
          tbb::task::allocate_root(), [this, iStreamIndex, finishedProcessingEventsPtr,
  ...
  void EventProcessor::processEventAsyncImpl(WaitingTaskHolder iHolder, unsigned int iStreamIndex) {
    ...
    WaitingTaskHolder finalizeEventTask(make_waiting_task(
        tbb::task::allocate_root(), [this, pep, iHolder](std::exception_ptr const* iPtr) mutable {
          ServiceRegistry::Operate operate(serviceToken_);
    ...
    schedule_->processOneEventAsync(std::move(afterProcessTask), iStreamIndex, *pep,
                                    esp_->eventSetup());
  }
