...
unsigned int EDConsumerBase::recordConsumes(BranchType iBranch, TypeToGet const& iType,
                                            edm::InputTag const& iTag, bool iAlwaysGets) {
  if (frozen_) {
  ...
  m_tokenInfo.emplace_back(
      TokenLookupInfo{iType.type(), ProductResolverIndexInvalid, skipCurrentProcess, iBranch},
      iAlwaysGets, LabelPlacement{labelStart, delta1, delta2}, iType.kind());

  const size_t additionalSize = skipCurrentProcess
    ...
    if (info.m_branchType == iBranchType &&
      ...
      for (unsigned int j = 0; j != matches.numberOfMatches(); ++j) {
        ...
        if (matches.isFullyResolved(j)) {
          ...
          m_tokenInfo.emplace_back(
              TokenLookupInfo{info.m_type, index, info.m_index.skipCurrentProcess(),
                              info.m_branchType},
...
void EDConsumerBase::itemsToGet(BranchType iBranch,
  ...
  unsigned int count = 0;
  {
    ...
    for (auto it = m_tokenInfo.begin<kLookupInfo>(), itEnd = m_tokenInfo.end<kLookupInfo>();
...
bool EDConsumerBase::registeredToConsume(ProductResolverIndex iIndex, bool skipCurrentProcess,
  ...
  for (auto it = m_tokenInfo.begin<kLookupInfo>(), itEnd = m_tokenInfo.end<kLookupInfo>();
    ...
    if (it->m_index.productResolverIndex() == iIndex and
        it->m_index.skipCurrentProcess() == skipCurrentProcess and it->m_branchType == iBranch) {
      return true;
    }
  }
  ...
  nonConstThis->m_tokenInfo.emplace_back(
      TokenLookupInfo{TypeID{}, iIndex, skipCurrentProcess, iBranch}, true, LabelPlacement{0, 0, 0},
      PRODUCT_TYPE);
...
void EDConsumerBase::throwBadToken(edm::TypeID const& iType, EDGetToken iToken) const {
  if (iToken.isUninitialized()) {
    throw cms::Exception("BadToken")
        ...
        << "' was made using an uninitialized token.\n Please check that the variable is being "
           "initialized from a 'consumes' call.";
  }
  throw cms::Exception("BadToken")
      << "A get using a EDGetToken with the C++ type '" << iType.className()
      ...
      << " which is beyond the range used by this module.\n Please check that the variable is "
         "being initialized from a 'consumes' call from this module.\n You can not share "
         "EDGetToken values between modules.";
}
    ...
    if (itInfo->m_branchType == InEvent and (not itInfo->m_index.skipCurrentProcess())) {
      ...
      if (*consumedModuleLabel != '\0') {    // not a consumesMany
        if (*consumedProcessName != '\0') {  // process name is specified in consumes call
          ...
          auto matches =
              iHelper.relatedIndexes(*itKind, itInfo->m_type, consumedModuleLabel,
                                     consumedModuleLabel + itLabels->m_deltaToProductInstance);
          for (unsigned int j = 0; j < matches.numberOfMatches(); ++j) {
            if (processName == matches.processName(j)) {
              insertFoundModuleLabel(consumedModuleLabel, modules, alreadyFound, labelsToDesc,
                                     preg);
            }
          }
        }
        ...
        for (unsigned int j = 0; j < matches.numberOfMatches(); ++j) {
          if (processName == matches.processName(j)) {
            insertFoundModuleLabel(matches.moduleLabel(j), modules, alreadyFound, labelsToDesc,
                                   preg);
          }
        }
      }
...
std::vector<ConsumesInfo> EDConsumerBase::consumesInfo() const {
  ...
  for (auto itInfo = m_tokenInfo.begin<kLookupInfo>(), itEnd = m_tokenInfo.end<kLookupInfo>();
    ...
    result.emplace_back(itInfo->m_type, consumedModuleLabel, consumedInstance, consumedProcessName,
                        itInfo->m_branchType, *itKind, *itAlways,
                        itInfo->m_index.skipCurrentProcess());
  }
  return result;
}
