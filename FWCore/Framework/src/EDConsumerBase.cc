...
bool EDConsumerBase::registeredToConsume(ProductResolverIndex iIndex, bool skipCurrentProcess,
  ...
  nonConstThis->m_tokenInfo.emplace_back(TokenLookupInfo{TypeID{}, iIndex, skipCurrentProcess, iBranch}, true,
...
bool EDConsumerBase::registeredToConsumeMany(TypeID const& iType, BranchType iBranch) const {
  ...
  // non thread-safe
  EDConsumerBase* nonConstThis = const_cast<EDConsumerBase*>(this);
  nonConstThis->m_tokenInfo.emplace_back(TokenLookupInfo{iType, ProductResolverIndexInvalid, false, iBranch}, true,
                                         LabelPlacement{0, 0, 0}, PRODUCT_TYPE);
  return false;
}
