...
namespace edm {
  namespace eventsetup {
        ...
        for (std::vector<std::string>::iterator itRecordName = recordNames.begin(),
          ...
          try {
            ...
            for (std::vector<std::string>::iterator itDatum = dataInfo.begin(),
              ...
              recordToData.insert(
                  std::make_pair(std::string(*itRecordName), std::make_pair(datumName, labelName)));
            }
    ...
    // ---------------------------------------------------------------
    void fillEventSetupProvider(EventSetupsController& esController, EventSetupProvider& cp,
                                ParameterSet& params) {
