...
namespace edm {
  namespace eventsetup {
    ...
    //
    EventSetupProvider::EventSetupProvider(unsigned subProcessIndex,
        ...
        : eventSetup_(),
          ...
          referencedDataKeys_(
              new std::map<EventSetupRecordKey, std::map<DataKey, ComponentDescription const*> >),
          recordToFinders_(
              new std::map<EventSetupRecordKey,
                           std::vector<std::shared_ptr<EventSetupRecordIntervalFinder> > >),
          psetIDToRecordKey_(new std::map<ParameterSetIDHolder, std::set<EventSetupRecordKey> >),
          recordToPreferred_(
              new std::map<EventSetupRecordKey, std::map<DataKey, ComponentDescription> >),
    ...
    typedef std::map<EventSetupRecordKey, EventSetupRecordProvider::DataToPreferredProviderMap>
        RecordToPreferred;
    ...
    static void preferEverything(const ComponentDescription& iComponent,
                                 const Providers& iProviders, RecordToPreferred& iReturnValue) {
        ...
        if (components.find(iComponent) != components.end()) {
          ...
          for (std::set<EventSetupRecordKey>::iterator itRecord = records.begin(),
            ...
            const DataProxyProvider::KeyedProxies& keyedProxies =
                proxyProv->keyedProxies(*itRecord);
              ...
              for (DataProxyProvider::KeyedProxies::const_iterator itProxy = keyedProxies.begin(),
                ...
                if (itFind != dataToProviderMap.end()) {
                  throw cms::Exception("ESPreferConflict")
                      ...
                      << "\""
                      << "\n the providers are "
                      << "\n 1) type=" << itFind->second.type_ << " label=\""
                      << itFind->second.label_ << "\""
    ...
    static RecordToPreferred determinePreferred(
        const EventSetupProvider::PreferredProviderInfo* iInfo, const Providers& iProviders) {
      ...
      if (0 != iInfo) {
        for (EventSetupProvider::PreferredProviderInfo::const_iterator itInfo = iInfo->begin(),
          ...
          if (itInfo->second.empty()) {
            ...
            for (EventSetupProvider::RecordToDataMap::const_iterator
                     itRecData = itInfo->second.begin(),
                     itRecDataEnd = itInfo->second.end();
                 itRecData != itRecDataEnd; ++itRecData) {
              std::string recordName = itRecData->first;
              EventSetupRecordKey recordKey(
              ...
              std::set<ComponentDescription> components =
                  itRecordProvider->second->proxyProviderDescriptions();
              ...
              eventsetup::TypeTag datumType =
                  eventsetup::TypeTag::findType(itRecData->second.first);
              if (datumType == eventsetup::TypeTag()) {
                // not found
                throw cms::Exception("ESPreferWrongDataType")
                    << "The es_prefer statement for type=" << itInfo->first.type_ << " label=\""
                    << itInfo->first.label_ << "\" has the unknown data type \""
                    << itRecData->second.first << "\""
                    << "\n Please check spelling";
              }
              ...
              const DataProxyProvider::KeyedProxies& keyedProxies =
                  proxyProv->keyedProxies(recordKey);
              if (std::find_if(keyedProxies.begin(), keyedProxies.end(),
                ...
                throw cms::Exception("ESPreferWrongData")
                    ...
                    << "  type=\"" << itRecData->second.first << "\" label=\""
                    << itRecData->second.second << "\""
    ...
    void EventSetupProvider::finishConfiguration() {
      // we delayed adding finders to the system till here so that everything would be loaded
      // first
      recordToFinders_->clear();
      for (std::vector<std::shared_ptr<EventSetupRecordIntervalFinder> >::iterator
               itFinder = finders_->begin(),
               itEnd = finders_->end();
        ...
        for (Keys::const_iterator itKey = recordsUsing.begin(), itKeyEnd = recordsUsing.end();
          ...
          if (providers_.end() == itFound) {
            ...
            insert(*itKey,
                   EventSetupRecordProviderFactoryManager::instance().makeRecordProvider(*itKey));
      ...
      // Now handle providers since sources can also be finders and the sources can delay
      // registering
      // their Records and therefore could delay setting up their Proxies
      psetIDToRecordKey_->clear();
      typedef std::set<EventSetupRecordKey> Keys;
      for (std::vector<std::shared_ptr<DataProxyProvider> >::iterator
               itProvider = dataProviders_->begin(),
      ...
      *recordToPreferred_ = determinePreferred(preferredProviderInfo_.get(), providers_);
      ...
      for (Providers::iterator itProvider = providers_.begin(), itProviderEnd = providers_.end();
        ...
        RecordToPreferred::const_iterator itRecordFound =
          ...
          if (!foundAllProviders) {
            edm::LogInfo("EventSetupDependency")
                ...
                << ") which is not present in the job."
                   ...
                   "If no exception occurs during the job than it is usually safe to ignore "
                   "this message.";
    ...
    static void findDependents(
        const EventSetupRecordKey& iKey, Itr itBegin, Itr itEnd,
      ...
      for (Itr it = itBegin; it != itEnd; ++it) {
        ...
        if (deps.end() != deps.find(iKey)) {
          ...
          // now see who is dependent on this record since they will be indirectly dependent on
          // iKey
    ...
    void EventSetupProvider::forceCacheClear() {
      for (Providers::iterator it = providers_.begin(), itEnd = providers_.end(); it != itEnd;
           ++it) {
    ...
    void EventSetupProvider::checkESProducerSharing(
      ...
      edm::LogVerbatim("EventSetupSharing")
          << "EventSetupProvider::checkESProducerSharing: Checking processes with SubProcess "
             "Indexes "
      ...
      for (auto const& iRecord : referencedESProducers) {
        for (auto const& iComponent : iRecord.second) {
          ...
          esController.lookForMatches(psetID, subProcessIndex_,
          ...
          if (iCandidateNotRejectedYet->second == true) {
            ...
            if (iAllComponentsMatch == allComponentsMatch.end()) {
              ...
              // now.
              bool match = doRecordsMatch(precedingESProvider, iRecord.first, allComponentsMatch,
                                          esController);
      ...
      for (auto const& candidate : candidateNotRejectedYet) {
        ...
        if (canBeShared) {
          ...
          ParameterSetIDHolder psetIDHolder(psetID);
          sharingCheckDone.insert(psetIDHolder);
          if (esController.isFirstMatch(psetID, subProcessIndex_,
                                        precedingESProvider.subProcessIndex_)) {
          ...
          std::set<EventSetupRecordKey> const& keysForPSetID1 =
              (*precedingESProvider.psetIDToRecordKey_)[psetIDHolder];
          for (auto const& key : keysForPSetID1) {
            std::shared_ptr<EventSetupRecordProvider> const& recordProvider =
                precedingESProvider.providers_[key];
          ...
          if (esController.isLastMatch(psetID, subProcessIndex_,
                                       precedingESProvider.subProcessIndex_)) {
            ParameterSet const& pset = *esController.getESProducerPSet(psetID, subProcessIndex_);
    ...
    bool EventSetupProvider::doRecordsMatch(EventSetupProvider& precedingESProvider,
      ...
      for (auto const& finder : (*recordToFinders_)[eventSetupRecordKey]) {
        ...
        bool itMatches = esController.isMatchingESSource(psetID, subProcessIndex_,
                                                         precedingESProvider.subProcessIndex_);
        if (!itMatches) {
          return false;
        }
      ...
      std::map<DataKey, ComponentDescription const*> const& dataItems =
      ...
      if (itFound != providers_.end()) {
        ...
        for (auto const& dependentRecord : dependentRecords) {
          ...
          bool match = doRecordsMatch(precedingESProvider, dependentRecord, allComponentsMatch,
                                      esController);
    ...
    void EventSetupProvider::fillReferencedDataKeys(
        EventSetupRecordKey const& eventSetupRecordKey) {
    ...
    //
    EventSetup const& EventSetupProvider::eventSetupForInstance(const IOVSyncValue& iValue) {
