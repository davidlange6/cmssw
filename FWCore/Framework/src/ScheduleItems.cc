...
namespace edm {
  ...
  ServiceToken ScheduleItems::initServices(std::vector<ParameterSet>& pServiceSets,
                                           ParameterSet& parameterSet, ServiceToken const& iToken,
  ...
  ServiceToken ScheduleItems::addCPRandTNS(ParameterSet const& parameterSet,
    ...
    ServiceToken tempToken(
        ServiceRegistry::createContaining(reg, token, serviceregistry::kOverlapIsError));
  ...
  std::unique_ptr<Schedule> ScheduleItems::initSchedule(ParameterSet& parameterSet,
    ...
    return std::make_unique<Schedule>(
        parameterSet, ServiceRegistry::instance().get<service::TriggerNamesService>(), *preg_,
        *branchIDListHelper_, *thinnedAssociationsHelper_,
        subProcessParentageHelper_ ? subProcessParentageHelper_.get() : nullptr, *act_table_,
        actReg_, processConfiguration(), hasSubprocesses, config, processContext);
  }
