...
namespace {
  struct StackGuard {
    StackGuard(std::string const& iTypeName, std::map<std::string, unsigned int>& ioStack,
               bool iFromListener)
...
void SignallingProductRegistry::addCalled(BranchDescription const& iProd, bool iFromListener) {
  ...
  if (guard.numType_ > 2) {
    throw cms::Exception("CircularReference")
        << "Attempted to register the production of " << iProd.className() << " from module "
        << iProd.moduleLabel() << " with product instance \"" << iProd.productInstanceName()
        << "\"\n"
