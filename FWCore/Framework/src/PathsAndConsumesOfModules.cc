...
namespace edm {
  ...
  void PathsAndConsumesOfModules::initialize(Schedule const* schedule,
    ...
    schedule->fillModuleAndConsumesInfo(allModuleDescriptions_, moduleIDToIndex_,
                                        modulesWhoseProductsAreConsumedBy_, *preg);
  }
  ...
  std::vector<ModuleDescription const*> const& PathsAndConsumesOfModules::doModulesOnPath(
  ...
  void checkForModuleDependencyCorrectness(edm::PathsAndConsumesOfModulesBase const& iPnC,
        ...
        for(auto const& depDescription: dependentModules) {
          ...
          if(itFind != modulesOnlyOnEndPaths.end()) {
            throw edm::Exception(edm::errors::ScheduleExecutionFailure, "Unrunnable schedule\n")
              <<"The module "<<moduleIndexToNames[moduleIndex]<<" is on a Path and depends on data
    from module "
        ...
        if (labelToID == pathStatusInserterModuleLabelToModuleID.end()) {
          ...
          throw Exception(errors::LogicError)
              << "PathsAndConsumesOfModules::moduleDescription:"
                 "checkForModuleDependencyCorrectness Could not find PathStatusInserter\n";
        }
        unsigned int pathStatusInserterModuleID = labelToID->second;
        if (pathIndex < kFirstEndPathIndex) {
          if ((lastModuleIndex != kInvalidIndex)) {
            edgeToPathMap[std::make_pair(pathStatusInserterModuleID, lastModuleIndex)].push_back(
                pathIndex);
            moduleIndexToNames.insert(std::make_pair(pathStatusInserterModuleID, kPathEnded));
            if (kTriggerResultsIndex != kInvalidIndex) {
              edgeToPathMap[std::make_pair(kTriggerResultsIndex, pathStatusInserterModuleID)]
                  .push_back(kDataDependencyIndex);
            }
            // Need to make dependency for finished process
            edgeToPathMap[std::make_pair(kFinishedProcessingIndex, pathStatusInserterModuleID)]
                .push_back(kDataDependencyIndex);
          ...
          if ((not moduleDescriptions->empty())) {
            if (kTriggerResultsIndex != kInvalidIndex) {
              ...
              edgeToPathMap[std::make_pair(moduleDescriptions->front()->id(),
                                           kPathToTriggerResultsDependencyLastIndex)]
                  .push_back(pathIndex);
              moduleIndexToNames.insert(
                  std::make_pair(kPathToTriggerResultsDependencyLastIndex, kEndPathStart));
              edgeToPathMap[std::make_pair(kPathToTriggerResultsDependencyLastIndex,
    ...
    }
    {
      ...
      for (auto const& description : iPnC.allModules()) {
        ...
        for (auto const& depDescription : dependentModules) {
          ...
          // see if all paths containing this module also contain the dependent module earlier
          // in the path
          // if it does, then treat this only as a path dependency and not a data dependency as
          // this
    ...
    graph::throwIfImproperDependencies(edgeToPathMap, pathIndexToModuleIndexOrder, pathNames,
                                       moduleIndexToNames);
  }
}
