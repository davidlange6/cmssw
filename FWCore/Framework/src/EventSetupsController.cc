  ...
  namespace eventsetup {
    ...
    std::shared_ptr<EventSetupProvider> EventSetupsController::makeProvider(ParameterSet& iPSet) {
      ...
      std::shared_ptr<EventSetupProvider> returnValue(
          makeEventSetupProvider(iPSet, providers_.size()));
    ...
    void EventSetupsController::eventSetupForInstance(IOVSyncValue const& syncValue) {
      ...
      std::for_each(providers_.begin(), providers_.end(),
                    [&syncValue](std::shared_ptr<EventSetupProvider> const& esp) {
    ...
    void EventSetupsController::putESProducer(ParameterSet const& pset,
      ...
      auto newElement = esproducers_.insert(
          std::pair<ParameterSetID, ESProducerInfo>(pset.id(), ESProducerInfo(&pset, component)));
      // Register processes with an exact match
      newElement->second.subProcessIndexes().push_back(subProcessIndex);
    }

    std::shared_ptr<EventSetupRecordIntervalFinder>
    EventSetupsController::getESSourceAndRegisterProcess(ParameterSet const& pset,
                                                         unsigned subProcessIndex) {
    ...
    void EventSetupsController::putESSource(
        ParameterSet const& pset, std::shared_ptr<EventSetupRecordIntervalFinder> const& component,
        unsigned subProcessIndex) {
      auto newElement = essources_.insert(
          std::pair<ParameterSetID, ESSourceInfo>(pset.id(), ESSourceInfo(&pset, component)));
      // Register processes with an exact match
      newElement->second.subProcessIndexes().push_back(subProcessIndex);
    }
    ...
    bool EventSetupsController::isFirstMatch(ParameterSetID const& psetID, unsigned subProcessIndex,
    ...
    bool EventSetupsController::isLastMatch(ParameterSetID const& psetID, unsigned subProcessIndex,
                                            unsigned precedingProcessIndex) const {
      auto elements = esproducers_.equal_range(psetID);
      for (auto it = elements.first; it != elements.second; ++it) {
        std::vector<unsigned> const& subProcessIndexes = it->second.subProcessIndexes();
    ...
    void EventSetupsController::checkESProducerSharing() {
      ...
      for (; esProvider != esProviderEnd; ++esProvider) {
        ...
        // SubProcess's)
        for (auto precedingESProvider = providers_.begin(); precedingESProvider != esProvider;
             ++precedingESProvider) {
          (*esProvider)
              ->checkESProducerSharing(**precedingESProvider, sharingCheckDone,
                                       referencedESProducers, *this);
        }
