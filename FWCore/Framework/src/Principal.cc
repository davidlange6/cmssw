...
namespace edm {
  ...
  static void throwProductNotFoundException(char const* where, errors::ErrorCodes error,
                                            BranchID const& bid) {
  ...
  static std::shared_ptr<cms::Exception> makeNotFoundException(
      char const* where, KindOfType kindOfType, TypeID const& productType, std::string const& label,
  ...
  static void throwAmbiguousException(const char* where, TypeID const& productType,
  ...
  namespace {
    ...
    void failedToRegisterConsumes(KindOfType kindOfType, TypeID const& productType,
      ...
      exception << "::getByLabel without corresponding call to consumes or mayConsumes for this "
                   "module.\n"
  ...
  Principal::Principal(std::shared_ptr<ProductRegistry const> reg,
    ...
    // The constructor of an alias product holder takes as an argument the product holder for
    ...
    std::vector<ProductResolverIndex> matchingHolders(lookupProcessNames.size(),
  ...
  void Principal::setupUnscheduled(UnscheduledConfigurator const& iConfigure) {
    applyToResolvers(
        [&iConfigure](ProductResolverBase* iResolver) { iResolver->setupUnscheduled(iConfigure); });
  }
  ...
  void Principal::fillPrincipal(ProcessHistoryID const& hist,
    ...
    if (historyAppender_ && productRegistry().anyProductProduced()) {
      ...
      if ((not processHistoryPtr_) || (processHistoryIDBeforeConfig_ != hist)) {
        if (hist.isValid()) {
          ...
          inputProcessHistory =
              std::shared_ptr<ProcessHistory const>(&s_emptyProcessHistory, [](void const*) {});
        }
    ...
    if (orderProcessHistoryID_ != processHistoryID_) {
      ...
      // transient.
      auto nameIter = std::find(lookupProcessNames.begin(), lookupProcessNames.end(),
  ...
  ProductResolverBase const* Principal::getExistingProduct(
    ...
    if (nullptr != phb &&
        BranchKey(productResolver.branchDescription()) != BranchKey(phb->branchDescription())) {
      ...
      if (newProduct.branchName() != existing.branchName() &&
        ...
        throw cms::Exception("HashCollision")
            ...
            << " Branch " << newProduct.branchName() << " has same branch ID as branch "
            << existing.branchName() << "\n"
  ...
  BasicHandle Principal::getByLabel(KindOfType kindOfType, TypeID const& typeID,
                                    ...
                                    ModuleCallingContext const* mcc) const {
    ProductData const* result =
        findProductByLabel(kindOfType, typeID, label, instance, process, consumer, sra, mcc);
    if (result == nullptr) {
      return BasicHandle(makeHandleExceptionFactory([=]() -> std::shared_ptr<cms::Exception> {
  ...
  BasicHandle Principal::getByToken(KindOfType, TypeID const&, ProductResolverIndex index,
  ...
  void Principal::prefetchAsync(WaitingTask* task, ProductResolverIndex index,
                                bool skipCurrentProcess, ModuleCallingContext const* mcc) const {
  ...
  void Principal::findProducts(std::vector<ProductResolverBase const*> const& holders,
                               TypeID const&, BasicHandleVec& results, SharedResourcesAcquirer* sra,
                               ModuleCallingContext const* mcc) const {
    for (auto iter = processHistoryPtr_->rbegin(), iEnd = processHistoryPtr_->rend(); iter != iEnd;
         ++iter) {
  ...
  ProductData const* Principal::findProductByLabel(KindOfType kindOfType, TypeID const& typeID,
    ...
    if (unlikely(consumer and
                 (not consumer->registeredToConsume(index, skipCurrentProcess, branchType())))) {
  ...
  ProductData const* Principal::findProductByLabel(
      ...
      std::string const& instance, std::string const& process, EDConsumerBase const* consumer,
      ...
      for (auto const& prod : preg_->productList()) {
        ...
        if (bd.branchType() == branchType_) {
          ...
          if (!productResolvers_[index]) {
            // no product holder.  Must add one. The new entry must be an input product
            // holder.
            assert(!bd.produced());
            auto cbd = std::make_shared<BranchDescription const>(bd);
            addInputProduct(cbd);
