...
namespace edm {
  ...
  ModuleDescription Maker::createModuleDescription(MakeModuleParams const& p) const {
    ...
    ModuleDescription md(conf.id(), conf.getParameter<std::string>("@module_type"),
                         ...
                         p.processConfiguration_.get(), ModuleDescription::getUniqueID());
    return md;
  }

  void Maker::throwValidationException(MakeModuleParams const& p,
  ...
  void Maker::throwConfigurationException(ModuleDescription const& md,
                                          cms::Exception& iException) const {
    std::ostringstream ost;
  ...
  std::shared_ptr<maker::ModuleHolder> Maker::makeModule(
      MakeModuleParams const& p, signalslot::Signal<void(ModuleDescription const&)>& pre,
