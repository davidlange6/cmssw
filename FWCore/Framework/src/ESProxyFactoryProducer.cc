...
namespace edm {
  ...
  //
  // const ESProxyFactoryProducer& ESProxyFactoryProducer::operator=(const ESProxyFactoryProducer&
  // rhs)
  // {
  ...
  void ESProxyFactoryProducer::registerFactoryWithKey(const EventSetupRecordKey& iRecord,
    ...
    if (range.second != std::find_if(range.first, range.second, [&info](const auto& r2f) {
