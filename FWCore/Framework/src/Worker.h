...
namespace edm {
  ...
  public:
    ...
    template <typename T>
    void doWorkAsync(WaitingTask* task, typename T::MyPrincipal const&, EventSetup const& c,
                     StreamID stream, ParentContext const& parentContext,
                     typename T::Context const* context);

    template <typename T>
    void doWorkNoPrefetchingAsync(WaitingTask* task, typename T::MyPrincipal const&,
                                  EventSetup const& c, StreamID stream,
                                  ParentContext const& parentContext,
                                  typename T::Context const* context);

    template <typename T>
    std::exception_ptr runModuleDirectly(typename T::MyPrincipal const& ep, EventSetup const& es,
  ...
  protected:
    ...
    virtual bool implDoBegin(LuminosityBlockPrincipal const& lbp, EventSetup const& c,
                             ModuleCallingContext const* mcc) = 0;
    virtual bool implDoStreamBegin(StreamID id, LuminosityBlockPrincipal const& lbp,
                                   EventSetup const& c, ModuleCallingContext const* mcc) = 0;
  ...
  private:
    ...
    bool shouldRethrowException(std::exception_ptr iPtr, ParentContext const& parentContext,
                                bool isEvent, TransitionIDValueBase const& iID) const;
    ...
    void emitPostModuleEventPrefetchingSignal() {
      actReg_->postModuleEventPrefetchingSignal_.emit(*moduleCallingContext_.getStreamContext(),
    ...
    public:
      RunModuleTask(Worker* worker, typename T::MyPrincipal const& ep, EventSetup const& es,
                    StreamID streamID, ParentContext const& parentContext,
                    typename T::Context const* context)
          : m_worker(worker),
            m_principal(ep),
      ...
      tbb::task* execute() override {
        ...
        if (not excptr) {
          if (auto queue = m_worker->serializeRunModule()) {
            ...
            queue.push(
                [worker, &principal, &es, streamID, parentContext, sContext, serviceToken]() {
        ...
        m_worker->runModuleAfterAsyncPrefetch<T>(excptr, m_principal, m_es, m_streamID,
                                                 m_parentContext, m_context);
        return nullptr;
      }
  ...
  namespace {
    ...
    private:
      ActivityRegistry*
          a_;  // We do not use propagate_const because the registry itself is mutable.
  ...
  namespace workerhelper {
    ...
    public:
      ...
      static bool call(Worker* iWorker, StreamID, LuminosityBlockPrincipal const& ep,
                       EventSetup const& es, ActivityRegistry* actReg,
                       ModuleCallingContext const* mcc, Arg::Context const* context) {
        ModuleSignalSentry<Arg> cpp(actReg, context, mcc);
        return iWorker->implDoBegin(ep, es, mcc);
      }
    ...
    public:
      ...
      static bool call(Worker* iWorker, StreamID id, LuminosityBlockPrincipal const& ep,
                       EventSetup const& es, ActivityRegistry* actReg,
                       ModuleCallingContext const* mcc, Arg::Context const* context) {
        ModuleSignalSentry<Arg> cpp(actReg, context, mcc);
        return iWorker->implDoStreamBegin(id, ep, es, mcc);
  ...
  void Worker::doWorkAsync(WaitingTask* task, typename T::MyPrincipal const& ep,
    ...
    if (workStarted_.compare_exchange_strong(expected, true)) {
      ...
      // if have TriggerResults based selection we want to reject the event before doing
      // prefetching
      try {
        if (not workerhelper::CallImpl<T>::prePrefetchSelection(this, streamID, ep,
                                                                &moduleCallingContext_)) {
  ...
  bool Worker::doWork(typename T::MyPrincipal const& ep, EventSetup const& es, StreamID streamID,
    ...
    if (T::isEvent_) {
      ...
      auto waitTask = edm::make_empty_waiting_task();
      {
        ...
        std::unique_ptr<ActivityRegistry, decltype(sentryFunc)> signalSentry(actReg_.get(),
                                                                             sentryFunc);
  ...
  bool Worker::runModule(typename T::MyPrincipal const& ep, EventSetup const& es, StreamID streamID,
    ...
    try {
      convertException::wrap([&]() {
        rc = workerhelper::CallImpl<T>::call(this, streamID, ep, es, actReg_.get(),
  ...
  std::exception_ptr Worker::runModuleDirectly(typename T::MyPrincipal const& ep,
    ...
    return runModuleAfterAsyncPrefetch<T>(prefetchingException, ep, es, streamID, parentContext,
                                          context);
  }
}
#endif
