...
namespace edm {
  ...
  public:
    ...
    virtual void resolvePutIndicies(
        BranchType iBranchType, std::unordered_multimap<std::string, edm::ProductResolverIndex> const& iIndicies) = 0;
  ...
  namespace workerhelper {
    ...
    public:
      ...
      static bool prePrefetchSelection(Worker* iWorker, StreamID id, typename Arg::MyPrincipal const& ep,
  ...
  void Worker::doWorkAsync(WaitingTask* task, typename T::MyPrincipal const& ep, EventSetup const& es,
    ...
    if (workStarted_.compare_exchange_strong(expected, true)) {
      ...
      auto runTask = new (tbb::task::allocate_root()) RunModuleTask<T>(this, ep, es, streamID, parentContext, context);
