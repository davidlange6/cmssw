...
namespace edm {
  ...
  public:
    ...
    StreamSchedule(std::shared_ptr<TriggerResultInserter> inserter,
                   ...
                   std::shared_ptr<ModuleRegistry>, ParameterSet& proc_pset, service::TriggerNamesService const& tns,
                   PreallocationConfiguration const& prealloc, ProductRegistry& pregistry,
                   BranchIDListHelper& branchIDListHelper, ExceptionToActionTable const& actions,
    ...
    template <typename T>
    void processOneStreamAsync(WaitingTaskHolder iTask, typename T::MyPrincipal& principal,
    ...
    void moduleDescriptionsInPath(std::string const& iPathLabel, std::vector<ModuleDescription const*>& descriptions,
                                  unsigned int hint) const;

    void moduleDescriptionsInEndPath(std::string const& iEndPathLabel,
                                     std::vector<ModuleDescription const*>& descriptions, unsigned int hint) const;
  ...
  private:
    ...
    void fillWorkers(ParameterSet& proc_pset, ProductRegistry& preg, PreallocationConfiguration const* prealloc,
                     std::shared_ptr<ProcessConfiguration const> processConfiguration, std::string const& name,
                     bool ignoreFilters, PathWorkers& out, std::vector<std::string> const& endPathNames);
    void fillTrigPath(ParameterSet& proc_pset, ProductRegistry& preg, PreallocationConfiguration const* prealloc,
                      std::shared_ptr<ProcessConfiguration const> processConfiguration, int bitpos,
                      std::string const& name, TrigResPtr, std::vector<std::string> const& endPathNames);
    void fillEndPath(ParameterSet& proc_pset, ProductRegistry& preg, PreallocationConfiguration const* prealloc,
                     std::shared_ptr<ProcessConfiguration const> processConfiguration, int bitpos,
    ...
    void initializeEarlyDelete(ModuleRegistry& modReg, edm::ParameterSet const& opts, edm::ProductRegistry const& preg,
  ...
  template <typename T>
  void StreamSchedule::processOneStreamAsync(WaitingTaskHolder iHolder, typename T::MyPrincipal& ep,
    ...
    auto task = make_functor_task(
        tbb::task::allocate_root(), [this, doneTask, &ep, &es, cleaningUpAfterException, token]() mutable {
          ServiceRegistry::Operate op(token);
          ...
          workerManager_.processOneOccurrenceAsync<T>(doneTask, ep, es, streamID_, &streamContext_, &streamContext_);
        });
