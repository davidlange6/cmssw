...
namespace edm {
  ...
  public:
    ...
    StreamSchedule(
        ...
        std::shared_ptr<ModuleRegistry>, ParameterSet& proc_pset,
        service::TriggerNamesService const& tns, PreallocationConfiguration const& prealloc,
        ProductRegistry& pregistry, BranchIDListHelper& branchIDListHelper,
  ...
  private:
    ...
    public:
      SendTerminationSignalIfException(edm::ActivityRegistry* iReg,
                                       edm::StreamContext const* iContext)
          : reg_(iReg), context_(iContext) {}
      ~SendTerminationSignalIfException() {
        if (reg_) {
          reg_->preStreamEarlyTerminationSignal_(*context_,
    ...
    void finishedPaths(std::exception_ptr, WaitingTaskHolder, EventPrincipal& ep,
                       EventSetup const& es);
    std::exception_ptr finishProcessOneEvent(std::exception_ptr);

    void reportSkipped(EventPrincipal const& ep) const;

    void fillWorkers(ParameterSet& proc_pset, ProductRegistry& preg,
                     PreallocationConfiguration const* prealloc,
                     std::shared_ptr<ProcessConfiguration const> processConfiguration,
                     std::string const& name, bool ignoreFilters, PathWorkers& out,
                     std::vector<std::string> const& endPathNames);
    void fillTrigPath(ParameterSet& proc_pset, ProductRegistry& preg,
                      PreallocationConfiguration const* prealloc,
                      std::shared_ptr<ProcessConfiguration const> processConfiguration, int bitpos,
                      std::string const& name, TrigResPtr,
                      std::vector<std::string> const& endPathNames);
    void fillEndPath(ParameterSet& proc_pset, ProductRegistry& preg,
                     PreallocationConfiguration const* prealloc,
                     std::shared_ptr<ProcessConfiguration const> processConfiguration, int bitpos,
    ...
    void initializeEarlyDelete(ModuleRegistry& modReg, edm::ParameterSet const& opts,
  ...
  void StreamSchedule::processOneStreamAsync(WaitingTaskHolder iHolder, typename T::MyPrincipal& ep,
    ...
    auto doneTask = make_waiting_task(
        tbb::task::allocate_root(), [this, iHolder, id, cleaningUpAfterException,
    ...
    auto task =
        make_functor_task(tbb::task::allocate_root(),
                          [this, doneTask, &ep, &es, cleaningUpAfterException, token]() mutable {
                            ServiceRegistry::Operate op(token);
                            ...
                            workerManager_.processOneOccurrenceAsync<T>(
                                doneTask, ep, es, streamID_, &streamContext_, &streamContext_);
                          });
