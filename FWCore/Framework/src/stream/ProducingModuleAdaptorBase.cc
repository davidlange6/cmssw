  ...
  namespace stream {
    ...
    void ProducingModuleAdaptorBase<T>::registerProductsAndCallbacks(
      ...
      if (firstMod->registrationCallback() and m_streamModules.size() > 1) {
        // we have a callback so we will collect all callbacks and create a new callback which
        // calls them all.
    ...
    void ProducingModuleAdaptorBase<T>::itemsToGet(
        BranchType iType, std::vector<ProductResolverIndexAndSkipBit>& iIndices) const {
    ...
    void ProducingModuleAdaptorBase<T>::modulesWhoseProductsAreConsumed(
      ...
      assert(not m_streamModules.empty());
      return m_streamModules[0]->modulesWhoseProductsAreConsumed(modules, preg, labelsToDesc,
                                                                 processName);
    }
    ...
    template <typename T>
    void ProducingModuleAdaptorBase<T>::doStreamBeginRun(StreamID id, RunPrincipal const& rp,
                                                         EventSetup const& c,
    ...
    template <typename T>
    void ProducingModuleAdaptorBase<T>::doStreamEndRun(StreamID id, RunPrincipal const& rp,
                                                       EventSetup const& c,
                                                       ModuleCallingContext const* mcc) {
      auto mod = m_streamModules[id];
      Run r(rp, moduleDescription_, mcc);
