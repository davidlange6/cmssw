...
namespace edm {
  ...
  template <>
  inline bool WorkerT<edm::limited::OutputModuleBase>::implDoPrePrefetchSelection(
      StreamID id, EventPrincipal const& ep, ModuleCallingContext const* mcc) {
    return module_->prePrefetchSelection(id, ep, mcc);
  }
  ...
  inline bool WorkerT<T>::implDoStreamEnd(StreamID id, RunPrincipal const& rp, EventSetup const& c,
                                          ModuleCallingContext const* mcc) {
    std::conditional_t<workerimpl::has_stream_functions<T>::value,
                       workerimpl::DoStreamEndTrans<T, RunPrincipal const>, workerimpl::DoNothing>
        might_call;
    might_call(this, id, rp, c, mcc);
    return true;
  }
  ...
  template <typename T>
  template <typename D>
  void WorkerT<T>::callWorkerStreamBegin(D, StreamID id, LuminosityBlockPrincipal const& rp,
                                         EventSetup const& c, ModuleCallingContext const* mcc) {
  ...
  template <typename T>
  template <typename D>
  void WorkerT<T>::callWorkerStreamEnd(D, StreamID id, LuminosityBlockPrincipal const& rp,
                                       EventSetup const& c, ModuleCallingContext const* mcc) {
    module_->doStreamEndLuminosityBlock(id, rp, c, mcc);
  }

  template <typename T>
  inline bool WorkerT<T>::implDoStreamBegin(StreamID id, LuminosityBlockPrincipal const& lbp,
                                            EventSetup const& c, ModuleCallingContext const* mcc) {
    std::conditional_t<workerimpl::has_stream_functions<T>::value,
  ...
  template <typename T>
  inline bool WorkerT<T>::implDoStreamEnd(StreamID id, LuminosityBlockPrincipal const& lbp,
                                          EventSetup const& c, ModuleCallingContext const* mcc) {
    std::conditional_t<workerimpl::has_stream_functions<T>::value,
  ...
  inline void WorkerT<T>::implBeginStream(StreamID id) {
    std::conditional_t<workerimpl::has_stream_functions<T>::value, workerimpl::DoBeginStream<T>,
                       workerimpl::DoNothing>
        might_call;
    might_call(this, id);
  }
  ...
  inline void WorkerT<T>::implEndStream(StreamID id) {
    std::conditional_t<workerimpl::has_stream_functions<T>::value, workerimpl::DoEndStream<T>,
                       workerimpl::DoNothing>
        might_call;
    might_call(this, id);
  }
