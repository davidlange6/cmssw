...
namespace edmtest {
  ...
  void OtherThingAnalyzer::doit(edm::Event const& dv, std::string const& label) {
    ...
    for (OtherThingCollection::const_iterator it = otherThings->begin(), itEnd = otherThings->end();
      ...
      if (thingWasDropped_) return;
      {
        ...
        if (tc.a == i && x == i) {
          edm::LogInfo("OtherThingAnalyzer") << " ITEM " << i << " LABEL " << label
        ...
        for (edm::RefVector<ThingCollection>::iterator it = otherThing.refVec.begin(),
      ...
      }
      {
        ...
        for (edm::PtrVector<Thing>::const_iterator it = otherThing.ptrVec.begin(),
  ...
  void OtherThingAnalyzer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
    ...
    desc.addUntracked<bool>("thingWasDropped", false)
        ->setComment(
            "true if the ref to the ThingCollection in the OtherThingCollection can not be "
            "resolved since the ThingCollection was not stored.");
