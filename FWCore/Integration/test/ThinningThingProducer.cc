...
namespace edmtest {
  ...
  public:
    ...
    void preChoose(edm::Handle<edmtest::ThingCollection> tc, edm::Event const& event,
  ...
  ThinningThingSelector::ThinningThingSelector(edm::ParameterSet const& pset,
    ...
    trackToken_ =
        cc.consumes<TrackOfThingsCollection>(pset.getParameter<edm::InputTag>("trackTag"));
  ...
  bool ThinningThingSelector::choose(unsigned int iIndex, edmtest::Thing const& iItem) {
    ...
    // Just using %10 because it coincidentally works with the arbitrary numbers I picked, no
    // meaning really.
...
typedef edm::ThinningProducer<edmtest::ThingCollection, edmtest::ThinningThingSelector>
