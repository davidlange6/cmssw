...
namespace edmtest {
  ...
  MissingDictionaryTestProducer::MissingDictionaryTestProducer(edm::ParameterSet const& pset) {
    ...
    inputToken2_ = consumes<std::vector<MissingDictionaryTestA> >(
        pset.getParameter<edm::InputTag>("inputTag"));
    inputToken3_ =
        consumes<std::list<MissingDictionaryTestA> >(pset.getParameter<edm::InputTag>("inputTag"));
