...
namespace edmtest {
  ...
  public:
    TableTestAnalyzer(edm::ParameterSet const& iConfig)
      ...
      tableToken_ =
          consumes<edmtest::TableTest>(iConfig.getUntrackedParameter<edm::InputTag>("table"));
    ...
    void analyze(edm::StreamID, edm::Event const& iEvent, edm::EventSetup const&) const final {
      ...
      for (auto const& row : *h) {
        if (anInts_[index] != row.get<edmtest::AnInt>()) {
          throw cms::Exception("RuntimeError")
              << "index " << index << " anInt =" << row.get<edmtest::AnInt>() << " expected "
              << anInts_[index];
        }
        if (aFloats_[index] != row.get<edmtest::AFloat>()) {
          throw cms::Exception("RuntimeError")
              << "index " << index << " aFloat =" << row.get<edmtest::AFloat>() << " expected "
              << aFloats_[index];
        }
        if (aStrings_[index] != row.get<edmtest::AString>()) {
          throw cms::Exception("RuntimeError")
              << "index " << index << " aString =" << row.get<edmtest::AString>() << " expected "
              << aStrings_[index];
        }
        ++index;
      }
