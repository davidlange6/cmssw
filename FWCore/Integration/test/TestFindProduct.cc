  ...
  TestFindProduct::TestFindProduct(edm::ParameterSet const& pset)
      : inputTags_(pset.getUntrackedParameter<std::vector<edm::InputTag> >("inputTags")),
        ...
        runProducerParameterCheck_(
            pset.getUntrackedParameter<bool>("runProducerParameterCheck", false)) {
    ...
    inputTagsView_ =
        pset.getUntrackedParameter<std::vector<edm::InputTag> >("inputTagsView", emptyTagVector);
    ...
    for (std::vector<edm::InputTag>::const_iterator iter = inputTags_.begin(),
      ...
      if (runProducerParameterCheck_) {
        ...
        if (par != expectedParameterValue) {
          std::cerr << "TestFindProduct::analyze unexpected value from producer parameter set"
                    << std::endl;
          abort();
        }
      }
