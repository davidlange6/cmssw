...
namespace edmtest {
  ...
  ThinningTestAnalyzer::ThinningTestAnalyzer(edm::ParameterSet const& pset) {
    ...
    associationToken_ =
        mayConsume<edm::ThinnedAssociation>(pset.getParameter<edm::InputTag>("associationTag"));
    ...
    if (!associationShouldBeDropped_) {
      expectedIndexesIntoParent_ =
          pset.getParameter<std::vector<unsigned int> >("expectedIndexesIntoParent");
    }
  ...
  void ThinningTestAnalyzer::analyze(edm::Event const& event, edm::EventSetup const&) {
    ...
    if (parentWasDropped_) {
      ...
      for (auto const& thing : *parentCollection) {
        ...
        int expected = static_cast<int>(expectedParentContent_.at(i) +
    ...
    if (thinnedWasDropped_) {
      ...
      for (auto const& thing : *thinnedCollection) {
        if (thing.a != static_cast<int>(expectedThinnedContent_.at(expectedIndex) +
                                        event.eventAuxiliary().event() * 100 + 100)) {
