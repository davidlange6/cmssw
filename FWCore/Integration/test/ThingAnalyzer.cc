...
namespace edmtest {
  struct Empty {};
  class ThingAnalyzer
      : public edm::global::EDAnalyzer<edm::RunCache<Empty>, edm::LuminosityBlockCache<Empty>> {
  public:
    ThingAnalyzer(edm::ParameterSet const&);

    void analyze(edm::StreamID, edm::Event const&, edm::EventSetup const&) const override final;
    std::shared_ptr<Empty> globalBeginRun(edm::Run const&,
  ...
  std::shared_ptr<Empty> ThingAnalyzer::globalBeginRun(edm::Run const& iRun,
                                                       edm::EventSetup const&) const {
    edm::Handle<ThingCollection> h;
