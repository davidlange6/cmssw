...
namespace edm {
  DaqProvenanceHelper::DaqProvenanceHelper(TypeID const& rawDataType)
    ...
    // We don't currently use the untracked parameters, However, we make them available, just in
    // case.
  ...
  ProcessHistoryID DaqProvenanceHelper::daqInit(
      ProductRegistry& productRegistry, ProcessHistoryRegistry& processHistoryRegistry) const {
    ...
    ProcessHistory ph;
    ph.emplace_back(constBranchDescription_.processName(), processParameterSet_.id(),
  ...
  void DaqProvenanceHelper::fixMetaData(std::vector<ProcessConfiguration>& pcv,
    ...
    phv.push_back(
        ProcessHistory());  // For new processHistory, containing only processConfiguration_
    std::vector<ProcessConfiguration> newPCs;
    for (auto const& pc : pcv) {
      if (pc.processName() == oldProcessName_) {
        newPCs.emplace_back(constBranchDescription_.processName(), processParameterSet_.id(),
