...
namespace edm {
  ...
  ProducerSourceBase::ProducerSourceBase(ParameterSet const& pset,
      ...
      : InputSource(pset, desc),
        numberEventsInRun_(
            pset.getUntrackedParameter<unsigned int>("numberEventsInRun", remainingEvents())),
        numberEventsInLumi_(pset.getUntrackedParameter<unsigned int>(
            "numberEventsInLuminosityBlock", remainingEvents())),
        presentTime_(
            pset.getUntrackedParameter<unsigned long long>("firstTime", 1ULL)),  // time in ns
    ...
    // std::string eType = pset.getUntrackedParameter<std::string>("experimentType",
    // std::string("Any"))),
  }
  ...
  std::shared_ptr<LuminosityBlockAuxiliary> ProducerSourceBase::readLuminosityBlockAuxiliary_() {
    ...
    Timestamp ts = Timestamp(presentTime_);
    resetNewLumi();
    return std::make_shared<LuminosityBlockAuxiliary>(eventID_.run(), eventID_.luminosityBlock(),
                                                      ts, Timestamp::invalidTimestamp());
  }
  ...
  void ProducerSourceBase::retreatToPrevious(EventID& eventID, TimeValue_t& time) {
    if (numberEventsInRun_ < 1 || numberEventsInThisRun_ > 0) {
      ...
      eventID = eventID.previousRunLastEvent(origEventID_.luminosityBlock() +
