...
namespace edm {
  EventSkipperByID::EventSkipperByID(ParameterSet const& pset)
      :  // The default value provided as the second argument to the getUntrackedParameter function
        ...
        // in the description.  As soon as all primary input sources and all modules with a
        // secondary
        ...
        whichEventsToProcess_(pset.getUntrackedParameter<std::vector<EventRange> >(
  ...
  // Determines whether a run, lumi, or event will be skipped based on the run, lumi, and event
  // number.
  // This function is called by a predicate, so it must not modify the state of the
  // EventSkipperByID_ object.
  ...
  bool EventSkipperByID::skipIt(RunNumber_t run, LuminosityBlockNumber_t lumi,
    ...
    if (run == firstRun_) {
      ...
      if (lumi < firstLumi_) {
        // This lumi or event entry is for a lumi prior to the first lumi to be processed.  Skip
        // it.
        return true;
      }
    ...
    if (skippingLumis()) {
      // If we get here, the entry was not skipped due to firstRun, firstLuminosityBlock, and/or
      // firstEvent.
  ...
  void EventSkipperByID::fillDescription(ParameterSetDescription& desc) {
    ...
    desc.addNode(edm::ParameterDescription<unsigned int>("firstEvent", 1U, false) xor
        ...
        ->setComment(
            ...
            "If 'firstLuminosityBlock' == 0, skip any event in run 'firstRun' with event number "
            "< 'firstEvent'.\n"
    ...
    desc.addUntracked<std::vector<EventRange> >("eventsToProcess", defaultEvents)
        ->setComment(
            "If not empty, skip any event outside the specified run:event or run:lumi:event "
