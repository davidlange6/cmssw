  ...
  namespace service {
    ...
    PrescaleService::PrescaleService(ParameterSet const& iPS, ActivityRegistry& iReg)
        : forceDefault_(iPS.getParameter<bool>("forceDefault")),
          ...
          lvl1Default_(
              findDefaultIndex(iPS.getParameter<std::string>("lvl1DefaultLabel"), lvl1Labels_)),
      ...
      if (lvl1Default_ >= lvl1Labels_.size()) {
        throw cms::Exception("InvalidLvl1Index")
            << "lvl1Default_ '" << lvl1Default_ << "' exceeds number of prescale columns "
            << lvl1Labels_.size() << "!";
      }
      ...
      for (unsigned int iVPSet = 0; iVPSet < vpsetPrescales_.size(); ++iVPSet) {
        ...
        if (prescales.size() != lvl1Labels_.size()) {
          throw cms::Exception("PrescaleServiceConfigError")
              << " Path '" << pathName << "' has " << prescales.size()
              << " prescales, instead of expected " << lvl1Labels_.size() << "!";
        }
    ...
    void PrescaleService::postBeginJob() {
      ...
      const std::vector<std::string> allModules =
          prcPS.getParameter<std::vector<std::string> >("@all_modules");
      ...
      const std::vector<std::string> allPaths =
          prcPS.getParameter<std::vector<std::string> >("@paths");
          ...
          if (module2path.find(moduleLabel) != module2path.end()) {
            ...
            if (module2path[moduleLabel] == "") {
              ...
              throw cms::Exception("PrescaleServiceConfigError")
                  ...
                  << "' on (>1) Paths: " << module2path[moduleLabel] << "+" << pathName << "!";
            }
          }
        }
      }
      // Check all HLTPrescaler instances are on a path
      for (std::map<std::string, std::string>::const_iterator it = module2path.begin();
           it != module2path.end(); ++it) {
        if (it->second == "") {
          throw cms::Exception("PrescaleServiceConfigError")
      ...
      for (std::map<std::string, std::vector<unsigned int> >::const_iterator it =
               prescaleTable_.begin();
           it != prescaleTable_.end(); ++it) {
    ...
    // static method
    unsigned int PrescaleService::findDefaultIndex(std::string const& label,
