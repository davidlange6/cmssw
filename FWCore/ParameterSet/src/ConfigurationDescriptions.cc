...
namespace {
  void matchLabel(std::pair<std::string, edm::ParameterSetDescription> const& thePair,
                  std::string const& moduleLabel, edm::ParameterSetDescription const*& psetDesc) {
...
namespace edm {
    ...
    if (0 == strcmp(baseType_.c_str(), kSource)) {
      if (0 != strcmp(label.c_str(), k_source)) {
        throw edm::Exception(edm::errors::LogicError,
                             "ConfigurationDescriptions::add, when adding a "
                             "ParameterSetDescription for a source the label must be "
                             "\"source\"\n");
      }
      if (descriptions_.size() != 0U || defaultDescDefined_ == true) {
        throw edm::Exception(edm::errors::LogicError,
                             "ConfigurationDescriptions::add, for a source only 1 "
                             "ParameterSetDescription may be added\n");
      }
    } else if (0 == strcmp(baseType_.c_str(), kService)) {
      if (descriptions_.size() != 0U || defaultDescDefined_ == true) {
        throw edm::Exception(edm::errors::LogicError,
                             "ConfigurationDescriptions::add, for a service only 1 "
                             "ParameterSetDescription may be added\n");
      }
    }
  ...
  void ConfigurationDescriptions::addDefault(ParameterSetDescription const& psetDescription) {
    if (0 == strcmp(baseType_.c_str(), kSource) || 0 == strcmp(baseType_.c_str(), kService)) {
      if (descriptions_.size() != 0U || defaultDescDefined_ == true) {
        throw edm::Exception(edm::errors::LogicError,
                             "ConfigurationDescriptions::addDefault, for a source or service "
                             "only 1 ParameterSetDescription may be added\n");
      }
    }
  ...
  ConfigurationDescriptions::iterator ConfigurationDescriptions::begin() {
    return descriptions_.begin();
  }

  ConfigurationDescriptions::iterator ConfigurationDescriptions::end() {
  ...
  void ConfigurationDescriptions::validate(ParameterSet& pset,
    ...
    for_all(descriptions_, std::bind(&matchLabel, std::placeholders::_1, std::cref(moduleLabel),
                                     std::ref(psetDesc)));
  ...
  void ConfigurationDescriptions::writeCfis(std::string const& baseType,
    ...
    for_all(descriptions_,
            std::bind(&ConfigurationDescriptions::writeCfiForLabel, std::placeholders::_1,
                      std::cref(baseType), std::cref(pluginName), std::ref(usedCfiFileNames)));
  }

  void ConfigurationDescriptions::writeCfiForLabel(
    ...
    if (0 == strcmp(baseType.c_str(), kService) && labelAndDesc.first != pluginName) {
      throw edm::Exception(edm::errors::LogicError,
                           ...
                           "label and the plugin name must be the same.\n")
          << "This error is probably caused by an incorrect label being passed\nto the "
             "ConfigurationDescriptions::add function earlier.\n"
    ...
    if (!usedCfiFileNames.insert(cfi_filename).second) {
      ...
      ex << "The cfi file name is '" << cfi_filename << "' and\n"
         ...
         << "This error is probably caused by an error in one or more fillDescriptions "
            "functions\n"
         << "where duplicate module labels are being passed to the "
            "ConfigurationDescriptions::add\n"
         ...
         << "share the same fillDescriptions function. Fix these by specializing the "
            "fillDescriptions\n"
         << "function for each template instantiation.\n"
         << "3. This can also happen when there is an inheritance heirarchy and multiple plugin "
            "modules\n"
         << "are defined using derived classes and the base class which share the same "
            "fillDescriptions\n"
         << "function. Fix these by redefining the fillDescriptions function in each derived "
            "class.\n";
  ...
  void ConfigurationDescriptions::print(std::ostream& os, std::string const& moduleLabel,
                                        bool brief, bool printOnlyLabels, size_t lineWidth,
      ...
      if (defaultDescDefined_) {
        ...
        if (descriptions_.size() == 1U) {
          ...
          ss << "This plugin has " << descriptions_.size() << " PSet descriptions. "
             << "The description used to validate a configuration is selected by "
             << "matching the module labels. If none match the first description below is "
                "used. "
    ...
    if (defaultDescDefined_) {
      printForLabel(os, std::string("@default"), defaultDesc_, moduleLabel, brief, printOnlyLabels,
                    lineWidth, indentation, counter);
    }
  }

  void ConfigurationDescriptions::printForLabel(
      std::pair<std::string, ParameterSetDescription> const& labelAndDesc, std::ostream& os,
      std::string const& moduleLabel, bool brief, bool printOnlyLabels, size_t lineWidth,
      int indentation, DescriptionCounter& counter) const {
    printForLabel(os, labelAndDesc.first, labelAndDesc.second, moduleLabel, brief, printOnlyLabels,
                  lineWidth, indentation, counter);
  }
  ...
  void ConfigurationDescriptions::printForLabel(std::ostream& os, std::string const& label,
                                                ...
                                                bool printOnlyLabels, size_t lineWidth,
                                                int indentation,
    ...
    if (!brief) {
      if (!description.comment().empty()) {
        DocFormatHelper::wrapAndPrintText(os, description.comment(), indentation,
