...
namespace edm {
  ...
  VParameterSetEntry::VParameterSetEntry(std::vector<ParameterSet> const& vpset, bool isTracked)
    ...
    for (std::vector<ParameterSet>::const_iterator i = vpset.begin(), e = vpset.end(); i != e;
         ++i) {
  ...
  void VParameterSetEntry::toString(std::string& result) const {
    ...
    for (std::vector<ParameterSetID>::const_iterator i = theIDs_->begin(), e = theIDs_->end();
         i != e; ++i) {
      result += start;
      i->toString(result);
      start = between;
  ...
  void VParameterSetEntry::registerPsetsAndUpdateIDs() {
    ...
    for (std::vector<ParameterSet>::iterator i = theVPSet_->begin(), e = theVPSet_->end(); i != e;
         ++i) {
      if (!i->isRegistered()) {
        i->registerIt();
      }
