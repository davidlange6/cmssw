...
namespace edm {
  ...
  namespace {
    void insertIntoReplace(ParameterSetConverter::StringMap& replace, std::string const& fromPrefix,
                           std::string const& from, std::string const& fromPostfix,
  ...
  ParameterSetConverter::ParameterSetConverter(ParameterSetMap const& psetMap,
    ...
    for (ParameterSetMap::const_iterator i = psetMap.begin(), iEnd = psetMap.end(); i != iEnd;
         ++i) {
    ...
    for (std::vector<MainParameterSet>::iterator j = mainParameterSets_.begin(),
         ...
         j != jEnd; ++j) {
      for (std::vector<TriggerPath>::iterator i = triggerPaths_.begin(), iEnd = triggerPaths_.end();
           i != iEnd; ++i) {
  ...
  void ParameterSetConverter::noConvertParameterSets() {
    for (StringWithIDList::iterator i = parameterSets_.begin(), iEnd = parameterSets_.end();
         i != iEnd; ++i) {
