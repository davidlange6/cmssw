...
namespace edm {
  ...
  void XORGroupDescription::checkAndGetLabelsAndTypes_(
  ...
  void XORGroupDescription::validate_(ParameterSet& pset, std::set<std::string>& validatedLabels,
                                      bool optional) const {
  ...
  void XORGroupDescription::writeCfi_(std::ostream& os, bool& startWithComma, int indentation,
                                      bool& wroteSomething) const {
    node_left_->writeCfi(os, startWithComma, indentation, wroteSomething);
  }

  void XORGroupDescription::print_(std::ostream& os, bool optional, bool writeToCfi,
  ...
  void XORGroupDescription::printNestedContent_(std::ostream& os, bool optional,
                                                DocFormatHelper& dfh) const {
    if (dfh.parent() == DocFormatHelper::XOR) {
      dfh.decrementCounter();
    ...
    printSpaces(os, indentation);
    if (optional) {
      os << "This optional XOR group requires exactly one or none of the following to be in the "
            "PSet\n";
    } else {
  ...
  bool XORGroupDescription::exists_(ParameterSet const& pset) const {
    int nTotal =
        node_left_->howManyXORSubNodesExist(pset) + node_right_->howManyXORSubNodesExist(pset);
    return nTotal == 1;
  }

  bool XORGroupDescription::partiallyExists_(ParameterSet const& pset) const {
    return exists(pset);
  }
