...
namespace edm {
  ...
  void ORGroupDescription::checkAndGetLabelsAndTypes_(
  ...
  void ORGroupDescription::validate_(ParameterSet& pset, std::set<std::string>& validatedLabels,
                                     bool optional) const {
  ...
  void ORGroupDescription::writeCfi_(std::ostream& os, bool& startWithComma, int indentation,
                                     bool& wroteSomething) const {
    node_left_->writeCfi(os, startWithComma, indentation, wroteSomething);
  }

  void ORGroupDescription::print_(std::ostream& os, bool optional, bool writeToCfi,
  ...
  void ORGroupDescription::printNestedContent_(std::ostream& os, bool optional,
                                               DocFormatHelper& dfh) const {
    if (dfh.parent() == DocFormatHelper::OR) {
      dfh.decrementCounter();
    ...
    if (optional) {
      ...
      os << "This optional OR group requires at least one or none of the following to be in the "
            "PSet\n";
    } else {
  ...
  int ORGroupDescription::howManyXORSubNodesExist_(ParameterSet const& pset) const {
  ...
  void ORGroupDescription::throwIfDuplicateLabels(std::set<std::string> const& labelsLeft,
    ...
    std::set_intersection(labelsLeft.begin(), labelsLeft.end(), labelsRight.begin(),
                          labelsRight.end(), insertIter);
  ...
  void ORGroupDescription::throwIfDuplicateTypes(std::set<ParameterTypes> const& types1,
    ...
    if (!types1.empty()) {
      ...
      std::insert_iterator<std::set<ParameterTypes> > insertIter(duplicateTypes,
                                                                 duplicateTypes.begin());
