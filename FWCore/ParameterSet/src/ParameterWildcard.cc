...
namespace edm {
  ...
  void ParameterWildcard<ParameterSetDescription>::validate_(ParameterSet& pset,
    ...
    if (psetDesc_) {
      for_all(parameterNames,
              std::bind(&ParameterWildcard<ParameterSetDescription>::validateDescription, this,
  ...
  ParameterWildcard<std::vector<ParameterSet> >::ParameterWildcard(
      std::string const& pattern, WildcardValidationCriteria criteria, bool isTracked)
  ...
  void ParameterWildcard<std::vector<ParameterSet> >::validate_(
    ...
    if (psetDesc_) {
      for_all(parameterNames,
              std::bind(&ParameterWildcard<std::vector<ParameterSet> >::validatePSetVector, this,
                        std::placeholders::_1, std::ref(pset)));
    }
  }
