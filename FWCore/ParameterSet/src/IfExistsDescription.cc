...
namespace edm {
  ...
  void IfExistsDescription::validate_(ParameterSet& pset, std::set<std::string>& validatedLabels,
                                      bool optional) const {
  ...
  void IfExistsDescription::print_(std::ostream& os, bool optional, bool writeToCfi,
  ...
  int IfExistsDescription::howManyXORSubNodesExist_(ParameterSet const& pset) const {
    ...
    if (!types1.empty()) {
      ...
      if (!duplicateTypes.empty()) {
        ...
        throw edm::Exception(errors::LogicError)
            ...
            << "\"ifExists\" expression must be different from types used for other "
               "parameters\n"
