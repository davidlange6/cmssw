...
namespace edm {
  ...
  ParameterDescription<ParameterSetDescription>::ParameterDescription(
      std::string const& iLabel, ParameterSetDescription const& value, bool isTracked,
  ...
  void ParameterDescription<ParameterSetDescription>::validate_(
      ParameterSet& pset, std::set<std::string>& validatedLabels, bool optional) const {
  ...
  void ParameterDescription<ParameterSetDescription>::writeDoc_(std::ostream&,
                                                                int /*indentation*/) const {}

  // These next two should not be needed for this specialization
  bool ParameterDescription<ParameterSetDescription>::exists_(ParameterSet const&,
  ...
  ParameterDescription<std::vector<ParameterSet> >::ParameterDescription(
      std::string const& iLabel, ParameterSetDescription const& psetDesc, bool isTracked,
      std::vector<ParameterSet> const& vPset, Comment const& iComment)
  ...
  ParameterSetDescription const*
  ParameterDescription<std::vector<ParameterSet> >::parameterSetDescription() const {
    return psetDesc_.operator->();
  }
  ...
  void ParameterDescription<std::vector<ParameterSet> >::writeOneElementToCfi(
      ParameterSet const& pset, std::ostream& os, int indentation, bool& nextOneStartsWithAComma) {
  ...
  void ParameterDescription<std::vector<ParameterSet> >::writeCfi_(std::ostream& os,
    ...
    for_all(vPset_, std::bind(&writeOneElementToCfi, std::placeholders::_1, std::ref(os),
                              indentation, std::ref(nextOneStartsWithAComma)));
    os << "\n";
    printSpaces(os, indentation);
  }

  void ParameterDescription<std::vector<ParameterSet> >::writeDoc_(std::ostream&,
                                                                   int /*indentation*/) const {}

  // These next two should not be needed for this specialization
  bool ParameterDescription<std::vector<ParameterSet> >::exists_(ParameterSet const&,
                                                                 bool /*isTracked*/) const {
    throw Exception(errors::LogicError);
    return true;
  }
  ...
  namespace writeParameterValue {
    ...
    template <>
    void writeSingleValue<LuminosityBlockID>(std::ostream& os, LuminosityBlockID const& value,
                                             ValueFormat format) {
      if (format == CFI)
    ...
    void writeSingleValue<EventRange>(std::ostream& os, EventRange const& value,
      ...
      if (value.startLumi() == 0U) {
        if (format == CFI)
          os << "'" << value.startRun() << ":" << value.startEvent() << "-" << value.endRun() << ":"
             << value.endEvent() << "'";
        else
          os << value.startRun() << ":" << value.startEvent() << "-" << value.endRun() << ":"
             << value.endEvent();
    ...
    template <>
    void writeValueInVector<LuminosityBlockID>(std::ostream& os, LuminosityBlockID const& value,
                                               ValueFormat format) {
      if (format == CFI)
        os << "'" << value.run() << ":" << value.luminosityBlock() << "'";
      else
    ...
    template <typename T>
    void writeValueInVectorWithSpace(T const& value, std::ostream& os, int indentation,
                                     bool& startWithComma, ValueFormat format, int& i) {
    ...
    template <typename T>
    void writeVector(std::ostream& os, int indentation, std::vector<T> const& value_,
                     ValueFormat format) {
      std::ios_base::fmtflags ff = os.flags(std::ios_base::dec);
      ...
      if (value_.empty() && format == DOC) {
        ...
        int i = 0;
        for_all(value_,
                std::bind(&writeValueInVectorWithSpace<T>, std::placeholders::_1, std::ref(os),
                          indentation + 2, std::ref(startWithComma), format, std::ref(i)));
        if (format == CFI) os << "\n" << std::setw(indentation) << "";
        if (format == CFI and value_.size() > 255U) os << ") ";
      }
      os.flags(ff);
