...
namespace edm {
  ...
  void ParameterSwitchBase::insertAndCheckTypes(ParameterTypes switchType,
    ...
    if (caseWildcardTypes.find(switchType) != caseWildcardTypes.end()) {
      throw Exception(errors::LogicError)
          ...
          << "must be different from the types used for wildcards in the associated cases.  The "
             "following\n"
  ...
  void ParameterSwitchBase::printBase(std::ostream& os, bool optional, bool writeToCfi,
  ...
  void ParameterSwitchBase::printNestedContentBase(std::ostream& os, DocFormatHelper& dfh,
  ...
  void ParameterSwitchBase::printCase(
      std::pair<bool, value_ptr<ParameterDescriptionNode> > const& p, std::ostream& os,
  ...
  void ParameterSwitchBase::printCase(
      std::pair<std::string, value_ptr<ParameterDescriptionNode> > const& p, std::ostream& os,
      bool /*optional*/, DocFormatHelper& dfh, std::string const& switchLabel) {
    if (dfh.pass() == 0) {
      p.second->print(os, false, true, dfh);
    }
  ...
  bool ParameterSwitchBase::partiallyExists_(ParameterSet const& pset) const {
    return exists(pset);
  }

  int ParameterSwitchBase::howManyXORSubNodesExist_(ParameterSet const& pset) const {
