...
namespace edm {
  ...
  AllowedLabelsDescriptionBase::AllowedLabelsDescriptionBase(std::string const& label,
                                                             ParameterTypes iType, bool isTracked)
      : parameterHoldingLabels_(label, std::vector<std::string>(), isTracked),
        type_(iType),
        isTracked_(isTracked) {}

  AllowedLabelsDescriptionBase::AllowedLabelsDescriptionBase(char const* label,
                                                             ParameterTypes iType, bool isTracked)
      : parameterHoldingLabels_(label, std::vector<std::string>(), isTracked),
        type_(iType),
        isTracked_(isTracked) {}

  void AllowedLabelsDescriptionBase::checkAndGetLabelsAndTypes_(
  ...
  void AllowedLabelsDescriptionBase::validate_(ParameterSet& pset,
    ...
    if (parameterHoldingLabels_.exists(pset)) {
      ...
      if (isTracked()) {
        allowedLabels =
            pset.getParameter<std::vector<std::string> >(parameterHoldingLabels_.label());
      } else {
        allowedLabels =
            pset.getUntrackedParameter<std::vector<std::string> >(parameterHoldingLabels_.label());
      }
      for_all(allowedLabels,
              std::bind(&AllowedLabelsDescriptionBase::validateAllowedLabel_, this,
                        std::placeholders::_1, std::ref(pset), std::ref(validatedLabels)));
    }
  }

  void AllowedLabelsDescriptionBase::writeCfi_(std::ostream& os, bool& startWithComma,
                                               int indentation, bool& wroteSomething) const {
    parameterHoldingLabels_.writeCfi(os, startWithComma, indentation, wroteSomething);
  }

  void AllowedLabelsDescriptionBase::print_(std::ostream& os, bool optional, bool writeToCfi,
  ...
  void AllowedLabelsDescriptionBase::printNestedContent_(std::ostream& os, bool optional,
                                                         DocFormatHelper& dfh) const {
    printNestedContentBase_(os, optional, dfh);
    if (!dfh.brief()) os << "\n";
  }

  void AllowedLabelsDescriptionBase::printNestedContentBase_(std::ostream& os, bool optional,
                                                             DocFormatHelper& dfh) const {
    ...
    os << "Section " << dfh.section() << "." << dfh.counter() << " "
       << parameterHoldingLabels_.label() << " - allowed labels description\n";
    printSpaces(os, indentation);
  ...
  bool AllowedLabelsDescriptionBase::partiallyExists_(ParameterSet const& pset) const {
    return exists(pset);
  }
