...
namespace edm {
  ...
  void ParameterSet::insertParameterSet(bool okay_to_replace, std::string const& name,
  ...
  void ParameterSet::augment(ParameterSet const& from) {
    ...
    for (psettable::const_iterator b = from.psetTable_.begin(), e = from.psetTable_.end(); b != e;
         ++b) {
      this->insertParameterSet(false, b->first, b->second);
    }
    for (vpsettable::const_iterator b = from.vpsetTable_.begin(), e = from.vpsetTable_.end();
         b != e; ++b) {
  ...
  ParameterSet ParameterSet::trackedPart() const {
    ...
    for (psettable::const_iterator psetItr = psetTable_.begin(); psetItr != psetTable_.end();
         ++psetItr) {
      if (psetItr->second.isTracked()) {
        result.addParameter<ParameterSet>(psetItr->first, psetItr->second.pset().trackedPart());
      }
    }
    for (vpsettable::const_iterator vpsetItr = vpsetTable_.begin(); vpsetItr != vpsetTable_.end();
         ++vpsetItr) {
  ...
  */
  ...
  bool isTransientEqual(ParameterSet const& a, ParameterSet const& b) {
    ...
    for (Ti i = a.tbl().begin(), e = a.tbl().end(), j = b.tbl().begin(), f = b.tbl().end();
         i != e && j != f; ++i, ++j) {
      if (*i != *j) {
        return false;
      }
    ...
    for (Pi i = a.psetTable().begin(), e = a.psetTable().end(), j = b.psetTable().begin(),
            f = b.psetTable().end();
         i != e && j != f; ++i, ++j) {
      if (i->first != j->first) {
        return false;
    ...
    for (PVi i = a.vpsetTable().begin(), e = a.vpsetTable().end(), j = b.vpsetTable().begin(),
             f = b.vpsetTable().end();
         i != e && j != f; ++i, ++j) {
      if (i->first != j->first) {
        return false;
  ...
  template <>
  std::vector<unsigned int> ParameterSet::getParameter<std::vector<unsigned int> >(
      std::string const& name) const {
  ...
  template <>
  double ParameterSet::getUntrackedParameter<double>(std::string const& name,
                                                     double const& defaultValue) const {
    Entry const* entryPtr = retrieveUntracked(name);
  ...
  template <>
  InputTag ParameterSet::getUntrackedParameter<InputTag>(std::string const& name,
                                                         InputTag const& defaultValue) const {
    Entry const* entryPtr = retrieveUntracked(name);
  ...
  template <>
  EventID ParameterSet::getUntrackedParameter<EventID>(std::string const& name,
                                                       EventID const& defaultValue) const {
    Entry const* entryPtr = retrieveUntracked(name);
  ...
  template <>
  long long ParameterSet::getUntrackedParameter<long long>(char const* name,
                                                           long long const& defaultValue) const {
    Entry const* entryPtr = retrieveUntracked(name);
    return entryPtr == 0 ? defaultValue : entryPtr->getInt64();
  }
  ...
  template <>
  double ParameterSet::getUntrackedParameter<double>(char const* name,
                                                     double const& defaultValue) const {
    Entry const* entryPtr = retrieveUntracked(name);
    return entryPtr == 0 ? defaultValue : entryPtr->getDouble();
  }
  ...
  template <>
  FileInPath ParameterSet::getUntrackedParameter<FileInPath>(char const* name,
                                                             FileInPath const& defaultValue) const {
    Entry const* entryPtr = retrieveUntracked(name);
    return entryPtr == 0 ? defaultValue : entryPtr->getFileInPath();
  }
  ...
  template <>
  InputTag ParameterSet::getUntrackedParameter<InputTag>(char const* name,
                                                         InputTag const& defaultValue) const {
    Entry const* entryPtr = retrieveUntracked(name);
    return entryPtr == 0 ? defaultValue : entryPtr->getInputTag();
  }
  ...
  template <>
  ESInputTag ParameterSet::getUntrackedParameter<ESInputTag>(char const* name,
                                                             ESInputTag const& defaultValue) const {
    Entry const* entryPtr = retrieveUntracked(name);
    return entryPtr == 0 ? defaultValue : entryPtr->getESInputTag();
  }
  ...
  template <>
  EventID ParameterSet::getUntrackedParameter<EventID>(char const* name,
                                                       EventID const& defaultValue) const {
    Entry const* entryPtr = retrieveUntracked(name);
    return entryPtr == 0 ? defaultValue : entryPtr->getEventID();
  }
  ...
  template <>
  EventRange ParameterSet::getUntrackedParameter<EventRange>(char const* name,
                                                             EventRange const& defaultValue) const {
    Entry const* entryPtr = retrieveUntracked(name);
    return entryPtr == 0 ? defaultValue : entryPtr->getEventRange();
  }
  ...
  template <>
  void ParameterSet::addUntrackedParameter<ParameterSet>(std::string const& name,
                                                         ParameterSet const& value) {
    insertParameterSet(true, name, ParameterSetEntry(value, false));
  }

  template <>
  void ParameterSet::addUntrackedParameter<VParameterSet>(std::string const& name,
                                                          VParameterSet const& value) {
    insertVParameterSet(true, name, VParameterSetEntry(value, false));
  }

  template <>
  void ParameterSet::addUntrackedParameter<ParameterSet>(char const* name,
                                                         ParameterSet const& value) {
    insertParameterSet(true, name, ParameterSetEntry(value, false));
  }

  template <>
  void ParameterSet::addUntrackedParameter<VParameterSet>(char const* name,
                                                          VParameterSet const& value) {
    insertVParameterSet(true, name, VParameterSetEntry(value, false));
  }
