...
namespace edm {
  ...
  ParameterWildcardBase::ParameterWildcardBase(ParameterTypes iType, bool isTracked,
  ...
  void ParameterWildcardBase::validateMatchingNames(std::vector<std::string> const& matchingNames,
    ...
    if (criteria_ == RequireAtLeastOne && matchingNames.size() < 1U && !optional) {
      ...
      if ((matchingNames.size() < 1U && !optional) || matchingNames.size() > 1U) {
        throw Exception(errors::Configuration)
            << "Parameter wildcard of type \"" << parameterTypeEnumToString(type())
            << "\" requires\n"
  ...
  void ParameterWildcardBase::checkAndGetLabelsAndTypes_(
      std::set<std::string>& /*usedLabels*/, std::set<ParameterTypes>& /*parameterTypes*/,
      std::set<ParameterTypes>& wildcardTypes) const {
    wildcardTypes.insert(type());
  }

  void ParameterWildcardBase::print_(std::ostream& os, bool optional, bool /*writeToCfi*/,
  ...
  void ParameterWildcardBase::writeCfi_(std::ostream&, bool& /*startWithComma*/,
                                        int /*indentation*/, bool& /*wroteSomething*/) const {
    // Until we implement default labels and values there is nothing
    // to do here.
  }

  bool ParameterWildcardBase::partiallyExists_(ParameterSet const& pset) const {
    return exists(pset);
  }

  int ParameterWildcardBase::howManyXORSubNodesExist_(ParameterSet const& pset) const {
