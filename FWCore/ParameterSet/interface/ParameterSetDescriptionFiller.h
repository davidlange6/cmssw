...
FWCore/ParameterSet/interface/ParameterSetDescriptionFiller.h
 ...
 Description: A concrete ParameterSetDescription filler which calls a static function of the
template argument
...
method of the templated argument.  This allows the ParameterSetDescriptionFillerPluginFactory to
communicate with existing plugins.
...
namespace edm {
  ...
  public:
    ...
    void fill(ConfigurationDescriptions& descriptions) const override {
      ...
      // we don't have a need for prevalidation of services at the moment, so this is a
      // placeholder
  ...
  public:
    ...
    void fill(ConfigurationDescriptions& descriptions) const override {
      ...
      std::conditional_t<edm::fillDetails::has_prevalidate_function<T>::value,
                         edm::fillDetails::DoPrevalidate<T>, edm::fillDetails::DoNothing<T>>
          prevalidate;
  ...
  private:
    DescriptionFillerForESProducers(const DescriptionFillerForESProducers&) =
        delete;  // stop default
