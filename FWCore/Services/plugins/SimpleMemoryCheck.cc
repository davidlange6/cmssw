...
#define SIMPLE_MEMORY_CHECK_ORIGINAL_XML_OUTPUT
      ...
      reportData.insert(
          std::make_pair("AverageGrowthRateVsize",
      ...
      reportData.insert(std::make_pair(
          "AverageGrowthRateRss", d2str(averageGrowthRate(current_->rss, growthRateRss_, count_))));
      ...
      if (FILE* fmeminfo = fopen("/proc/meminfo", "r")) {
        ...
        while (fgets(buf, sizeof(buf), fmeminfo)) {
          ...
          if (token != nullptr) {
            ...
            reportMemoryProperties.insert(
                std::make_pair(category.substr(0, strlen(token) - 1), i2str(value)));
          }
        }
...
// This is a form of reportMemoryInfo taking s vector, not a map
      ...
      if (not jobReportOutputOnly_ && ((*current_ > max_) || oncePerEventMode_)) {
        if (count_ >= num_to_skip_) {
          ...
          if (!showMallocInfo_) {  // default
            LogWarning("MemoryCheck") << "MemoryCheck: " << type << " " << mdname << ":" << mdlabel
            ...
            LogWarning("MemoryCheck")
                << "MemoryCheck: " << type << " " << mdname << ":" << mdlabel << " VSIZE "
...
#ifdef SIMPLE_MEMORY_CHECK_DIFFERENT_XML_OUTPUT
    std::string SimpleMemoryCheck::eventStatOutput(std::string title,
    ...
    std::ostream& operator<<(std::ostream& os, SimpleMemoryCheck::SignificantEvent const& se) {
      os << "[" << se.count << "] " << se.event << "  vsize = " << se.vsize
         << " deltaVsize = " << se.deltaVsize << " rss = " << se.rss << " delta = " << se.deltaRss;
    ...
    std::ostream& operator<<(std::ostream& os, SimpleMemoryCheck::SignificantModule const& sm) {
      if (sm.postEarlyCount > 0) {
        os << "\nPost Early Events:  TotalDeltaVsize: " << sm.totalDeltaVsize
           << " (avg: " << sm.totalDeltaVsize / sm.postEarlyCount << "; max: " << sm.maxDeltaVsize
