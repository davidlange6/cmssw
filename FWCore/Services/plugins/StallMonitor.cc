...
namespace {
  ...
  std::enable_if_t<std::is_integral<H>::value> concatenate(std::ostream& os, H const h,
                                                           T const... t) {
    os << ' ' << h;
    concatenate(os, t...);
  }
...
namespace edm {
  namespace service {
    ...
    private:
      ...
      tbb::concurrent_unordered_map<std::pair<StreamID_value, ModuleID>, decltype(beginTime_)>
          stallStart_{};
...
StallMonitor::StallMonitor(ParameterSet const& iPS, ActivityRegistry& iRegistry)
    : file_{iPS.getUntrackedParameter<std::string>("fileName", filename_default)},
      validFile_{file_},
      stallThreshold_{
          static_cast<long int>(iPS.getUntrackedParameter<double>("stallThreshold") * 1000)} {
...
void StallMonitor::fillDescriptions(ConfigurationDescriptions& descriptions) {
  ...
  descriptions.setComment(
      "This service keeps track of various times in event-processing to determine which modules "
      "are stalling.");
}
...
void StallMonitor::postBeginJob() {
  ...
  if (validFile_) {
    ...
    for (std::size_t i{}; i < moduleLabels_.size(); ++i) {
      ...
      oss << "#M " << std::setw(width) << std::left << col0(i) << space << std::left
          << moduleLabels_[i] << '\n';
    }
    oss << '\n';
    file_.write(oss.str());
...
void StallMonitor::preEvent(StreamContext const& sc) {
  ...
  auto msg = assembleMessage<step::preEvent>(stream_id(sc), eid.run(), eid.luminosityBlock(),
                                             eid.event(), t);
  file_.write(std::move(msg));
}
...
void StallMonitor::preModuleEvent(StreamContext const& sc, ModuleCallingContext const& mcc) {
  ...
  if (validFile_) {
    auto msg = assembleMessage<step::preModuleEvent>(
        sid, mid, duration_cast<milliseconds>(preModEvent - beginTime_).count());
    file_.write(std::move(msg));
  }

  auto const preFetch_to_preModEvent =
      duration_cast<milliseconds>(preModEvent - stallStart_[std::make_pair(sid, mid)]);
...
void StallMonitor::postEvent(StreamContext const& sc) {
  auto const t = duration_cast<milliseconds>(now() - beginTime_).count();
  auto const& eid = sc.eventID();
  auto msg = assembleMessage<step::postEvent>(stream_id(sc), eid.run(), eid.luminosityBlock(),
                                              eid.event(), t);
  file_.write(std::move(msg));
}
...
void StallMonitor::postEndJob() {
  ...
  out << tag << space << std::setfill('-') << col1(std::string{}) << space << col2(std::string{})
      << space << col3(std::string{}) << space << col4(std::string{}) << '\n';
  ...
  for (auto const& stats : moduleStats_) {
    if (stats.label().empty() ||  // See comment in filling of moduleLabels_;
      ...
      continue;
    out << std::left << tag << space << col1(stats.label()) << space << std::right
        << col2(stats.numberOfStalls()) << space << col3(to_seconds_str(stats.totalStalledTime()))
        << space << col4(to_seconds_str(stats.maxStalledTime())) << '\n';
  }
}
