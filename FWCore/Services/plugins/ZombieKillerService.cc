...
ZombieKillerService::ZombieKillerService(edm::ParameterSet const& iPSet,
    ...
    : m_checkThreshold(
          iPSet.getUntrackedParameter<unsigned int>("numberOfAllowedFailedChecksInARow")),
  ...
  iRegistry.watchPreModuleBeginStream(
  ...
  iRegistry.watchPreModuleEndStream(
      [this](StreamContext const&, ModuleCallingContext const&) { notAZombieYet(); });
  iRegistry.watchPostModuleEndStream(
      [this](StreamContext const&, ModuleCallingContext const&) { notAZombieYet(); });
  ...
  iRegistry.watchPreModuleEvent(
      [this](StreamContext const&, ModuleCallingContext const&) { notAZombieYet(); });
  iRegistry.watchPostModuleEvent(
      [this](StreamContext const&, ModuleCallingContext const&) { notAZombieYet(); });
  ...
  iRegistry.watchPreModuleStreamEndRun(
      [this](StreamContext const&, ModuleCallingContext const&) { notAZombieYet(); });
...
void ZombieKillerService::checkForZombie() {
  if (not m_stillAlive) {
    ...
    if (m_numberChecksWhenNotAlive > m_checkThreshold) {
      ...
      edm::LogWarning("JobProgressing")
          << "It has been " << m_numberChecksWhenNotAlive * m_secsBetweenChecks
