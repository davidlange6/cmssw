  ...
  namespace service {
    ...
    void UnixSignalService::fillDescriptions(ConfigurationDescriptions& descriptions) {
      ...
      desc.addUntracked<bool>("EnableCtrlC", true)
          ->setComment(
              "If 'true', you can stop a cmsRun job gracefully by sending it a '<control> c' "
              "keyboard interrupt (i.e. SIGINT).");
      descriptions.add("UnixSignalService", desc);
      descriptions.setComment(
          "This service sets up unix signal handlers for the unix signal SIGUSR2 and optionally "
          "SIGINT"
          " so that when cmsRun is sent a signal the application will stop processing and shut "
          "down gracefully.");
    }
  }  // end of namespace service
}  // end of namespace edm
