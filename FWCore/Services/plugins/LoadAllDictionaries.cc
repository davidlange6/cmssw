...
edm::service::LoadAllDictionaries::LoadAllDictionaries(const edm::ParameterSet& iConfig) {
  ...
  if (doLoad) {
    ...
    for (edmplugin::PluginManager::Infos::const_iterator itInfo = itFound->second.begin(),
      ...
      // NOTE: since we have the library already, we could be more efficient if we just load it
      // ourselves
    }
  }
}
...
void edm::service::LoadAllDictionaries::fillDescriptions(
  ...
  descriptions.setComment(
      "This service allows you to force all known dictionaries to be loaded at the beginning of "
      "the job");
}

using edm::service::LoadAllDictionaries;
DEFINE_FWK_SERVICE_MAKER(LoadAllDictionaries,
