...
namespace {
  ...
  public:
    ...
    friend std::ostream& operator<<(std::ostream& out, TimeStamper const& timestamp) {
      if (timestamp.enabled_) out << std::setprecision(2) << edm::TimeOfDay() << "  ";
      return out;
    }
...
Tracer::Tracer(ParameterSet const& iPS, ActivityRegistry& iRegistry)
  ...
  for (std::string& label :
       iPS.getUntrackedParameter<std::vector<std::string>>("dumpContextForLabels"))
      ...
      [this](edm::StreamContext const& iContext, edm::TerminationOrigin iOrigin) {
        ...
        if (iContext.eventID().luminosityBlock() == 0) {
          ...
          if (iContext.eventID().event() == 0) {
            out << indention_ << indention_
                << " early termination of stream lumi: stream = " << iContext.streamID()
                << " run = " << iContext.eventID().run()
...
void Tracer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  desc.addUntracked<std::vector<std::string>>("dumpContextForLabels", std::vector<std::string>{})
      ->setComment(
          "Prints context information to cout for the module transitions associated with these "
          "modules' labels");
  desc.addUntracked<bool>("dumpNonModuleContext", false)
      ->setComment(
          "Prints context information to cout for the transitions not associated with any module "
          "label");
  desc.addUntracked<bool>("dumpPathsAndConsumes", false)
      ->setComment(
          "Prints information to cout about paths, endpaths, products consumed by modules and the "
          "dependencies between modules created by the products they consume");
  ...
  descriptions.setComment(
      "This service prints each phase the framework is processing, e.g. constructing a "
      "module,running a module, etc.");
}

void Tracer::preallocate(service::SystemBounds const& bounds) {
  LogAbsolute("Tracer") << TimeStamper(printTimestamps_) << indention_
                        << " preallocate: " << bounds.maxNumberOfConcurrentRuns()
                        << " concurrent runs, " << bounds.maxNumberOfConcurrentLuminosityBlocks()
                        << " concurrent luminosity sections, " << bounds.maxNumberOfStreams()
                        << " streams";
}
...
void Tracer::preBeginJob(PathsAndConsumesOfModulesBase const& pathsAndConsumes,
  ...
  if (dumpPathsAndConsumes_) {
    ...
    out << "For products not in the event, \'run\' or \'lumi\' is added to indicate the TTree "
           "they are from.\n";
...
void Tracer::preModuleBeginStream(StreamContext const& sc, ModuleCallingContext const& mcc) {
  ...
  out << TimeStamper(printTimestamps_);
  out << indention_ << indention_
      << " starting: begin stream for module: stream = " << sc.streamID() << " label = '"
...
void Tracer::postModuleBeginStream(StreamContext const& sc, ModuleCallingContext const& mcc) {
  ...
  out << TimeStamper(printTimestamps_);
  out << indention_ << indention_
      << " finished: begin stream for module: stream = " << sc.streamID() << " label = '"
      << desc.moduleLabel() << "' id = " << desc.id();
  if (dumpContextForLabels_.find(desc.moduleLabel()) != dumpContextForLabels_.end()) {
    out << "\n" << sc;
    out << mcc;
...
void Tracer::preModuleEndStream(StreamContext const& sc, ModuleCallingContext const& mcc) {
  ...
  LogAbsolute out("Tracer");
  out << TimeStamper(printTimestamps_);
  out << indention_ << indention_ << " starting: end stream for module: stream = " << sc.streamID()
...
void Tracer::postModuleEndStream(StreamContext const& sc, ModuleCallingContext const& mcc) {
  ...
  LogAbsolute out("Tracer");
  out << TimeStamper(printTimestamps_);
  out << indention_ << indention_ << " finished: end stream for module: stream = " << sc.streamID()
      << " label = '" << desc.moduleLabel() << "' id = " << desc.id();
  if (dumpContextForLabels_.find(desc.moduleLabel()) != dumpContextForLabels_.end()) {
    out << "\n" << sc;
    out << mcc;
...
void Tracer::preStreamEndRun(StreamContext const& sc) {
  LogAbsolute out("Tracer");
  out << TimeStamper(printTimestamps_);
  out << indention_ << indention_ << " starting: end run: stream = " << sc.streamID()
      << " run = " << sc.eventID().run() << " time = " << sc.timestamp().value();
  if (dumpNonModuleContext_) {
    out << "\n" << sc;
  }
...
void Tracer::postStreamEndRun(StreamContext const& sc) {
  LogAbsolute out("Tracer");
  out << TimeStamper(printTimestamps_);
  out << indention_ << indention_ << " finished: end run: stream = " << sc.streamID()
      << " run = " << sc.eventID().run() << " time = " << sc.timestamp().value();
  if (dumpNonModuleContext_) {
    out << "\n" << sc;
  }
...
void Tracer::preModuleConstruction(ModuleDescription const& desc) {
  LogAbsolute out("Tracer");
  out << TimeStamper(printTimestamps_);
  out << indention_ << indention_ << " starting: constructing module with label '"
      << desc.moduleLabel() << "' id = " << desc.id();
  if (dumpContextForLabels_.find(desc.moduleLabel()) != dumpContextForLabels_.end()) {
    out << "\n" << desc;
  }
...
void Tracer::postModuleConstruction(ModuleDescription const& desc) {
  LogAbsolute out("Tracer");
  out << TimeStamper(printTimestamps_);
  out << indention_ << indention_ << " finished: constructing module with label '"
      << desc.moduleLabel() << "' id = " << desc.id();
  if (dumpContextForLabels_.find(desc.moduleLabel()) != dumpContextForLabels_.end()) {
    out << "\n" << desc;
  }
...
void Tracer::preModuleEventPrefetching(StreamContext const& sc, ModuleCallingContext const& mcc) {
  ...
  out << " starting: prefetching before processing event for module: stream = " << sc.streamID()
...
void Tracer::postModuleEventPrefetching(StreamContext const& sc, ModuleCallingContext const& mcc) {
  ...
  out << " finished: prefetching before processing event for module: stream = " << sc.streamID()
      << " label = '" << mcc.moduleDescription()->moduleLabel()
      << "' id = " << mcc.moduleDescription()->id();
  if (dumpContextForLabels_.find(mcc.moduleDescription()->moduleLabel()) !=
      dumpContextForLabels_.end()) {
...
void Tracer::preModuleEventDelayedGet(StreamContext const& sc, ModuleCallingContext const& mcc) {
  ...
  out << " starting: delayed processing event for module: stream = " << sc.streamID()
      << " label = '" << mcc.moduleDescription()->moduleLabel()
      << "' id = " << mcc.moduleDescription()->id();
  if (dumpContextForLabels_.find(mcc.moduleDescription()->moduleLabel()) !=
      dumpContextForLabels_.end()) {
...
void Tracer::postModuleEventDelayedGet(StreamContext const& sc, ModuleCallingContext const& mcc) {
  ...
  out << " finished: delayed processing event for module: stream = " << sc.streamID()
      << " label = '" << mcc.moduleDescription()->moduleLabel()
      << "' id = " << mcc.moduleDescription()->id();
  if (dumpContextForLabels_.find(mcc.moduleDescription()->moduleLabel()) !=
      dumpContextForLabels_.end()) {
