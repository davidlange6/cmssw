...
namespace edm {
  ...
  namespace service {
    ...
    private:
      ...
      void preSourceConstruction(ModuleDescription const &md, int maxEvents, int maxLumis,
...
void CondorStatusService::beginPost() {
  ...
  int maxEvents = processParameterSet.getUntrackedParameterSet("maxEvents", ParameterSet())
  ...
  unsigned int eventsPerLumi =
      pset.getUntrackedParameter<unsigned int>("numberEventsInLuminosityBlock", 0);
  if ((lumiCount == 0) && (maxEvents > 0) && (eventsPerLumi > 0)) {
    lumiCount = static_cast<unsigned int>(
        std::ceil(static_cast<float>(maxEvents) / static_cast<float>(eventsPerLumi)));
  }
...
void CondorStatusService::updateImpl(time_t sinceLastUpdate) {
  ...
  float ema_coeff =
      1 - std::exp(-static_cast<float>(sinceLastUpdate) /
...
bool CondorStatusService::updateChirpQuoted(const std::string &key_suffix,
  ...
  value_copy.erase(
      remove_if(value_copy.begin(), value_copy.end(),
...
bool CondorStatusService::updateChirpImpl(const std::string &key_suffix, const std::string &value) {
  ...
  int status = posix_spawnp(&pid, "condor_chirp", &file_actions, nullptr,
...
void CondorStatusService::fillDescriptions(ConfigurationDescriptions &descriptions) {
  ...
  desc.addOptionalUntracked<std::string>("tag")->setComment(
      "Identifier tag for this process (a value of 'Foo' results in ClassAd attributes of the "
      "form 'ChirpCMSSWFoo*')");
