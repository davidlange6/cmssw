  ...
  namespace service {
    ...
    void Timing::fillDescriptions(ConfigurationDescriptions& descriptions) {
      ...
      desc.addUntracked<double>("excessiveTimeThreshold", 0.)
          ->setComment(
              "Amount of time in seconds before reporting a module or source has taken "
              "excessive time. A value of 0.0 turns off this reporting.");
    ...
    void Timing::postEndJob() {
      ...
      LogImportant("TimeReport") << "TimeReport> Time report complete in " << total_job_time
                                 << " seconds"
                                 << "\n"
                                 << " Time Summary: \n"
    ...
    void Timing::postEvent(StreamContext const& iStream) {
      ...
      if (not summary_only_) {
        ...
        LogPrint("TimeEvent") << "TimeEvent> " << eventID.event() << " " << eventID.run() << " "
                              << curr_event_time;
      }
    ...
    void Timing::postModuleEvent(StreamContext const& iStream,
      ...
      double t = postCommon();
      if (not summary_only_) {
        LogPrint("TimeModule") << "TimeModule> " << eventID.event() << " " << eventID.run() << " "
                               << desc.moduleLabel() << " " << desc.moduleName() << " " << t;
      }
    }
    ...
    void Timing::postModuleGlobal(GlobalContext const&, ModuleCallingContext const& mcc) {
      postCommon();
    }

    void Timing::preModuleStream(StreamContext const&, ModuleCallingContext const&) { pushStack(); }

    void Timing::postModuleStream(StreamContext const&, ModuleCallingContext const& mcc) {
      postCommon();
    }
    ...
    double Timing::postCommon() const {
      double t = popStack();
      if (t > threshold_) {
        LogError("ExcessiveTime") << "ExcessiveTime: Module used " << t
                                  << " seconds of time which exceeds the error threshold "
                                     "configured in the Timing Service of "
                                  << threshold_ << " seconds.";
      }
      return t;
    }
