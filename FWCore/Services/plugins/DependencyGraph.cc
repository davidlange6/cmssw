...
private:
  ...
  static constexpr const char *module_type_desc[]{"Unknown",    "Source",      "ESSource",
                                                  "ESProducer", "EDAnalyzer",  "EDProducer",
                                                  "EDFilter",   "OutputModule"};
  ...
  boost::subgraph<boost::adjacency_list<
      ...
      boost::property<
          ...
          boost::property<
              boost::graph_graph_attribute_t, GraphvizAttributes,  // Graphviz graph attributes
              boost::property<
...
DependencyGraph::EDMModuleType DependencyGraph::edmModuleTypeEnum(
  ...
  if (not pset.existsAs<std::string>("@module_edm_type")) return EDMModuleType::Unknown;

  std::string const &t = pset.getParameter<std::string>("@module_edm_type");
  for (EDMModuleType v : {EDMModuleType::Source, EDMModuleType::ESSource, EDMModuleType::ESProducer,
                          EDMModuleType::EDAnalyzer, EDMModuleType::EDProducer,
                          EDMModuleType::EDFilter, EDMModuleType::OutputModule}) {
    if (t == module_type_desc[static_cast<std::underlying_type_t<EDMModuleType>>(v)]) return v;
  }
...
DependencyGraph::DependencyGraph(ParameterSet const &config, ActivityRegistry &registry)
    : m_filename(config.getUntrackedParameter<std::string>("fileName")),
      m_highlightModules(make_unordered_set(
          config.getUntrackedParameter<std::vector<std::string>>("highlightModules"))),
...
void DependencyGraph::preSourceConstruction(ModuleDescription const &module) {
  ...
  attributes["shape"] =
      shapes[static_cast<std::underlying_type_t<EDMModuleType>>(EDMModuleType::Source)];
...
void DependencyGraph::preBeginJob(PathsAndConsumesOfModulesBase const &pathsAndConsumes,
  ...
  for (edm::ModuleDescription const *module : pathsAndConsumes.allModules()) {
    ...
    attributes["tooltip"] = module->moduleName();
    attributes["shape"] =
        shapes[static_cast<std::underlying_type_t<EDMModuleType>>(edmModuleTypeEnum(*module))];
    attributes["style"] = "filled";
    attributes["color"] = "black";
  ...
  for (edm::ModuleDescription const *consumer : pathsAndConsumes.allModules()) {
    for (edm::ModuleDescription const *module :
      ...
      edm::LogInfo("DependencyGraph")
          << "module " << consumer->moduleLabel() << " depends on module " << module->moduleLabel();
  ...
  // marke the modules in the paths as scheduled, and add a soft dependency to reflect the order of
  // modules along each path
