...
namespace {
  ...
  void RootErrorHandlerImpl(int level, char const* location, char const* message) {
    ...
    if ((el_message.find("no dictionary for class") != std::string::npos) ||
        ...
        (el_message.find(
             "Announced number of args different from the real number of argument passed") !=
         std::string::npos) ||  // Always printed if gDebug>0 - regardless of whether warning
                                // message is real.
...
namespace edm {
  namespace service {
    ...
    void cmssw_stacktrace_fork() {
      ...
      int pid =
#ifdef __linux__
          clone(edm::service::cmssw_stacktrace, child_stack_ptr, CLONE_VM | CLONE_FS | SIGCHLD,
                nullptr);
#else
          fork();
      if (child_stack_ptr) {
...
// calls dlsym.
    ...
    char* const InitRootHandlers::pstackArgv_[] = {pstackName, dashC, InitRootHandlers::pidString_,
                                                   nullptr};
    ...
    InitRootHandlers::InitRootHandlers(ParameterSet const& pset, ActivityRegistry& iReg)
        : RootHandlers(),
          ...
          autoLibraryLoader_(loadAllDictionaries_ or
    ...
    void InitRootHandlers::fillDescriptions(ConfigurationDescriptions& descriptions) {
      ...
      desc.addUntracked<bool>("ResetRootErrHandler", true)
          ->setComment(
              "If True, ROOT messages (e.g. errors, warnings) are handled by this service, "
              "rather than by ROOT.");
      ...
      desc.addUntracked<bool>("AbortOnSignal", true)
          ->setComment(
              "If True, do an abort when a signal occurs that causes a crash. If False, ROOT "
              "will do an exit which attempts to do a clean shutdown.");
