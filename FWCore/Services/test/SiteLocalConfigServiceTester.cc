...
SiteLocalConfigServiceTester::SiteLocalConfigServiceTester(const edm::ParameterSet& iPSet)
    : m_cacheHint(iPSet.getUntrackedParameter<std::string>("sourceCacheHint")),
      ...
      m_nativeProtocols(
          iPSet.getUntrackedParameter<std::vector<std::string> >("sourceNativeProtocols")),
      m_valuesSet(iPSet.getUntrackedParameter<bool>("sourceValuesSet", true)) {}

// SiteLocalConfigServiceTester::SiteLocalConfigServiceTester(const SiteLocalConfigServiceTester&
// rhs)
// {
...
//
// const SiteLocalConfigServiceTester& SiteLocalConfigServiceTester::operator=(const
// SiteLocalConfigServiceTester& rhs)
// {
...
static void throwNotSet(const char* iName) {
  throw cms::Exception("TestFailure")
      << "The value " << iName << " should have been set but was not";
}
...
namespace {
  ...
  void checkNotSet(const char* iName, const std::vector<std::string>* iRetrieved) {
    if (0 != iRetrieved) {
      throw cms::Exception("TestFailure")
          << "The value " << iName << " should not have been set but was set";
    }
  }
}
//
...
void SiteLocalConfigServiceTester::analyze(const edm::Event&, const edm::EventSetup&) {
  ...
  if (m_valuesSet) {
    ...
    if (protocols->size() != m_nativeProtocols.size()) {
      throw cms::Exception("TestFailure")
          << "The value sourceNativeProtocols has size " << protocols->size()
