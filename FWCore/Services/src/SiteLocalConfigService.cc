...
namespace {
  ...
  template <typename T>
  static void overrideFromPSet(char const *iName, edm::ParameterSet const &iPSet, T &iHolder,
                               T const *&iPointer) {
...
namespace edm {
  namespace service {
    ...
    SiteLocalConfigService::SiteLocalConfigService(ParameterSet const &pset)
      ...
      overrideFromPSet("overrideSourceCloneCacheHintDir", pset, m_cloneCacheHint,
                       m_cloneCacheHintPtr);
      overrideFromPSet("overrideSourceReadHint", pset, m_readHint, m_readHintPtr);
      overrideFromPSet("overrideSourceNativeProtocols", pset, m_nativeProtocols,
      ...
      overrideFromPSet("overrideStatisticsDestination", pset, m_statisticsDestination,
                       tmpStringPtr);
    ...
    std::string const *SiteLocalConfigService::sourceCacheTempDir() const {
    ...
    std::string const *SiteLocalConfigService::sourceCloneCacheHint() const {
      return m_cloneCacheHintPtr;
    }

    std::string const *SiteLocalConfigService::sourceReadHint() const { return m_readHintPtr; }

    unsigned int const *SiteLocalConfigService::sourceTTreeCacheSize() const {
          ...
          for (XMLSize_t i = 0; i < numSites; ++i) {
            ...
            // Parsing of the calib-data section
            {
              ...
              if (calibDataList->getLength() > 0) {
                ...
                DOMNodeList *frontierConnectList =
                    calibData->getElementsByTagName(uStr("frontier-connect").ptr());
            ...
            // Parsing of the source config section
            {
              DOMNodeList *sourceConfigList =
                  site->getElementsByTagName(uStr("source-config").ptr());
              ...
              if (sourceConfigList->getLength() > 0) {
                ...
                DOMNodeList *cacheTempDirList =
                    sourceConfig->getElementsByTagName(uStr("cache-temp-dir").ptr());
                ...
                DOMNodeList *cacheMinFreeList =
                    sourceConfig->getElementsByTagName(uStr("cache-min-free").ptr());
                ...
                DOMNodeList *cacheHintList =
                    sourceConfig->getElementsByTagName(uStr("cache-hint").ptr());
                ...
                DOMNodeList *cloneCacheHintList =
                    sourceConfig->getElementsByTagName(uStr("clone-cache-hint").ptr());

                if (cloneCacheHintList->getLength() > 0) {
                  DOMElement *cloneCacheHint =
                      static_cast<DOMElement *>(cloneCacheHintList->item(0));
                ...
                DOMNodeList *readHintList =
                    sourceConfig->getElementsByTagName(uStr("read-hint").ptr());
                ...
                DOMNodeList *ttreeCacheSizeList =
                    sourceConfig->getElementsByTagName(uStr("ttree-cache-size").ptr());
                ...
                DOMNodeList *timeoutList =
                    sourceConfig->getElementsByTagName(uStr("timeout-in-seconds").ptr());
                ...
                DOMNodeList *nativeProtocolsList =
                    sourceConfig->getElementsByTagName(uStr("native-protocols").ptr());
                ...
                if (nativeProtocolsList->getLength() > 0) {
                  ...
                  for (XMLSize_t i = 0; i < numNodes; ++i) {
                    ...
                    DOMElement *child = static_cast<DOMElement *>(childNode);
                    m_nativeProtocols.push_back(
                        toString(child->getAttribute(uStr("prefix").ptr())));
                  }
    ...
    void SiteLocalConfigService::computeStatisticsDestination() {
      ...
      const std::string &port =
          (inputStrings.size() > 1) ? inputStrings[1] : m_statisticsDefaultPort;
      struct addrinfo *res;
    ...
    void SiteLocalConfigService::fillDescriptions(ConfigurationDescriptions &descriptions) {
      ...
      desc.addOptionalUntracked<std::string>("overrideStatisticsDestination")
          ->setComment(
              "Provide an alternate network destination for I/O statistics (must be in the form "
              "of host:port).");
      desc.addOptionalUntracked<std::vector<std::string> >("overrideStatisticsInfo")
          ->setComment(
              "Provide an alternate listing of statistics to send (comma separated list; "
              "current options are 'dn' or 'nodn').  If left blank, all information is snet "
              "(including DNs).");
