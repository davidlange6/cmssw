...
std::string edm::pythonFileToConfigure(const std::string& iPythonFileName) {
  ...
  try {
    try {
      object result((boost::python::handle<>(PyRun_String(
          initCommand.c_str(), Py_file_input, main_namespace.ptr(), main_namespace.ptr()))));
    } catch (error_already_set) {
      edm::pythonToCppException("Configuration");
    }
    try {
      std::string command("cms.findProcess(fileDict).dumpConfig()");
      object result((handle<>(PyRun_String(command.c_str(), Py_eval_input, main_namespace.ptr(),
                                           main_namespace.ptr()))));
