...
PythonParameterSet::PythonParameterSet() : theParameterSet() {}

void PythonParameterSet::addVPSet(bool tracked, std::string const& name,
                                  boost::python::list value) {
  std::vector<PythonParameterSet> v = edm::toVector<PythonParameterSet>(value);
...
boost::python::list PythonParameterSet::getVPSet(bool tracked, std::string const& name) {
  std::vector<edm::ParameterSet> const& v =
      (tracked ? theParameterSet.getParameterSetVector(name)
               : theParameterSet.getUntrackedParameterSetVector(name));
...
void PythonParameterSet::addNewFileInPath(bool tracked, std::string const& name,
