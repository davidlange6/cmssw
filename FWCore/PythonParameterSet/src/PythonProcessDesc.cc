...
void PythonProcessDesc::readFile(std::string const& fileName) {
  ...
  handle<>(PyRun_String(initCommand.c_str(), Py_file_input, theMainNamespace.ptr(),
                        theMainNamespace.ptr()));
  std::string command("process.fillProcessDesc(processPSet)");
  handle<>(
      PyRun_String(command.c_str(), Py_eval_input, theMainNamespace.ptr(), theMainNamespace.ptr()));
}
...
void PythonProcessDesc::readString(std::string const& pyConfig) {
  ...
  handle<>(
      PyRun_String(command.c_str(), Py_file_input, theMainNamespace.ptr(), theMainNamespace.ptr()));
}
