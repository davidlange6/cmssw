...
void testmakepset::secsourceAux() {
  ...
  CPPUNIT_ASSERT(secondarySourceParams.getParameter<std::string>("@module_type") ==
                 "EmbeddedRootSource");
  CPPUNIT_ASSERT(secondarySourceParams.getParameter<std::string>("@module_label") == "input");
  CPPUNIT_ASSERT(secondarySourceParams.getUntrackedParameter<std::vector<std::string> >(
...
void testmakepset::fileinpathAux() {
  ...
  bool localArea =
      (releaseBase != nullptr && strlen(releaseBase) != 0 && strcmp(releaseBase, localBase));
  if (localArea) {
  ...
  std::string tmpout =
      fullpath.substr(0, fullpath.find("FWCore/PythonParameterSet/test/fip.txt")) + "tmp.py";
...
void testmakepset::typesTest() {
  ...
  char const* kTest =
      ...
      "cms.InputTag('l3','i3','p3'), cms.InputTag('l4','','p4'), cms.InputTag('source'), \n"
