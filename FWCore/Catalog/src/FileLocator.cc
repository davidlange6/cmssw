...
namespace {
  ...
  std::string replaceWithRegexp(std::smatch const& matches, std::string const& outputFormat) {
    ...
    for (size_t i = 1; i < matches.size(); ++i) {
      ...
      if (!matchedString.empty()) boost::algorithm::replace_all(result, str.str(), matchedString);
    }
...
namespace edm {
  ...
  std::string FileLocator::convert(std::string const& input, ProtocolRules const& rules,
                                   bool direct) const {
    std::string out = "";
  ...
  void FileLocator::parseRule(DOMNode* ruleNode, ProtocolRules& rules) {
    ...
    std::string destinationMatchRegexp =
        toString(ruleElement->getAttribute(uStr("destination-match").ptr()));
  ...
  std::string FileLocator::applyRules(ProtocolRules const& protocolRules,
                                      std::string const& protocol, std::string const& destination,
                                      bool direct, std::string name) const {
