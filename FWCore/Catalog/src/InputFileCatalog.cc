...
namespace edm {
  ...
  InputFileCatalog::InputFileCatalog(std::vector<std::string> const& fileNames,
                                     std::string const& override, bool useLFNasPFNifLFNnotFound)
  ...
  void InputFileCatalog::init(std::string const& inputOverride,
    ...
    for (iter it = fileNames_.begin(), lt = logicalFileNames_.begin(), itEnd = fileNames_.end(),
      ...
      if (isPhysical(*it)) {
        if (it->back() == ':') {
          throw Exception(errors::Configuration, "InputFileCatalog::InputFileCatalog()\n")
              << "An empty physical file name specified in the fileNames parameter for input "
                 "source.\n";
        }
        // Clear the LFN.
        lt->clear();
        ...
        if (!overrideFileLocator_ && !inputOverride.empty()) {
          overrideFileLocator_ = std::make_unique<FileLocator>(
              inputOverride, false);  // propagate_const<T> has no reset() function
        }
        ...
        if (!overrideFallbackFileLocator_ && !inputOverrideFallback.empty()) {
          overrideFallbackFileLocator_ = std::make_unique<FileLocator>(
              inputOverrideFallback, true);  // propagate_const<T> has no reset() function
        }
        boost::trim(*lt);
