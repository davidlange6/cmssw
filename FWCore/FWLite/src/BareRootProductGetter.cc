...
edm::WrapperBase const* BareRootProductGetter::getIt(edm::ProductID const& pid) const {
  ...
  if (nullptr == eventTree) {
    throw cms::Exception("NoEventsTree")
        << "unable to find the TTree '" << edm::poolNames::eventTreeName()
        << "' in the last open file, \n"
        << "file: '" << branchMap_.getFile()->GetName()
        << "'\n Please check that the file is a standard CMS ROOT format.\n"
        << "If the above is not the file you expect then please open your data file after all "
           "other files.";
  }
  ...
  if (eventEntry < 0) {
    throw cms::Exception("GetEntryNotCalled")
        << "please call GetEntry for the 'Events' TTree for each event in order to make "
           "edm::Ref's work."
...
edm::WrapperBase const* BareRootProductGetter::getIt(edm::BranchID const& branchID,
  ...
  if (buffer->eventEntry_ != eventEntry) {
    ...
    edm::WrapperBase const* prod = static_cast<edm::WrapperBase const*>(
        buffer->class_->DynamicCast(edproductTClass, address, true));

    if (nullptr == prod) {
      cms::Exception("FailedConversion")
          << "failed to convert a '" << buffer->class_->GetName() << "' to a edm::WrapperBase."
  ...
  if (!buffer->product_) {
    throw cms::Exception("BranchGetEntryFailed")
        << "Calling GetEntry with index " << eventEntry << "for branch "
        << buffer->branch_->GetName() << " failed.";
  }
...
edm::WrapperBase const* BareRootProductGetter::getThinnedProduct(edm::ProductID const& pid,
  ...
  edm::ThinnedAssociationsHelper const& thinnedAssociationsHelper =
  ...
  for (auto associatedBranches = thinnedAssociationsHelper.parentBegin(parent),
    ...
    if (associatedBranches->parent() !=
        branchMap_.productToBranchID(thinnedAssociation->parentCollectionID())) {
      continue;
    }
...
void BareRootProductGetter::getThinnedProducts(
    edm::ProductID const& pid, std::vector<edm::WrapperBase const*>& foundContainers,
...
BareRootProductGetter::Buffer* BareRootProductGetter::createNewBuffer(
  ...
  static TClass const* edproductTClass = TClass::GetClass(typeid(edm::WrapperBase));
  edm::WrapperBase const* prod = static_cast<edm::WrapperBase const*>(
      rootClassType->DynamicCast(edproductTClass, address, true));
  if (nullptr == prod) {
...
edm::ThinnedAssociation const* BareRootProductGetter::getThinnedAssociation(
  ...
  if (wrapperBase == nullptr) {
    throw edm::Exception(edm::errors::LogicError) << "BareRootProductGetter::"
                                                     "getThinnedAssociation, product "
                                                     "ThinnedAssociation not found.\n";
  }
  if (!(typeid(edm::ThinnedAssociation) == wrapperBase->dynamicTypeInfo())) {
    throw edm::Exception(edm::errors::LogicError) << "BareRootProductGetter::"
                                                     "getThinnedAssociation, product has wrong "
                                                     "type, not a ThinnedAssociation.\n";
  }
