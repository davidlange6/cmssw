...
namespace fwlite {
  namespace internal {
    ...
    TBranch* Strategy::getBranchRegistry(edm::ProductRegistry** ppReg) {
      ...
      TTree* metaDataTree =
          dynamic_cast<TTree*>(currentFile_->Get(edm::poolNames::metaDataTreeName().c_str()));
    ...
    bool BranchMapReaderStrategyV8::updateFile(TFile* file) {
      ...
      TTree* metaDataTree =
          dynamic_cast<TTree*>(currentFile_->Get(edm::poolNames::eventMetaDataTreeName().c_str()));
      if (nullptr != metaDataTree) {
        entryInfoBranch_ =
            metaDataTree->GetBranch(BranchTypeToBranchEntryInfoBranchName(edm::InEvent).c_str());
    ...
    bool BranchMapReaderStrategyV11::updateFile(TFile* file) {
      ...
      eventHistoryTree_ =
          dynamic_cast<TTree*>(currentFile_->Get(edm::poolNames::eventHistoryTreeName().c_str()));
    ...
    bool BranchMapReaderStrategyV11::updateMap() {
      if (!mapperFilled_) {
        TBranch* eventHistoryBranch =
            eventHistoryTree_->GetBranch(edm::poolNames::eventHistoryBranchName().c_str());
    ...
    bool BranchMapReaderStrategyV17::updateFile(TFile* file) {
      ...
      if (metaDataTree->FindBranch(edm::poolNames::thinnedAssociationsHelperBranchName().c_str()) !=
          nullptr) {
        TBranch* b =
            metaDataTree->GetBranch(edm::poolNames::thinnedAssociationsHelperBranchName().c_str());
      ...
      eventsTree_ =
          dynamic_cast<TTree*>(currentFile_->Get(edm::poolNames::eventTreeName().c_str()));
  ...
  int BranchMapReader::getFileVersion(TFile* file) {
    TTree* metaDataTree =
        dynamic_cast<TTree*>(file->Get(edm::poolNames::metaDataTreeName().c_str()));
    if (nullptr == metaDataTree) {
      return 0;
    }
  ...
  bool BranchMapReader::updateFile(TFile* file) {
    ...
    if (!isNew) {
      // could still have a new TFile which just happens to share the same memory address as the
      // previous file
      // will assume that if the Event tree's address and UUID are the same as before then we do
      // not have
      // to treat this like a new file
      TTree* eventTreeTemp =
          dynamic_cast<TTree*>(currentFile->Get(edm::poolNames::eventTreeName().c_str()));
      isNew =
          eventTreeTemp != strategy_->eventTree_ || strategy_->fileUUID_ != currentFile->GetUUID();
    }
    if (isNew) {
