...
int main(int argc, char* argv[]) try {
  ...
  descString +=
      "] file_name"
      "\n The program dumps information about how much storage space is needed to store a given "
      "TBranch within a ROOT file"
      "\nAllowed options";
  boost::program_options::options_description desc(descString);
  desc.add_options()(kHelpCommandOpt, "show this help message")(
      kEntryNumberCommandOpt, boost::program_options::value<int>(),
      "read branch from the given entry value (default 0)")(kBranchNameOpt, "name of branch")(
  ...
  try {
    store(boost::program_options::command_line_parser(argc, argv).options(desc).positional(p).run(),
          vm);
    notify(vm);
  ...
  std::cout << "\nNOTE: add 4 bytes for each 'has written' value because of a bug in ROOT's "
               "printout of the accounting"
            << "\n  Each class (inheriting or as member data) has metadata an overhead of 10 bytes"
            << std::endl;
  return 0;
