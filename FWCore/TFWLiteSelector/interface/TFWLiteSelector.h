...
/**\class TFWLiteSelector TFWLiteSelector.h FWCore/TFWLiteSelector/interface/TFWLiteSelector.h
 ...
 methods
    1) a constructor which takes a 'const TList*' and a 'TList&' as its arguments.  The 'const
 TList*' holds
       objects The 'TList&' is used to hold all the
       'TObject' items (e.g. histograms) you want to access for the final result (e.g. the sum of
 all
       histograms created by the many Workers running on different computers).  You should create
 the
       items in the constructor, hold onto them as member data in the Worker and 'Add' them to the
 TList.
    ...
    You should inherit from TFWLiteSelector<...> where the template argument should be the worker
 you want
    to use.  You need to implement the following methods
    1) 'begin(const TList*& itemsForProcessing)'
      this is called before processing has started and before any workers get created.  If you want
 to pass data to
      your workers, you can create a new TList and assign it to 'itemsForProcessing' and then add
 the objects you
      want passed into that list.
      NOTE: you are responsible for deleting the created TList and for deleting all items held by
 the TList. The easiest
      way to do this is to add a 'std::unique_ptr<TList>' member data to your Selector and then call
 'SetOwner()' on the TList.
    2) 'terminate(TList&)'
       this is called after all processing has finished.  The TList& contains all the accumulated
 information
...
private:
  ...
  virtual void process(const edm::Event& iEvent) { worker_->process(iEvent); }
  virtual void postProcessing(TList& out) { worker_->postProcess(out); }
