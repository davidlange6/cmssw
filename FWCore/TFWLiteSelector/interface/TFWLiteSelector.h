...
/**\class TFWLiteSelector TFWLiteSelector.h FWCore/TFWLiteSelector/interface/TFWLiteSelector.h
 ...
 Usage:
    ...
    1) 'begin(const TList*& itemsForProcessing)'
      ...
      way to do this is to add a 'std::unique_ptr<TList>' member data to your Selector and then call 'SetOwner()' on the
 TList.
    2) 'terminate(TList&)'
...
private:
  ...
  virtual void preProcessing(const TList* in, TList& out) { worker_ = std::make_shared<TWorker>(in, out); }
