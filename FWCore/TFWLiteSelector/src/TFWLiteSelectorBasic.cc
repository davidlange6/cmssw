...
namespace edm {
  namespace root {
    ...
    private:
      ...
      virtual std::unique_ptr<WrapperBase> getProduct_(BranchKey const& k,
                                                       EDProductGetter const* ep) override;
    ...
    std::unique_ptr<WrapperBase> FWLiteDelayedReader::getProduct_(BranchKey const& k,
...
Bool_t TFWLiteSelectorBasic::Process(Long64_t iEntry) {
  ...
  if (everythingOK_) {
    ...
    TBranch* branch =
        m_->tree_->GetBranch(edm::BranchTypeToAuxiliaryBranchName(edm::InEvent).c_str());
    ...
    TBranch* eventSelectionsBranch =
        m_->tree_->GetBranch(edm::poolNames::eventSelectionsBranchName().c_str());
    ...
    TBranch* branchListIndexBranch =
        m_->tree_->GetBranch(edm::poolNames::branchListIndexesBranchName().c_str());
    ...
    try {
      ...
      auto lumiAux =
          std::make_shared<edm::LuminosityBlockAuxiliary>(rp->run(), 1, aux.time(), aux.time());
      auto lbp =
          std::make_shared<edm::LuminosityBlockPrincipal>(lumiAux, m_->reg(), m_->pc_, nullptr, 0);
      m_->ep_->fillEventPrincipal(*eaux, *m_->phreg_, std::move(eventSelectionIDs),
                                  std::move(branchListIndexes), *(m_->provRetriever_),
                                  m_->reader_.get());
      ...
      std::cout << "While processing entry " << iEntry << " an unknown exception was caught"
                << std::endl;
    }
  }
  return everythingOK_ ? kTRUE : kFALSE;
...
void TFWLiteSelectorBasic::setupNewFile(TFile& iFile) {
  ...
  if (metaDataTree->FindBranch(edm::poolNames::parameterSetMapBranchName().c_str()) != nullptr) {
    metaDataTree->SetBranchAddress(edm::poolNames::parameterSetMapBranchName().c_str(),
                                   &psetMapPtr);
  } else {
    TTree* psetTree =
        dynamic_cast<TTree*>(iFile.Get(edm::poolNames::parameterSetsTreeName().c_str()));
    if (nullptr == psetTree) {
    ...
    psetTree->SetBranchAddress(edm::poolNames::idToParameterSetBlobsBranchName().c_str(),
                               &pIdToBlob);
  ...
  if (metaDataTree->FindBranch(edm::poolNames::processHistoryBranchName().c_str()) != nullptr) {
    metaDataTree->SetBranchAddress(edm::poolNames::processHistoryBranchName().c_str(),
                                   &pHistVectorPtr);
  }

  edm::ProcessConfigurationVector procConfigVector;
  edm::ProcessConfigurationVector* procConfigVectorPtr = &procConfigVector;
  if (metaDataTree->FindBranch(edm::poolNames::processConfigurationBranchName().c_str()) !=
      nullptr) {
    metaDataTree->SetBranchAddress(edm::poolNames::processConfigurationBranchName().c_str(),
                                   &procConfigVectorPtr);
  }
  ...
  for (auto& item : prodList2) {
    ...
    if (prod.branchType() == edm::InEvent) {
      ...
      // CDJ need to fix provenance and be backwards compatible, for now just don't read the
      // branch
