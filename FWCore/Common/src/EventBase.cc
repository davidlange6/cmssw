...
namespace {
  ...
  typedef tbb::concurrent_unordered_map<edm::ParameterSetID, edm::TriggerNames, key_hash>
      TriggerNamesMap;
  ...
  TriggerNames const* EventBase::triggerNames_(edm::TriggerResults const& triggerResults) {
    ...
    if (0 != (pset = psetRegistry->getMapped(triggerResults.parameterSetID()))) {
      if (pset->existsAs<std::vector<std::string> >("@trigger_paths", true)) {
        ...
        std::pair<TriggerNamesMap::iterator, bool> ret =
            triggerNamesMap.insert(std::pair<edm::ParameterSetID, edm::TriggerNames>(
                triggerResults.parameterSetID(), triggerNames));
    ...
    if (triggerResults.getTriggerNames().size() > 0U) {
      ...
      std::pair<TriggerNamesMap::iterator, bool> ret = triggerNamesMap.insert(
          std::pair<edm::ParameterSetID, edm::TriggerNames>(fakePset.id(), triggerNames));
      return &(ret.first->second);
    }
    return 0;
  }
