  ...
  TriggerResultsByName::TriggerResultsByName(TriggerResults const* triggerResults,
    ...
    if (triggerResults_->parameterSetID() != triggerNames_->parameterSetID()) {
      throw edm::Exception(edm::errors::Unknown)
          << "TriggerResultsByName::TriggerResultsByName, Trigger names vector and "
             "TriggerResults\n"
             "have different ParameterSetID's.  This should be impossible when the object is "
             "obtained\n"
             "from the function named triggerResultsByName in the Event class, which is the "
             "way\n"
