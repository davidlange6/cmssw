...
namespace edm {
  namespace serviceregistry {
    ...
    ServicesManager::MakerHolder::MakerHolder(std::shared_ptr<ServiceMakerBase> iMaker,
                                              ParameterSet& iPSet, ActivityRegistry& iRegistry)
    ...
    ServicesManager::ServicesManager(ServiceToken iToken, ServiceLegacy iLegacy,
      ...
      if (0 != iToken.manager_.get()) {
        ...
        std::set_intersection(configTypes.begin(), configTypes.end(), tokenTypes.begin(),
                              tokenTypes.end(), inserter(intersection, intersection.end()));
        ...
        switch (iLegacy) {
          ...
          case kTokenOverrides:
            ...
            for (IntersectionType::iterator itType = intersection.begin(),
              ...
              // HLT needs it such that even if a service isn't created we store its PSet if
              // needed
          ...
          case kConfigurationOverrides:
            ...
            for (IntersectionType::iterator itType = intersection.begin(),
              ...
              if (itFound->second.maker_->processWideService()) {
                ...
                // HLT needs it such that even if a service isn't created we store its PSet
                // if needed
                if (itFound->second.maker_->saveConfiguration()) {
                  itFound->second.pset_->addUntrackedParameter("@save_config", true);
                }
                ...
                // This is not a process wide service, so the configuration overrides the
                // token.
      ...
      for (std::vector<ParameterSet>::iterator itParam = iConfiguration.begin(),
        ...
        if (0 == base.get()) {
          throw Exception(errors::Configuration, "Service")
              ...
              << itParam->getParameter<std::string>("@service_type") << ". Please check spelling.";
        }
