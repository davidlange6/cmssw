...
namespace edm {
  ...
  StreamContext const* ModuleCallingContext::getStreamContext() const {
    ...
    if (mcc->type() == ParentContext::Type::kPlaceInPath) {
      ...
      throw Exception(errors::LogicError) << "ModuleCallingContext::getStreamContext() called "
                                             "in context not linked to a StreamContext\n";
    }
  ...
  GlobalContext const* ModuleCallingContext::getGlobalContext() const {
    ...
    if (mcc->type() != ParentContext::Type::kGlobal) {
      throw Exception(errors::LogicError) << "ModuleCallingContext::getGlobalContext() called "
                                             "in context not linked to a GlobalContext\n";
    }
