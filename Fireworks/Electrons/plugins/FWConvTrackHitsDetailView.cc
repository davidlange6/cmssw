...
FWConvTrackHitsDetailView::FWConvTrackHitsDetailView()
    : m_modules(nullptr), m_moduleLabels(nullptr), m_hits(nullptr), m_legend(nullptr), m_orthographic(false) {}

FWConvTrackHitsDetailView::~FWConvTrackHitsDetailView() {}

namespace {
  void setCameraInit(
      TGLViewer* v, TGLViewer::ECameraType type, const TEveVectorD b1, TEveVectorD b3, TEveVector center) {
...
void FWConvTrackHitsDetailView::build(const FWModelId& id, const reco::Conversion* conv) {
  if (conv->nTracks() < 2)
    return;
...
void FWConvTrackHitsDetailView::setTextInfo(const FWModelId& id, const reco::Conversion* conv) {
  m_infoCanvas->cd();
  ...
  latex->DrawLatex(x,
                   y,
                   Form("p_{T}=%.1f GeV, #eta=%0.2f, #varphi=%0.2f",
                        sqrt(conv->pairMomentum().Perp2()),
                        conv->pairMomentum().eta(),
                        conv->pairMomentum().phi()));
  y -= lineH;
  latex->DrawLatex(x,
                   y,
                   Form("vtx=(%.1f, %.1f, %.1f) r=%.1f [cm]",
                        conv->conversionVertex().x(),
                        conv->conversionVertex().y(),
                        conv->conversionVertex().z(),
                        conv->conversionVertex().position().rho()));
  y -= lineH;
  latex->DrawLatex(x, y, Form("#Deltactg#theta=%.3f", conv->pairCotThetaSeparation()));
  y -= lineH;
  latex->DrawLatex(x, y, Form("#Delta#phi_{vtx}=%.3f", conv->dPhiTracksAtVtx()));
  y -= lineH;
  latex->DrawLatex(x, y, Form("dist. min. app.=%.3f cm", conv->distOfMinimumApproach()));
  y -= lineH;
  y -= lineH;
  ...
  y -= lineH;
  latex->DrawLatex(x, y, Form("p_{T}=%.1f GeV, #eta=%0.2f, #varphi=%0.2f", track0->pt(), track0->eta(), track0->phi()));
  y -= lineH;
  ...
  for (int i_tk0 = 0; i_tk0 < hp0.numberOfAllHits(reco::HitPattern::TRACK_HITS); i_tk0++) {
    ...
    if (reco::HitPattern::validHitFilter(hit)) {
      nvalid_tk0++;
      if (reco::HitPattern::pixelHitFilter(hit))
        npix_tk0++;
      else if (reco::HitPattern::stripHitFilter(hit))
        nstrip_tk0++;
    } else
      ninvalid_tk0++;
  }
  ...
  y -= lineH;
  latex->DrawLatex(x, y, Form("p_{T}=%.1f GeV, #eta=%0.2f, #varphi=%0.2f", track1->pt(), track1->eta(), track1->phi()));
  y -= lineH;
