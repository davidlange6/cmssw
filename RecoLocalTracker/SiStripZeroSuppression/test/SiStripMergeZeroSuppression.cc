      ...
      if (isModuleRestored) {
        ...
        if (!suppressedDigis.empty()) {
          LogTrace("SiStripMergeZeroSuppression::produce")
              << "Looking for the detId with the new ZS in the collection of the zero suppressed data"
              << "\n";
          auto zsModule = std::lower_bound(
              std::begin(outputdigi),
              std::end(outputdigi),
              rawDetId,
          ...
          LogTrace("SiStripMergeZeroSuppression::produce")
              << "After the look " << rawDetId << " ==== " << (isModuleInZscollection ? zsModule->id : 0) << "\n";
          LogTrace("SiStripMergeZeroSuppression::produce")
              << "Exiting looking for the detId with the new ZS in the collection of the zero suppressed data"
              << "\n";
          ...
          if (!isModuleInZscollection) {
            ...
            for (const auto itZsMod : *zsModule) {
              ...
              if (!restoredAPV[strip / 128]) {
                ...
                ++count;
                LogTrace("SiStripMergeZeroSuppression::produce")
                    << "original count: " << count << " strip: " << strip << " adc: " << adc << "\n";
              }
            }
          }
          ...
          for (const auto itSuppDigi : suppressedDigis) {
            ...
            if (restoredAPV[strip / 128]) {
              mergedRawDigis[strip] = adc;
              LogTrace("SiStripMergeZeroSuppression::produce")
                  << "new suppressed strip: " << strip << " adc: " << adc << "\n";
            }
          }
          ...
          for (uint16_t strip = 0; strip < mergedRawDigis.size(); ++strip) {
            ...
            if (adc)
              zsModule->push_back(SiStripDigi(strip, adc));
          }
          LogTrace("SiStripMergeZeroSuppression::produce")
              << "size zsModule after the merging: " << zsModule->size() << "\n";
          if ((count + suppressedDigis.size()) != zsModule->size())
            LogTrace("SiStripMergeZeroSuppression::produce")
                << "WE HAVE A PROBLEM!!!! THE NUMBER OF DIGIS IS NOT RIGHT=============="
                << "\n";
