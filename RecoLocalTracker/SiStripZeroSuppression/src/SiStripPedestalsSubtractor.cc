...
void SiStripPedestalsSubtractor::subtract(uint32_t detId, uint16_t firstStrip, std::vector<int16_t>& digis) {
  subtract_(detId, firstStrip, digis, digis);
}
...
inline void SiStripPedestalsSubtractor::subtract_(uint32_t id,
                                                  uint16_t firstStrip,
                                                  const input_t& input,
  ...
  try {
    ...
    edm::LogError("SiStripPedestalsSubtractor") << "[SiStripPedestalsSubtractor::subtract] DetId " << id
                                                << " propagating error from SiStripPedestal" << e.what();
    output.clear();
  }
}
