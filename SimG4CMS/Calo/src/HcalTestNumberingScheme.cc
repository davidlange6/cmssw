...
HcalTestNumberingScheme::HcalTestNumberingScheme(bool forTB) : HcalNumberingScheme(), forTBH2(forTB) {
  edm::LogVerbatim("HcalSim") << "Creating HcalTestNumberingScheme with TB Flag " << forTBH2;
}
...
uint32_t HcalTestNumberingScheme::getUnitID(const HcalNumberingFromDDD::HcalID& id) {
  ...
  if (forTBH2) {
    ...
    // Test case
    index = HcalTestNumbering::packHcalIndex(id.subdet, id.zside, id.depth, id.etaR, id.phis, id.lay);
  }
#ifdef EDM_ML_DEBUG
  edm::LogVerbatim("HcalSim") << "HcalTestNumberingScheme det = " << id.subdet << " depth/lay = " << id.depth << "/"
                              << id.lay << " zside = " << id.zside << " eta/R = " << id.etaR << " phi = " << id.phis
                              << " packed index = 0x" << std::hex << index << std::dec << std::endl;
#endif
  return index;
}

uint32_t HcalTestNumberingScheme::packHcalIndex(int det, int z, int depth, int eta, int phi, int lay) {
