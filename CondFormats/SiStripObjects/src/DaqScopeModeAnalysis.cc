...
uint16_t DaqScopeModeAnalysis::frameFindingThreshold() const {
  if ((getErrorCodes().empty() || getErrorCodes()[0] == "TickMarkRecovered") && base_ < sistrip::valid_ &&
      peak_ < sistrip::valid_ && height_ < sistrip::valid_ && height_ > tickMarkHeightThreshold_) {
    return ((static_cast<uint16_t>(base_ + height_ * DaqScopeModeAnalysis::frameFindingThreshold_) / 32) * 32);
  } else {
...
bool DaqScopeModeAnalysis::foundTickMark() const {
  return ((getErrorCodes().empty() || getErrorCodes()[0] == "TickMarkRecovered") && base_ < sistrip::valid_ &&
          peak_ < sistrip::valid_ && height_ < sistrip::valid_ && frameFindingThreshold() < sistrip::valid_);
}

// ----------------------------------------------------------------------------
//
bool DaqScopeModeAnalysis::isValid() const {
  return (getErrorCodes().empty() && base_ < sistrip::valid_ && peak_ < sistrip::valid_ && height_ < sistrip::valid_ &&
          frameFindingThreshold() < sistrip::valid_ && pedsMean_[0] < sistrip::maximum_ &&
          pedsMean_[1] < sistrip::maximum_ && pedsSpread_[0] < sistrip::maximum_ &&
          ...
          rawMin_[0] < sistrip::maximum_ && rawMin_[1] < sistrip::maximum_);
}
...
void DaqScopeModeAnalysis::print(std::stringstream& ss, uint32_t iapv) {
  ...
  header(ss);
  ss << std::fixed << std::setprecision(2) << " Tick mark bottom (baseline)       [ADC] : " << base_ << std::endl
