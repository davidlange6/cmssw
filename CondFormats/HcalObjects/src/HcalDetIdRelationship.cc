...
bool hcalEqualDetId(uint32_t id, const DetId& fId) {
  return ((fId.det() == DetId::Hcal && HcalDetId(id) == HcalDetId(fId)) ||
          (fId.det() == DetId::Calo && fId.subdetId() == HcalZDCDetId::SubdetectorId &&
           HcalZDCDetId(id) == HcalZDCDetId(fId)) ||
          (fId.det() != DetId::Hcal && (fId.det() == DetId::Calo && fId.subdetId() != HcalZDCDetId::SubdetectorId) &&
           (id == fId.rawId())));
}
